...
generate_rpm_spec() {
    ...
    if [ "$WITH_PKGS" != "" ] ; then
        ...
        # exclude other packages by replacing
        #   %package_option <package>
        # with
        #   %global with_<package> 0
        commands="${commands}; s/^%package_option *\(.*\)\$/%global with_\1 0/g"

    elif [ "$WITHOUT_PKGS" != "" ] ; then

        # use exclusion method by replacing
        #   %bcond_with pkgs
        # with
        #   %global with_pkgs 0
        commands="${commands}; s/%bcond_with *pkgs\$/%global with_pkgs 0/g"

        # exclude specified packages by replacing
        #   %package_option <package>
        # with
        #   %global with_<package> 0
        for package in `echo $WITHOUT_PKGS | sed 's/,/\n/g'`
        do
            commands="${commands}; s/^%package_option *$package\$/%global with_$package 0/g"
        done
