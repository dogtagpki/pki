...
{
    rlPhaseStartTest "run_rhcs_install_topo_default - install ROOTCA on Host1"
    ...
    rlPhaseStartTest "run_rhcs_install_topo_default - install CLONE1 on Host2"
	rlLog "In default topo function"
    ...
    rlPhaseStartTest "run_rhcs_install_topo_default - install Subca1 on Host3"
...
{
    rlPhaseStartTest "run_rhcs_install_topo_5 - Install ROOTCA on Host 1"
	rlLog "Topo_5 - run_rhcs_install_topo_5"
	local CA=ROOTCA
	local number=3
	if [ "$(hostname)" = "$BEAKERMASTER" ]; then
            	run_rhcs_install_packages
    ...
    rlPhaseStartTest "run_rhcs_install_topo_5 - install SUBCA1 on Host2"
    	rlLog "In default topo function"
	if [ "$(hostname)" = "$BEAKERSUBCA1" ]; then
        	rlRun "rhts-sync-block -s 'Master instances installed' $BEAKERMASTER"
                local CA=ROOTCA
    ...
    rlPhaseStartTest "run_rhcs_install_topo_5 - install SUBCA2 on Host3"
...
{
    rlPhaseStartTest "install_topo_6 - Install RootCA on Host1"
    ...
    rlPhaseEnd
    rlPhaseStartTest "run_rhcs_install_topo_6 - install CLONE1 on Host3"
        if [ "$(hostname)" = "$BEAKERCLONE1" ]; then
                rlRun "rhts-sync-block -s 'SUBCA1 instances installed' $BEAKERSUBCA1"
                local CA=SUBCA1
                local number=1
...
{
    rlPhaseStartTest "run_rhcs_install_topo_7 - Install Master, Subca and Clone"
    ...
    rlPhaseEnd
    rlPhaseStartTest "run_rhcs_install_topo_7 - Install SUbCA1 on Host 2"
    ...
    rlPhaseEnd
    rlPhaseStartTest "run_rhcs_install_topo_7 - install Host3 in Default Topology"
    	if [ "$(hostname)" = "$BEAKERCLONE1" ]; then
        	rlRun "rhts-sync-block -s 'SUBCA1 instances installed' $BEAKERSUBCA1"
                local CA=SUBCA1
                local number=1
...
run_rhcs_install_topo_1()
{
    ...
    rlPhaseEnd 
    rlPhaseStartTest "run_rhcs_install_topo_1 - Install SubCA1 on Host2"
	rlLog "In subca topo1"
	if [ "$(hostname)" = "$BEAKERSUBCA1" ]; then
		rlLog "inside if"	
		rlRun "rhts-sync-block -s 'Master Instances Installed' $BEAKERMASTER"
		local number=1
...
{
    rlPhaseStartTest "run_rhcs_install_topo_2 - Install RootCA on host 1"
	rlLog "In topo2"
	if [ "$(hostname)" = "$BEAKERMASTER" ]; then
		local number=3
		local CA=ROOTCA
    ...
    rlPhaseEnd
    rlPhaseStartTest "run_rhcs_install_topo_2 - Install SUBCA1 on Host 2"
	rlLog "in topo2"
	if [ "$(hostname)" = "$BEAKERSUBCA1" ]; then
		local CA=ROOTCA
		local number=1
...
{
    rlPhaseStartTest "run_rhcs_install_topo_4 - Install ROOTCA Host 1"
 	if [ "$(hostname)" = "$BEAKERMASTER" ]; then
        	local CA=ROOTCA
		local number=3
		run_rhcs_install_packages
