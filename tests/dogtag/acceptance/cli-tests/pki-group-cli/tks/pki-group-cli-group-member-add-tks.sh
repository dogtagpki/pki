...
#   Description: PKI group-cli-group-membership-add-tks CLI tests
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# The following pki cli commands needs to be tested:
#  pki-group-cli-group-member-add-tks    Add group member.
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#
...
. /opt/rhqa_pki/env.sh
######################################################################################
#create_role_users.sh should be first executed prior to pki-group-cli-group-member-add-tks.sh
######################################################################################

########################################################################
# Test Suite Globals
########################################################################
run_pki-group-cli-group-member-add-tks_tests(){
	rlPhaseStartSetup "pki_group_cli_group_membership-add-tks-001: Create temporary directory"
...
groupid1="Token Key Service Manager Agents"
	...
	rlPhaseStartTest "pki_group_cli_group_member-add-tks-001: Add users to available groups using valid admin user TKS_adminV"
		i=1
		while [ $i -lt 7 ] ; do
		    ...
		    -t tks \
		       ...
		       rlRun "pki -d $CERTDB_DIR \
				  -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
				   user-add --fullName=\"fullNameu$i\" u$i > $TmpDir/pki-tks-group-member-add-group-add-00$i.out" \
				   0 \
				   "Adding user u$i"
			...
			rlRun "pki -d $CERTDB_DIR \
				   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
				    user-show u$i > $TmpDir/pki-tks-group-member-add-group-show-00$i.out" \
				    0 \
			...
			rlRun "pki -d $CERTDB_DIR \
				   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                                    group-member-add \"$gid\" u$i > $TmpDir/pki-tks-group-member-add-groupadd-00$i.out" \
                                    0 \
			...
			rlLog "Check if the user is added to the group"
				   ...
				   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                                    group-member-find \"$gid\" > $TmpDir/pki-tks-group-member-add-groupadd-find-00$i.out" \
                                    0 \
	...
	rlPhaseStartTest "pki_group_cli_group_member-add-tks-002: Add a user to all available groups using TKS_adminV"
		rlRun "pki -d $CERTDB_DIR \
			   -n $(eval echo \$${subsystemId}_adminV_user) \
                    -c $CERTDB_DIR_PASSWORD \
                    -h $TKS_HOST \
                    -p $TKS_PORT \
                    -t tks \
                            user-add --fullName=\"fullName_userall\" userall > $TmpDir/pki-tks-group-member-add-user-add-userall-001.out" \
                            0 \
                            "Adding user userall"
		rlAssertGrep "Added user \"userall\"" "$TmpDir/pki-tks-group-member-add-user-add-userall-001.out"
			   ...
			   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                            user-show userall > $TmpDir/pki-tks-group-member-add-user-show-userall-001.out" \
                            0 \
                            "Show pki TKS_adminV user"
		...
		while [ $i -lt 7 ] ; do
			...
			rlRun "pki -d $CERTDB_DIR \
				   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
				    group-member-add \"$gid\" userall > $TmpDir/pki-tks-group-member-add-groupadd-userall-00$i.out" \
				    0 \
			...
			rlRun "pki -d $CERTDB_DIR \
				   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
				    group-member-find \"$gid\" > $TmpDir/pki-tks-group-member-add-groupadd-find-userall-00$i.out" \
				    0 \
				    "User added to group \"$gid\""
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_group_cli_group_member-add-tks-003: Add a user to same group multiple times"
                rlRun "pki -d $CERTDB_DIR \
			   -n $(eval echo \$${subsystemId}_adminV_user) \
                    -c $CERTDB_DIR_PASSWORD \
                    -h $TKS_HOST \
                    -p $TKS_PORT \
                    -t tks \
                            user-add --fullName=\"fullName_user1\" user1 > $TmpDir/pki-tks-group-member-add-user-add-user1-001.out" \
                            0 \
                            "Adding user user1"
			   ...
			   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                            user-show user1 > $TmpDir/pki-tks-group-member-add-user-show-user1-001.out" \
                            0 \
                            "Show pki TKS_adminV user"
		...
		rlRun "pki -d $CERTDB_DIR \
			    -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                            group-member-add \"Administrators\" user1 > $TmpDir/pki-tks-group-member-add-groupadd-user1-001.out" \
                            0 \
		...
		command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_adminV_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT -t tks group-member-add \"Administrators\" user1" 
	...
	rlPhaseStartTest "pki_group_cli_group_member-add-tks-004: should not be able to add user to a non existing group"
		...
		rlRun "pki -d $CERTDB_DIR \
			   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                          user-add --fullName=\"fullName_user1\" testuser1 > $TmpDir/pki-tks-group-member-add-user-add-user1-008.out" \
                            0 \
                            "Adding user testuser1"
		command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_adminV_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT -t tks group-member-add \"$dummy_group\" testuser1"
	...
	rlPhaseStartTest "pki_group_cli_group_member-add-tks-005: Should be able to group-member-add groupid with i18n characters"
                rlLog "pki -d $CERTDB_DIR \
			   -n $(eval echo \$${subsystemId}_adminV_user) \
                    -c $CERTDB_DIR_PASSWORD \
                    -h $TKS_HOST \
		...
		rlRun "pki -d $CERTDB_DIR \
			   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                            group-add 'dadministʁasjɔ̃' --description \"Admininstartors in French\" 2>&1 > $TmpDir/pki-tks-group-member-add-groupadd-010_1.out" \
                            0 \
			    ...
			    -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                            group-member-add \"dadministʁasjɔ̃\" u14 > $TmpDir/pki-tks-group-member-add-groupadd-010_2.out" \
                            0 \
                ...
                rlAssertGrep "User: u14" "$TmpDir/pki-tks-group-member-add-groupadd-010_2.out"
				   ...
				   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                                    group-member-find 'dadministʁasjɔ̃' > $TmpDir/pki-tks-group-member-add-groupadd-find-010_3.out" \
                                    0 \
	...
	rlPhaseStartTest "pki_group_cli_group_member-add-tks-006: Should not be able to group-member-add using a revoked cert TKS_adminR"
                command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_adminR_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT -t tks group-member-add \"$groupid7\" testuser1"
                errmsg="PKIException: Unauthorized"
                errorcode=255
                rlRun "verifyErrorMsg \"$command\" \"$errmsg\" \"$errorcode\"" 0 "Should not be able to group-member-add using a revoked cert TKS_adminR"
		rlLog "PKI Ticket: https://fedorahosted.org/pki/ticket/1134"
	        rlLog "PKI Ticket: https://fedorahosted.org/pki/ticket/1182"
	rlPhaseEnd
	
	rlPhaseStartTest "pki_group_cli_group_member-add-tks-007: Should not be able to group-member-add using an agent with revoked cert TKS_agentR"
		command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_agentR_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT -t tks group-member-add \"$groupid7\" testuser1"
                errmsg="PKIException: Unauthorized"
                errorcode=255
                rlRun "verifyErrorMsg \"$command\" \"$errmsg\" \"$errorcode\"" 0 "Should not be able to group-member-add using an agent with revoked cert TKS_agentR"
		rlLog "PKI Ticket: https://fedorahosted.org/pki/ticket/1134"
	        rlLog "PKI Ticket: https://fedorahosted.org/pki/ticket/1182"
	rlPhaseEnd

	rlPhaseStartTest "pki_group_cli_group_member-add-tks-008: Should not be able to group-member-add using admin user with expired cert TKS_adminE"
		rlRun "date --set='+2 days'" 0 "Set System date 2 days ahead"
                rlRun "date"
                command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_adminE_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT -t tks group-member-add \"Administrators\" testuser1"
                errmsg="ForbiddenException: Authorization Error"
                errorcode=255
                rlRun "verifyErrorMsg \"$command\" \"$errmsg\" \"$errorcode\"" 0 "Should not be able to group-member-add using admin user with expired cert TKS_adminE"
		rlLog "PKI Ticket::  https://fedorahosted.org/pki/ticket/962"
                rlRun "date --set='2 days ago'" 0 "Set System back to the present day"
        rlPhaseEnd

	rlPhaseStartTest "pki_group_cli_group_member-add-tks-009: Should not be able to group-member-add using TKS_agentE cert"
		rlRun "date --set='+2 days'" 0 "Set System date 2 days ahead"
                rlRun "date"
                command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_agentE_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT -t tks group-member-add \"Administrators\" testuser1"
                errmsg="ForbiddenException: Authorization Error"
                errorcode=255
                rlRun "verifyErrorMsg \"$command\" \"$errmsg\" \"$errorcode\"" 0 "Should not be able to group-member-add using TKS_agentE cert"
		rlLog "PKI Ticket::  https://fedorahosted.org/pki/ticket/962"
                rlRun "date --set='2 days ago'" 0 "Set System back to the present day"
        rlPhaseEnd

	rlPhaseStartTest "pki_group_cli_group_member-add-tks-010: Should not be able to group-member-add using TKS_auditV cert"
                command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_auditV_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT -t tks group-member-add \"Administrators\" testuser1"
                errmsg="ForbiddenException: Authorization Error"
                errorcode=255
                rlRun "verifyErrorMsg \"$command\" \"$errmsg\" \"$errorcode\"" 0 "Should not be able to group-member-add using TKS_auditV cert"
	rlPhaseEnd

	rlPhaseStartTest "pki_group_cli_group_member-add-tks-011: Should not be able to group-member-add using TKS_operatorV cert"
                command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_operatorV_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT -t tks group-member-add \"Administrators\" testuser1"
                errmsg="ForbiddenException: Authorization Error"
                errorcode=255
                rlRun "verifyErrorMsg \"$command\" \"$errmsg\" \"$errorcode\"" 0 "Should not be able to group-member-add using TKS_operatorV cert"
	rlPhaseEnd

	rlPhaseStartTest "pki_group_cli_group_member-add-tks-012: Should not be able to group-member-add using TKS_adminUTCA cert"
		command="pki -d $UNTRUSTED_CERT_DB_LOCATION -n role_user_UTCA -c $UNTRUSTED_CERT_DB_PASSWORD -h $TKS_HOST -p $TKS_PORT -t tks group-member-add \"Administrators\" testuser1"
                errmsg="PKIException: Unauthorized"
                errorcode=255
                rlRun "verifyErrorMsg \"$command\" \"$errmsg\" \"$errorcode\"" 0 "Should not be able to group-member-add using TKS_adminUTCA cert"
	rlPhaseEnd

	rlPhaseStartTest "pki_group_cli_group_member-add-tks-013: Should not be able to group-member-add using TKS_agentUTCA cert"
		command="pki -d $UNTRUSTED_CERT_DB_LOCATION -n role_user_UTCA -c $UNTRUSTED_CERT_DB_PASSWORD -h $TKS_HOST -p $TKS_PORT -t tks group-member-add \"Administrators\" testuser1"
                errmsg="PKIException: Unauthorized"
                errorcode=255
                rlRun "verifyErrorMsg \"$command\" \"$errmsg\" \"$errorcode\"" 0 "Should not be able to group-member-add using TKS_agentUTCA cert"
		rlLog "PKI Ticket::  https://fedorahosted.org/pki/ticket/962"
	rlPhaseEnd

	#Usability tests
	rlPhaseStartTest "pki_group_cli_group_member-add-tks-014: User associated with Administrators group only can create a new user"
		i=2
                while [ $i -lt 7 ] ; do
                        eval gid=\$groupid$i
			if [ "$gid" = "Administrators" ] ; then
				rlLog "Not adding testuser1 to $gid group"
                    ...
                    -p $TKS_PORT \
                    -t tks \
                                    group-member-add \"$gid\" testuser1 > $TmpDir/pki-tks-group-member-add-groupadd-testuser1-00$i.out" \
                                    0 \
	...
	rlRun "pki -d $CERTDB_DIR/ \
			   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                            user-cert-add testuser1 --input $TmpDir/pki_tks_group_member_add_encoded_0019pkcs10.pem  > $TmpDir/useraddcert_019_2.out" \
                            0 \
                            "Cert is added to the user testuser1"
		command="pki -d $TEMP_NSS_DB -n testuser1 -c $TEMP_NSS_DB_PASSWD -h $TKS_HOST -p $TKS_PORT -t tks user-add --fullName=test_user u39"
		errmsg="ForbiddenException: Authorization Error"
		errorcode=255
		...
		rlRun "pki -d $CERTDB_DIR \
			   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                            group-member-add \"$groupid4\" testuser1 > $TmpDir/pki-tks-group-member-add-groupadd-usertest1-019_2.out 2>&1" \
                            0 \
			   ...
			   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                            group-member-find $groupid4 > $TmpDir/pki-tks-group-member-add-groupadd-find-usertest1-019_3.out" \
                            0 \
		...
		rlRun "pki -d $TEMP_NSS_DB \
			   ...
			   -h $TKS_HOST \
                    -p $TKS_PORT \
                    -t tks \
			    user-add --fullName=test_user us19 > $TmpDir/pki-tks-user-add-019_4.out 2>&1" \
                            0 \
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_group_cli_group_member-add-tks-015: Should not be able to group-member-add using TKS_agentV cert"
                command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_agentV_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT -t tks group-member-add \"Administrators\" testuser1"
                errmsg="ForbiddenException: Authorization Error"
                errorcode=255
                rlRun "verifyErrorMsg \"$command\" \"$errmsg\" \"$errorcode\"" 0 "Should not be able to group-member-add using TKS_agentV cert"
        rlPhaseEnd	

	#Usability test

	rlPhaseStartTest "pki_group_cli_group_member-add-tks-016: Should not be able to add a non existing user to a group"
		user="tuser3"
		command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_adminV_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT -t tks group-member-add \"$groupid5\" $user"
	...
	rlPhaseStartTest "pki_group_cli_group_member-add-tks-017: Add a group and add a user to the group using valid admin user TKS_adminV"
		       rlLog "pki -d $CERTDB_DIR \
				  -n $(eval echo \$${subsystemId}_adminV_user) \
                    -c $CERTDB_DIR_PASSWORD \
                    -h $TKS_HOST \
		       ...
		       rlRun "pki -d $CERTDB_DIR \
			 	  -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                                   group-add --description=\"g1description\" g1 > $TmpDir/pki-tks-group-member-add-group-add-022.out" \
				   0 \
				   "Adding group g1"
		       rlAssertGrep "Added group \"g1\"" "$TmpDir/pki-tks-group-member-add-group-add-022.out"
				  ...
				  -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                                   user-add --fullName=\"fullNameu9\" u9 > $TmpDir/pki-tks-group-member-add-user-add-022.out" \
                                   0 \
                                   "Adding user u9"
				   ...
				   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                                    group-member-add g1 u9 > $TmpDir/pki-tks-group-member-add-groupadd-022.out" \
                                    0 \
				   ...
				   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                                    group-member-find g1 > $TmpDir/pki-tks-group-member-add-groupadd-find-022.out" \
                                    0 \
                                    "User added to group g1"
                        rlAssertGrep "User: u9" "$TmpDir/pki-tks-group-member-add-groupadd-find-022.out"
        rlPhaseEnd
	
	rlPhaseStartTest "pki_group_cli_group_member-add-tks-018: Add two group and add a user to the two different group using valid admin user TKS_adminV"
                       rlLog "pki -d $CERTDB_DIR \
				  -n $(eval echo \$${subsystemId}_adminV_user) \
                    -c $CERTDB_DIR_PASSWORD \
                    -h $TKS_HOST \
				  ...
				  -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                                   group-add --description=\"g2description\" g2 > $TmpDir/pki-tks-group-member-add-group-add-023.out" \
                                   0 \
                                   "Adding group g2"
			...
			rlLog "pki -d $CERTDB_DIR \
				  ...
				  -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                                   group-add --description=\"g3description\" g3 > $TmpDir/pki-tks-group-member-add-group-add-023_1.out" \
                                   0 \
                                   "Adding group g3"
				  ...
				  -n $(eval echo \$${subsystemId}_adminV_user) \
				   ...
				   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                      user-add --fullName=\"fullNameu10\" u10 > $TmpDir/pki-tks-group-member-add-user-add-023.out" \
                                   0 \
                                   "Adding user u10"
				   ...
				   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                                    group-member-add g2 u10 > $TmpDir/pki-tks-group-member-add-groupadd-023.out" \
                                    0 \
				   ...
				   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                                    group-member-find g2 > $TmpDir/pki-tks-group-member-add-groupadd-find-023.out" \
                                    0 \
			...
			rlLog "Adding the user u10 to group g3"
				   ...
				   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                                    group-member-add g3 u10 > $TmpDir/pki-tks-group-member-add-groupadd-023_1.out" \
                                    0 \
				   ...
				   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                                    group-member-find g3 > $TmpDir/pki-tks-group-member-add-groupadd-find-023_1.out" \
                                    0 \
                                    "User added to group g3"
                        rlAssertGrep "User: u10" "$TmpDir/pki-tks-group-member-add-groupadd-find-023_1.out"
        rlPhaseEnd

	rlPhaseStartTest "pki_group_cli_group_member-add-tks-019: Add a group, add a user to the group and delete the group using valid admin user TKS_adminV"
                       rlLog "pki -d $CERTDB_DIR \
				  -n $(eval echo \$${subsystemId}_adminV_user) \
                    -c $CERTDB_DIR_PASSWORD \
                    -h $TKS_HOST \
				  ...
				  -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                                   group-add --description=\"g4description\" gr4 > $TmpDir/pki-tks-group-member-add-group-add-024.out" \
                                   0 \
                                   "Adding group gr4"
				  ...
				  -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                                   user-add --fullName=\"fullNameu11\" u11 > $TmpDir/pki-tks-group-member-add-user-add-024.out" \
                                   0 \
                                   "Adding user u11"
				   ...
				   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                                    group-member-add gr4 u11 > $TmpDir/pki-tks-group-member-add-groupadd-024.out" \
                                    0 \
				   ...
				   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                                    group-member-find gr4 > $TmpDir/pki-tks-group-member-add-groupadd-find-024.out" \
                                    0 \
			...
			rlLog "pki -d $CERTDB_DIR \
				   ...
				   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                                    group-del gr4 > $TmpDir/pki-tks-group-member-add-groupdel-024.out" \
                                    0 \
                                    "Deleting group gr4"
			...
			rlLog "pki -d $CERTDB_DIR \
				   ...
				   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                                    user-membership-find u11 > $TmpDir/pki-tks-group-member-add-usermembership-024.out" \
                                    0 \
                                    "Checking for user membership of u11"
			rlAssertGrep "0 entries matched" "$TmpDir/pki-tks-group-member-add-usermembership-024.out"
        rlPhaseEnd
	
	rlPhaseStartTest "pki_group_cli_group_member-add-tks-020: Add a group, add a user to the group and modify the group using valid admin user TKS_adminV"
                       rlLog "pki -d $CERTDB_DIR \
				  -n $(eval echo \$${subsystemId}_adminV_user) \
                    -c $CERTDB_DIR_PASSWORD \
                    -h $TKS_HOST \
				  ...
				  -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                                   group-add --description=\"g5description\" g4 > $TmpDir/pki-tks-group-member-add-group-add-025.out" \
                                   0 \
                                   "Adding group g4"
				  ...
				  -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                                   user-add --fullName=\"fullNameu12\" u12 > $TmpDir/pki-tks-group-member-add-user-add-025.out" \
                                   0 \
                                   "Adding user u12"
				   ...
				   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                                    group-member-add g4 u12 > $TmpDir/pki-tks-group-member-add-groupadd-025.out" \
                                    0 \
				   ...
				   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                                    group-member-find g4 > $TmpDir/pki-tks-group-member-add-groupadd-find-025.out" \
                                    0 \
			...
			rlRun "pki -d $CERTDB_DIR \
				   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                                    group-mod g4 --description=\"Modified group\" > $TmpDir/pki-tks-group-member-add-groupmod-025.out" \
                                    0 \
                                    "Modifying group g4"
	...
	rlPhaseStartTest "pki_group_cli_group_member-add-tks-021: Add a group, add a user to the group, run user-membership-del on the user and run group-member-find using valid admin user TKS_adminV"
                       rlLog "pki -d $CERTDB_DIR \
				  -n $(eval echo \$${subsystemId}_adminV_user) \
                    -c $CERTDB_DIR_PASSWORD \
                    -h $TKS_HOST \
				  ...
				  -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                                   group-add --description=\"g6description\" g5 > $TmpDir/pki-tks-group-member-add-group-add-026.out" \
                                   0 \
                                   "Adding group g5"
				  ...
				  -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                                   user-add --fullName=\"fullNameu13\" u13 > $TmpDir/pki-tks-group-member-add-user-add-026.out" \
                                   0 \
                                   "Adding user u13"
				   ...
				   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                              group-member-add g5 u13 > $TmpDir/pki-tks-group-member-add-groupadd-026.out 2>&1" \
                                    0 \
				   ...
				   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                                group-member-find g5 > $TmpDir/pki-tks-group-member-add-groupadd-find-026.out" \
                                    0 \
                                    "User added to group g5"
			...
			rlRun "pki -d $CERTDB_DIR \
				   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                              user-membership-del u13 g5 > $TmpDir/pki-tks-group-member-add-user-membership-del-026.out" \
				    0 \
			...
			rlRun "pki -d $CERTDB_DIR \
				   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                              group-member-find g5 > $TmpDir/pki-tks-group-member-add-group-member-find-026.out" \
                                    0 \
                                    "Find member in group g5"
			rlAssertGrep "0 entries matched" "$TmpDir/pki-tks-group-member-add-group-member-find-026.out"
	rlPhaseEnd
	rlPhaseStartTest "pki_group_cli_group_member-add-cleanup-tks-001: Deleting the temp directory and users and groups"
		...
		while [ $i -lt 7 ] ; do
		       rlRun "pki -d $CERTDB_DIR \
				  -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
			user-del  u$i > $TmpDir/pki-user-del-tks-group-member-add-user-del-tks-00$i.out" \
				   0 \
				   "Deleting user u$i"
		...
		while [ $i -lt 6 ] ; do
				  ...
				  -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                             group-del  g$i > $TmpDir/pki-user-del-tks-group-member-add-group-del-tks-00$i.out" \
                                   0 \
                                   "Deleting group g$i"
	   	...
	   	rlRun "pki -d $CERTDB_DIR \
			   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                            user-del userall > $TmpDir/pki-group-del-tks-group-member-add-user-del-tks-userall-001.out" \
                            0 \
		...
		rlRun "pki -d $CERTDB_DIR \
			   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                            user-del user1 >  $TmpDir/pki-user-del-tks-group-member-add-user-del-tks-user1-001.out" \
                            0 \
                            "Deleting user user1"
		...
		rlRun "pki -d $CERTDB_DIR \
			   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                            user-del us19 >  $TmpDir/pki-user-del-tks-group-member-add-user-del-tks-u13-001.out" \
                            0 \
                            "Deleting user us19"
		...
		#===Deleting users created using TKS_adminV cert===#
	        ...
	        while [ $i -lt 2 ] ; do
        		rlRun "pki -d $CERTDB_DIR \
                   ...
                   -p $TKS_PORT \
                    -t tks \
          	                    user-del  testuser$i > $TmpDir/pki-group-member-add-tks-user-00$i.out" \
                   	            0 \
		...
		rlRun "pki -d $CERTDB_DIR \
			-n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
	                group-del 'dadministʁasjɔ̃' > $TmpDir/pki-group-del-tks-group-i18n_1.out" \
        	        0 \
	...
	rlPhaseEnd
        ...
        rlPhaseStartCleanup "pki group-member-add-tks cleanup: Delete temp dir"
        rlRun "popd"
        rlRun "rm -r $TmpDir" 0 "Removing tmp directory"
