...
#  pki-group-cli-group-mod-tks    Modify existing groups in the pki tks subsystem.
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#
...
######################################################################################
#create-role-users.sh should be first executed prior to pki-group-cli-group-mod-tks.sh
######################################################################################
...
########################################################################
run_pki-group-cli-group-mod-tks_tests(){
#####Create temporary dir to save the output files #####
    rlPhaseStartSetup "pki_group_cli_group_mod_tks-startup: Create temporary directory"
...
local TEMP_NSS_DB_PASSWD="redhat123"
    ...
    rlPhaseStartTest "pki_group_cli_group_mod_tks-002: Modify a group's description in TKS using TKS_adminV"
	rlRun "pki -d $CERTDB_DIR \
        ...
        rlRun "pki -d $CERTDB_DIR \
		   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                    group-mod --description=\"$group1_mod_description\" $group1 > $TmpDir/pki-tks-group-mod-002.out" \
		    0 \
...
rlPhaseStartTest "pki_group_cli_group_mod_tks-003:--description with characters and numbers"
	rlRun "pki -d $CERTDB_DIR \
		   -n $(eval echo \$${subsystemId}_adminV_user) \
                    -c $CERTDB_DIR_PASSWORD \
                    -h $TKS_HOST \
        ...
        rlRun "pki -d $CERTDB_DIR \
		   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                    group-mod --description=abcdefghijklmnopqrstuvwxyx12345678 g1 > $TmpDir/pki-tks-group-mod-004.out" \
                    0 \
	...
	rlPhaseStartTest "pki_group_cli_group_mod_tks-004:--description with maximum length and symbols "
	randsym_b64=$(openssl rand -base64 1024 |  perl -p -e 's/\n//')
        ...
        rlRun "pki -d $CERTDB_DIR \
		   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                    group-mod --description=\"$randsym\" g2 > $TmpDir/pki-tks-group-mod-005.out" \
                    0 \
    ...
    rlPhaseStartTest "pki_group_cli_group_mod_tks-005:--description with $ character "
	rlRun "pki -d $CERTDB_DIR \
		   -n $(eval echo \$${subsystemId}_adminV_user) \
                    -c $CERTDB_DIR_PASSWORD \
                    -h $TKS_HOST \
        ...
        rlRun "pki -d $CERTDB_DIR \
		    -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                    group-mod --description=$ g3 > $TmpDir/pki-tks-group-mod-008.out" \
                    0 \
                    "Modified group using TKS_adminV with --description $ character"
 ...
 rlPhaseStartTest "pki_group_cli_group_mod_tks-006: Modify a group to TKS with -t option"
	rlRun "pki -d $CERTDB_DIR \
		   -n $(eval echo \$${subsystemId}_adminV_user) \
                    -c $CERTDB_DIR_PASSWORD \
                    -h $TKS_HOST \
        ...
        rlRun "pki -d $CERTDB_DIR \
		   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                   -t tks \
                    group-mod --description=\"$group1desc\" g4 > $TmpDir/pki-tks-group-mod-007.out" \
                    0 \
                    "Modified group g4 to TKS"
	...
	rlLog "PKI TICKET: https://fedorahosted.org/pki/ticket/818"
    rlPhaseEnd
    rlPhaseStartTest "pki_group_cli_group_mod_tks-007:  Modify a group -- missing required option group id"
	command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_adminV_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT -t tks group-mod --description='$group1desc'"
...
##### Tests to modify groups using revoked cert#####
    rlPhaseStartTest "pki_group_cli_group_mod_tks-008: Should not be able to modify groups using a revoked cert TKS_adminR"
	command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_adminR_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT -t tks group-mod --description='$group1_mod_description' $group1"
    ...
    rlPhaseEnd
    rlPhaseStartTest "pki_group_cli_group_mod_tks-009: Should not be able to modify group using an agent or a revoked cert TKS_agentR"
	command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_agentR_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT -t tks group-mod --description='$group1desc' $group1"
	errmsg="PKIException: Unauthorized"
        errorcode=255
        rlRun "verifyErrorMsg \"$command\" \"$errmsg\" \"$errorcode\"" 0 "Verify expected error message - Cannot modify group $group1 using a user having revoked cert"
	rlLog "PKI Ticket: https://fedorahosted.org/pki/ticket/1134"
...
##### Tests to modify groups using an agent user#####
    rlPhaseStartTest "pki_group_cli_group_mod_tks-010: Should not be able to modify groups using a TKS_agentV user"
	command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_agentV_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT -t tks group-mod --description='$group1desc' $group1"
...
##### Tests to modify groups using expired cert#####
    rlPhaseStartTest "pki_group_cli_group_mod_tks-011: Should not be able to modify group using a TKS_adminE cert"
        rlRun "date --set='next day'" 0 "Set System date a day ahead"
                                rlRun "date --set='next day'" 0 "Set System date a day ahead"
                                rlRun "date"
	command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_adminE_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT -t tks group-mod --description='$group1desc' $group1"
	errmsg="ForbiddenException: Authorization Error"
        errorcode=255
    ...
    rlPhaseStartTest "pki_group_cli_group_mod_tks-012: Should not be able to modify group using a TKS_agentE cert"
        rlRun "date --set='next day'" 0 "Set System date a day ahead"
                                rlRun "date --set='next day'" 0 "Set System date a day ahead"
                                rlRun "date"
	command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_agentE_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT -t tks group-mod --description='$group1desc' $group1"
	errmsg="ForbiddenException: Authorization Error"
        errorcode=255
 ...
 ##### Tests to modify groups using audit users#####
    rlPhaseStartTest "pki_group_cli_group_mod_tks-013: Should not be able to modify group using a TKS_auditV"
	command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_auditV_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT -t tks group-mod --description='$group1desc' $group1"
	errmsg="ForbiddenException: Authorization Error"
	errorcode=255
    ...
    rlPhaseStartTest "pki_group_cli_group_mod_tks-014: Should not be able to modify group using a TKS_operatorV"
	command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_operatorV_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT -t tks group-mod --description='$group1desc' $group1"
	errmsg="ForbiddenException: Authorization Error"
        errorcode=255
...
##### Tests to modify groups using TKS_adminUTCA and TKS_agentUTCA  user's certificate will be issued by an untrusted CA users#####
    rlPhaseStartTest "pki_group_cli_group_mod_tks-015: Should not be able to modify groups using a cert created from a untrusted CA TKS_adminUTCA"
	command="pki -d $UNTRUSTED_CERT_DB_LOCATION -n role_user_UTCA -c $UNTRUSTED_CERT_DB_PASSWORD  -h $TKS_HOST -p $TKS_PORT -t tks group-mod --description='$group1desc' $group1"
	errmsg="PKIException: Unauthorized"
        errorcode=255
        rlRun "verifyErrorMsg \"$command\" \"$errmsg\" \"$errorcode\"" 0 "Verify expected error message - Cannot modify group $group1 as adminUTCA"
    rlPhaseEnd

rlPhaseStartTest "pki_group_cli_group_mod_tks-016:  Modify a group -- Group ID does not exist"
        command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_adminV_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT -t tks group-mod --description='$group1desc' g5"
...
##### Tests to modify TKS groups with empty parameters ####
    ...
    rlPhaseStartTest "pki_group_cli_group_mod_tks-017: Modify a user created group in TKS using TKS_adminV - description is empty"
	rlRun "pki -d $CERTDB_DIR \
		   -n $(eval echo \$${subsystemId}_adminV_user) \
                    -c $CERTDB_DIR_PASSWORD \
                    -h $TKS_HOST \
	...
	rlLog "pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_adminV_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT -t tks group-mod --description=\"\" g5"
	rlRun "pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_adminV_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT -t tks group-mod --description=\"\" g5 > $TmpDir/pki-tks-group-mod-0017.out" 0 "Group modified successfully with empty description"
...
##### Tests to modify TKS groups with the same value ####
    ...
    rlPhaseStartTest "pki_group_cli_group_mod_tks-018: Modify a group in TKS using TKS_adminV - description same old value"
	rlRun "pki -d $CERTDB_DIR \
		   -n $(eval echo \$${subsystemId}_adminV_user) \
                    -c $CERTDB_DIR_PASSWORD \
                    -h $TKS_HOST \
                    -p $TKS_PORT \
                    -t tks \
                    group-show $group1 > $TmpDir/pki-tks-group-mod-041_1.out"
	...
	rlAssertGrep "Group ID: $group1" "$TmpDir/pki-tks-group-mod-041_1.out"
        ...
        rlRun "pki -d $CERTDB_DIR \
		   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                    group-mod --description=\"$group1_mod_description\" $group1 > $TmpDir/pki-tks-group-mod-041_2.out" \
                    0 \
...
rlPhaseStartTest "pki_group_cli_group_mod_tks-019: Modify a groups's description having i18n chars in TKS using TKS_adminV"
        rlRun "pki -d $CERTDB_DIR \
		    -n $(eval echo \$${subsystemId}_adminV_user) \
                    -c $CERTDB_DIR_PASSWORD \
                    -h $TKS_HOST \
        ...
        rlRun "pki -d $CERTDB_DIR \
		   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                    group-mod --description=\"$i18ngroup_mod_description\" $i18ngroup > $TmpDir/pki-tks-group-mod-043.out" \
                   0 \
...
##### Tests to modify system generated TKS groups ####
    rlPhaseStartTest "pki_group_cli_group_mod_tks-021: Modify Administrator group's description in TKS using TKS_adminV"
	rlRun "pki -d $CERTDB_DIR \
		    -n $(eval echo \$${subsystemId}_adminV_user) \
                    -c $CERTDB_DIR_PASSWORD \
                    -h $TKS_HOST \
                    -p $TKS_PORT \
                    -t tks \
                    group-show Administrators > $TmpDir/pki-tks-group-mod-group-show-022.out"
	admin_group_desc=$(cat $TmpDir/pki-tks-group-mod-group-show-022.out| grep Description | cut -d- -f2)
        ...
        rlRun "pki -d $CERTDB_DIR \
		    -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                    group-mod --description=\"$group1_mod_description\" Administrators > $TmpDir/pki-tks-group-mod-022.out" \
                    0 \
	...
	rlPhaseStartTest "pki_group_cli_group_mod_tks-022: Modify Administrators group in TKS using TKS_adminV - description is empty"
	rlRun "pki -d $CERTDB_DIR \
                    -n $(eval echo \$${subsystemId}_adminV_user) \
                    -c $CERTDB_DIR_PASSWORD \
                    -h $TKS_HOST \
                    -p $TKS_PORT \
                    -t tks \
                    group-show Administrators > $TmpDir/pki-tks-group-mod-group-show-023.out"
	admin_group_desc=$(cat $TmpDir/pki-tks-group-mod-group-show-023.out| grep Description | cut -d- -f2)
	rlLog "pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_adminV_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT -t tks group-mod --description=\"\" Administrators"
        rlRun "pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_adminV_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT -t tks group-mod --description=\"\" Administrators > $TmpDir/pki-tks-group-mod-023.out" 0 "Successfully modified Administrator group description"
...
#===Deleting groups===#
rlPhaseStartTest "pki_group_cli_group_cleanup_tks: Deleting role groups"
	...
	rlRun "pki -d $CERTDB_DIR \
			  -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -p $TKS_PORT \
                    -t tks \
                           group-del $i18ngroup > $TmpDir/pki-group-del-tks-i18ngroup-001.out" \
                           0 \
    ...
    rlPhaseEnd
        ...
        rlPhaseStartCleanup "pki group-mod-tks cleanup: Delete temp dir"
        rlRun "popd"
        rlRun "rm -r $TmpDir" 0 "Removing tmp directory"
