...
run_pki-kra-key-template-show-kra_tests()
{
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_kra_key_template_show-003: verify template files are saved and the json file is valid"
        for i in "${keytemplates[@]}"; do
        rlRun "pki kra-key-template-show $i --output $i-json"  0 "Save $i template in $i-json"
        done
        rlPhaseEnd
