...
run_pki-kra-key-request-show-kra_tests()
{
        local cs_Type=$1
        local cs_Role=$2

        # Creating Temporary Directory for pki kra-key-request-show
        rlPhaseStartSetup "pki kra-key-request-show Temporary Directory"
        ...
        local key_request_find_output=$TmpDir/kra-key-request-show.out
        local key_generate_output=$TmpDir/key-generate.out
        local key_archive_output=$TmpDir/key-archive.out
        local key_request_show_output=$TmpDir/kra-key-request-show.out
        local tmp_request_review_out=$TmpDir/key-request-review.out
        local rand=$RANDOM
        local tmp_junk_data=$(openssl rand -base64 50 |  perl -p -e 's/\n//')

	# Config test of pki kra-key-request-show 
	rlPhaseStartTest "pki_key_request_show_cli-configtest: pki kra-key-request-show --help configuration test"
	rlRun "pki kra-key-request-show --help > $key_request_show_output" 0 "pki kra-key-request-show --help"
	rlAssertGrep "usage: kra-key-request-show <Request ID> \[OPTIONS...\]" "$key_request_show_output"
	rlAssertGrep "    --help   Show help options" "$key_request_show_output"
	rlPhaseEnd

	rlPhaseStartTest "pki_key_request_show-001: pki kra-key-request-show < valid Request ID(hexadecimal) > should show key details"
        local rand=$RANDOM
	...
	local tmp_request_id=$(cat $key_generate_output | grep "Request ID" | awk -F ": " '{print $2}')
        rlRun "pki -d $CERTDB_DIR \
                ...
                -n \"$valid_agent_cert\" \
                kra-key-request-show $tmp_request_id > $key_request_show_output" 0 "pki kra-key-request-show $tmp_request_id"
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_key_request_show-002: pki kra-key-request-show < valid Request ID(decimal) > should show key details"
        local rand=$RANDOM
        local client_id=temp$rand
        local algo=AES
        local action=NULL
	...
	decimal_valid_keyId=$(echo "ibase=16;$CONV_UPP_VAL_KEY_ID"|bc)
        rlRun "pki -d $CERTDB_DIR \
                ...
                -p $target_unsecure_port \
                -n \"$valid_agent_cert\" \
                kra-key-request-show $decimal_valid_keyId > $key_request_show_output" 0 "pki kra-key-request-show $tmp_request_id"
        rlAssertGrep "Request ID: $tmp_request_id" "$key_request_show_output"
        rlAssertGrep "Key ID: $tmp_key_id" "$key_request_show_output"
        rlAssertGrep "Type: symkeyGenRequest" "$key_request_show_output"
        rlAssertGrep "Status: complete" "$key_request_show_output"
	...
	rlPhaseStartTest "pki_key_request_show-003: pki kra-key-request-show < In-valid Request ID(hexadecimal) > should show key details"
	tmp_request_id=0xfffffff
        rlRun "pki -d $CERTDB_DIR \
                ...
                -p $target_unsecure_port \
                -n \"$valid_agent_cert\" \
                kra-key-request-show $tmp_request_id > $key_request_show_output 2>&1" 255,1 "pki kra-key-request-show $tmp_request_id"
	rlAssertGrep "RequestNotFoundException: Request ID $tmp_request_id not found" "$key_request_show_output"
	rlPhaseEnd


        rlPhaseStartTest "pki_key_request_show-004: pki kra-key-request-show < In-valid Request ID(decimal) > should show key details"
	...
	local conv_lower_hex_invalidserialNum=${invalid_hex_sno,,}
        ...
        rlPhaseStartTest "pki_key_request_show-005: Archive a passphrase and issue pki kra-key-request-show against the Request ID to verify the output is correct"
        local rand=$RANDOM
        local client_id=temp$rand
	...
	rlAssertGrep "Key ID: $tmp_key_id" "$key_request_show_output"
        ...
        rlPhaseStartTest "pki_key_request_show-006: Executing pki kra-key-request-show <Request ID> using valid admin cert should fail"
        local rand=$RANDOM
        local client_id=temp$rand
        local algo=AES
        local action=NULL
        ...
        rlRun "generate_key $client_id $algo $key_size $usages $action $tmp_kra_host $target_unsecure_port $valid_agent_cert $key_generate_output" \
                0 "Generate Symmetric key with client $client_id, algo $algo, key_size $key_size, usages $usages"
        local tmp_request_id=$(cat $key_generate_output | grep "Request ID" | awk -F ": " '{print $2}')
        rlLog "Executing pki kra-key-request-show <Request ID> as $valid_admin_cert"
        rlRun "pki -d $CERTDB_DIR \
                -c $CERTDB_DIR_PASSWORD \
                -h $tmp_kra_host \
                -p $target_unsecure_port \
                -n \"$valid_admin_cert\" \
                kra-key-request-show $tmp_request_id  > $key_request_show_output 2>&1" 255,1 "Execute pki kra-key-request-show $tmp_request_ida as $valid_admin_cert"
        rlAssertGrep "ForbiddenException: Authorization Error" "$key_request_show_output"
        rlPhaseEnd

        rlPhaseStartTest "pki_key_request_show-007: Executing pki kra-key-request-show <Request ID> using revoked Agent cert should fail"
        rlLog "Executing pki kra-key-request-show as $revoked Agent Cert"
        rlRun "pki -d $CERTDB_DIR \
                -c $CERTDB_DIR_PASSWORD \
                -h $tmp_kra_host \
                -p $target_unsecure_port \
                -n \"$revoked_agent_cert\" \
                kra-key-request-show $tmp_request_id > $key_request_show_output 2>&1" 255,1 "Execute pki kra-key-request-show $tmp_request_id as $revoked_agent_cert"
        rlAssertGrep "ForbiddenException: Authorization Error" "$key_request_show_output"
        rlPhaseEnd

        rlPhaseStartTest "pki_key_request_show-008: Executing pki kra-key-request-show <key ID> using Expired admin(not a member of agents Group)cert should fail"
        rlLog "Executing pki kra-key-request-show $tmp_request_id as $expired_admin_cert"
        local cur_date=$(date)
        ...
        rlRun "pki -d $CERTDB_DIR \
                ...
                kra-key-request-show $tmp_request_id > $key_request_show_output 2>&1" 255,1 "Execute pki kra-key-request-show $tmp_request_id as $expired_admin_cert"
        ...
        rlPhaseStartTest "pki_key_request_show-009: Executing pki kra-key-request-show <key ID> using Expired agent cert should fail"
        rlLog "Executing pki kra-key-request-show as $expired_agent_cert"
        local cur_date=$(date)
        ...
        rlRun "pki -d $CERTDB_DIR \
                ...
                kra-key-request-show $tmp_request_id > $key_request_show_output 2>&1" 255,1 "Execute pki kra-key-request-show $tmp_request_id as $expired_agent_cert"
        rlAssertGrep "ProcessingException: Unable to invoke request" "$key_request_show_output"
        rlLog "Set the date back to its original date & time"
        rlRun "chronyc -a -m 'settime $cur_date + 10 seconds' 'makestep' 'manual reset' 'online' 1> $TmpDir/chrony.out"
        rlAssertGrep "200 OK" "$TmpDir/chrony.out"
        ...
        rlPhaseStartTest "pki_key_request_show-0010: Executing pki kra-key-request-show <Request ID> using valid Audit cert should fail"
        rlLog "Executing pki kra-key-request-show $tmp_request_id as $valid_audit_cert"
        rlRun "pki -d $CERTDB_DIR \
                -c $CERTDB_DIR_PASSWORD \
                -h $tmp_kra_host \
                -p $target_unsecure_port \
                -n \"$valid_audit_cert\" \
                kra-key-request-show $tmp_request_id > $key_request_show_output 2>&1" 255,1 "Execute pki kra-key-request-show $tmp_request_id as $valid_audit_cert"
        rlAssertGrep "ForbiddenException: Authorization Error" "$key_request_show_output"
        rlPhaseEnd

        rlPhaseStartTest "pki_key_request_show-0011: Executing pki kra-key-request-show <Request ID> using valid Operator cert should fail"
        rlLog "Executing pki kra-key-request-show <Request ID> as $valid_operator_cert"
        rlRun "pki -d $CERTDB_DIR \
                -c $CERTDB_DIR_PASSWORD \
                -h $tmp_kra_host \
                -p $target_unsecure_port \
                -n \"$valid_operator_cert\" \
                kra-key-request-show $tmp_request_id > $key_request_show_output 2>&1" 255,1 "Execute pki kra-key-request-show $tmp_request_id as $valid_operator_cert"
        rlAssertGrep "ForbiddenException: Authorization Error" "$key_request_show_output"
        rlPhaseEnd

        rlPhaseStartTest "pki_key_request_show-0012: Executing pki kra-key-request-show <Request ID> using normal user cert(without any privileges) should fail"
        local rand=$RANDOM
        ...
        rlLog "Executing pki kra-key-request-show as $pki_user_fullName"
        rlRun "pki -d $TEMP_NSS_DB\
                -c $TEMP_NSS_DB_PWD \
                -h $tmp_kra_host \
                -p $target_unsecure_port \
                -n \"$pki_user\" \
                kra-key-request-show $tmp_request_id > $key_request_show_output 2>&1" 255,1 "Executing pki kra-key-request-show $tmp_request_id as $pki_user_fullName"
        rlAssertGrep "ForbiddenException: Authorization Error" "$key_request_show_output"
        rlPhaseEnd

        rlPhaseStartTest "pki_key_request_show-0013: Executing pki kra-key-request-show using host URI parameter(https) should succeed"
        rlLog "Executing pki kra-key-request-show using http host URI parameter(https)"
        ...
        rlRun "pki -d $CERTDB_DIR \
                ...
                -c $CERTDB_DIR_PASSWORD \
                -n $valid_agent_cert \
                kra-key-request-show $tmp_request_id  > $key_request_show_output" 0 "Executing pki kra-key-request-show as $valid_agent_cert"
        rlAssertGrep "Request ID: $tmp_request_id" "$key_request_show_output"
        ...
        rlPhaseStartTest "pki_key_request_show-0014: Executing pki kra-key-request-show <Request ID> using valid user(Not a member of any group) should fail"
        rlRun "pki -d $CERTDB_DIR\
                -h $tmp_kra_host \
                -p $target_unsecure_port \
                -u $pki_user \
        ...
        rlPhaseStartTest "pki_key_request_show-0015: Executing pki kra-key-request-show <Request ID> as in-valid user should fail"
        local invalid_pki_user=test1
        local invalid_pki_user_pwd=Secret123
        rlLog "Executing pki kra-key-request-show using user $pki_user"
        rlRun "pki -d $CERTDB_DIR \
                -h $tmp_kra_host -p $target_unsecure_port \
                -u $invalid_pki_user \
                -w $invalid_pki_user_pwd  \
                kra-key-request-show $tmp_request_id > $key_request_show_output 2>&1" 255,1 "Search key requests as $invalid_pki_user"
        rlAssertGrep "PKIException: Unauthorized" "$key_request_show_output"
        rlPhaseEnd

        rlPhaseStartCleanup "pki kra-key-request-show cleanup: Delete temp dir"
        rlRun "popd"
