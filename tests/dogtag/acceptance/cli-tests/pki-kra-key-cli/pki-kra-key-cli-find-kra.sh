...
. /opt/rhqa_pki/env.sh
run_pki-kra-key-find-kra_tests()
{
        local cs_Type=$1
        local cs_Role=$2
	
	# Creating Temporary Directory for pki kra-key-find
	...
	local key_find_output=$TmpDir/kra-key-find.out
	...
	# Config test of pki kra-key-find
	rlPhaseStartTest "pki_key_cli-configtest: pki kra-key-find --help configuration test"
	rlRun "pki kra-key-find --help > $key_find_output" 0 "pki kra-key-find --help"
	rlAssertGrep "usage: kra-key-find \[OPTIONS...\]" "$key_find_output"
	...
	rlPhaseStartTest "pki_kra_key_find-001: Search keys with Valid clientId using Agent Cert"
	local rand=$(cat /dev/urandom | tr -dc '0-9' | fold -w 5 | head -n 1)
	...
	rlRun "pki -d $CERTDB_DIR \
		...
		-h $tmp_kra_host \
                ...
                -n \"$valid_agent_cert\" \
                kra-key-find --clientKeyID $client_id > $key_find_output" 0 "Search key with client Id temp$rand using $valid_agent_cert"
	...
	rlPhaseStartTest "pki_kra_key_find-002: Searching keys with Non Existent ClientID should give no results"
	local client_id=invalidtemp$rand
        rlRun "pki -d $CERTDB_DIR \
                ...
                -p $target_unsecure_port \
                -n \"$valid_agent_cert\" \
                kra-key-find --clientKeyID $client_id > $key_find_output" 0 "Search key with client Id $client_id using $valid_agent_cert"
	rlAssertGrep "Number of entries returned 0" "$key_find_output"
	rlPhaseEnd

        rlPhaseStartTest "pki_kra_key_find-003: Searching keys with junkdata passed to ClientID should give no results"
        ...
        rlRun "pki -d $CERTDB_DIR \
                ...
                -h $tmp_kra_host \
                -n \"$valid_agent_cert\" \
                kra-key-find --clientKeyID $client_id > $key_find_output" 0 "Search key with client Id invalidtemp$rand using $valid_agent_cert"
        rlAssertGrep "Number of entries returned 0" "$key_find_output"
        rlPhaseEnd

        rlPhaseStartTest "pki_kra_key_find-004: Searching keys when no data passed to ClientID should fail with command help"
        local client_id=' '
        rlRun "pki -d $CERTDB_DIR \
                -c $CERTDB_DIR_PASSWORD \
                -p $target_unsecure_port \
                -h $tmp_kra_host \
                -n \"$valid_agent_cert\" \
                kra-key-find --clientKeyID $client_id > $key_find_output 2>&1" 255,1 "Search key with client Id invalidtemp$rand using $valid_agent_cert"
	...
	rlPhaseStartTest "pki_kra_key_find-005: Search keys with --maxResults 5 and verify 5 results are returned"
        local rand=$(cat /dev/urandom | tr -dc '0-9' | fold -w 5 | head -n 1)
        local client_id=temp$rand
	local maxResults=5
        local algo=AES
	...
	local tmp_key_id=$(cat $key_generate_output | grep "Key ID" | awk -F ": " '{print $2}')
        rlRun "pki -d $CERTDB_DIR \
                ...
                -h $tmp_kra_host \
                -n \"$valid_agent_cert\" \
                kra-key-find --maxResults $maxResults  > $key_find_output" 0 "Search keys with --maxResults 5"
	rlAssertGrep "Number of entries returned 5" "$key_find_output"
	rlPhaseEnd

        rlPhaseStartTest "pki_kra_key_find-006: Search keys with junkvalue passed --maxResults and verify no results are returned"
        ...
        rlRun "pki -d $CERTDB_DIR \
                ...
                -p $target_unsecure_port \
                -n \"$valid_agent_cert\" \
                 kra-key-find --maxResults $maxResults  > $key_find_output 2>&1" 255, 1 "Search keys with junk data passed to --maxResults"
        rlAssertGrep "NumberFormatException: For input string: \"$maxResults\"" "$key_find_output"
        rlPhaseEnd

        rlPhaseStartTest "pki_kra_key_find-007: Search keys with Negative value passed --maxResults and verify no results are returned"
        ...
        rlRun "pki -d $CERTDB_DIR \
                ...
                -p $target_unsecure_port \
                -n \"$valid_agent_cert\" \
                kra-key-find --maxResults $maxResults  > $key_find_output 2>&1" 255,1 "Search keys with negative value -128 passed to maxResults"
        rlAssertGrep "NumberFormatException: For input string: \"$maxResults\" " "$key_find_output"
	rlLog "PKI TICKET:: https://fedorahosted.org/pki/ticket/1121"
        rlPhaseEnd

        rlPhaseStartTest "pki_kra_key_find-008: Search keys with Maximum integer value(10 digits) passed --maxResults and verify no results are returned"
        ...
        rlRun "pki -d $CERTDB_DIR \
                ...
                -p $target_unsecure_port \
                -n \"$valid_agent_cert\" \
                kra-key-find --maxResults $maxResults  > $key_find_output 2>&1" 255,1 "Search keys with 10 digit integer passed to --maxResults"
        rlAssertGrep "NumberFormatException: For input string: \"$maxResults\"" "$key_find_output"
        rlPhaseEnd

	rlPhaseStartTest "pki_kra_key_find-009: Search keys with no value passed to --maxResults and verify command help is returned"
        local maxResults=' '
        rlRun "pki -d $CERTDB_DIR \
                -c $CERTDB_DIR_PASSWORD \
                -h $tmp_kra_host \
                -p $target_unsecure_port \
                -n \"$valid_agent_cert\" \
                kra-key-find --maxResults $maxResults  > $key_find_output 2>&1" 255,1
        ...
        rlPhaseStartTest "pki_kra_key_find-0010: Search keys with --size 5 and verify 5 results are returned"
        local size=5
        rlRun "pki -d $CERTDB_DIR \
                -c $CERTDB_DIR_PASSWORD \
                -h $tmp_kra_host \
                -p $target_unsecure_port \
                -n \"$valid_agent_cert\" \
                kra-key-find --size $size  > $key_find_output" 0 "Search keys with --size $size"
        rlAssertGrep "Number of entries returned 5" "$key_find_output"
        rlPhaseEnd

        rlPhaseStartTest "pki_kra_key_find-0011: Search keys with junkvalue passed --size and verify no results are returned"
        ...
        rlRun "pki -d $CERTDB_DIR \
                ...
                -p $target_unsecure_port \
                -n \"$valid_agent_cert\" \
                 kra-key-find --size $size  > $key_find_output 2>&1" 255, 1 "Search keys with junk data passed to --size"
        rlAssertGrep "NumberFormatException: For input string: \"$size\"" "$key_find_output"
        rlPhaseEnd

        rlPhaseStartTest "pki_kra_key_find-0012: Search keys with Negative value passed --size and verify no results are returned"
        local size=-128
        rlRun "pki -d $CERTDB_DIR \
                -c $CERTDB_DIR_PASSWORD \
                -h $tmp_kra_host \
                -p $target_unsecure_port \
                -n \"$valid_agent_cert\" \
                kra-key-find --size $size  > $key_find_output 2>&1" 255,1,0
        rlAssertGrep "NumberFormatException: For input string: \"$size\" " "$key_find_output"
        rlPhaseEnd

        rlPhaseStartTest "pki_kra_key_find-0013: Search keys with Maximum integer value(10 digits) passed --size and verify no results are returned"
        ...
        rlRun "pki -d $CERTDB_DIR \
                ...
                -p $target_unsecure_port \
                -n \"$valid_agent_cert\" \
                kra-key-find --size $size  > $key_find_output 2>&1" 255,1 "Searching keys with 10digit integer value passed to --size"
        rlAssertGrep "NumberFormatException: For input string: \"$size\"" "$key_find_output"
        rlPhaseEnd

        rlPhaseStartTest "pki_kra_key_find-0014: Search keys with no value passed to --size and verify command help is returned"
        local size=' '
        rlRun "pki -d $CERTDB_DIR \
                -c $CERTDB_DIR_PASSWORD \
                -h $tmp_kra_host \
                -p $target_unsecure_port \
                -n \"$valid_agent_cert\" \
                 kra-key-find --size $size  > $key_find_output 2>&1" 255,1 "Searching keys with no value passed to --size"
        ...
        rlPhaseStartTest "pki_kra_key_find-0015: Search keys with key status active using Agent Cert"
        local rand=$(cat /dev/urandom | tr -dc '0-9' | fold -w 5 | head -n 1)
        local client_id=temp$rand
        local algo=AES
        local action=approve
	...
	local status=active
        ...
        rlRun "pki -d $CERTDB_DIR \
                ...
                -p $target_unsecure_port \
                -n \"$valid_agent_cert\" \
                 kra-key-find --status $status --maxResults 1000 --size 1000 > $key_find_output" 0 "Search keys with $status status"
	...
	rlPhaseStartTest "pki_kra_key_find-0016: Search keys with key status inactive using Agent Cert"
        local rand=$(cat /dev/urandom | tr -dc '0-9' | fold -w 5 | head -n 1)
        local client_id=temp$rand
        local algo=AES
        local action=approve
	...
	rlRun "pki -d $CERTDB_DIR -c $CERTDB_DIR_PASSWORD \
		...
		-p $target_unsecure_port \
                -n \"$valid_agent_cert\" \
                kra-key-find --status $status --size 100 > $key_find_output" 0 "Search keys with $status status"
        local check_result=$(cat $key_find_output | grep inactive | wc -l)
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_kra_key_find-0017: Search keys with Invalid key status using Agent Cert"
        ...
        rlRun "pki -d $CERTDB_DIR \
                ...
                -p $target_unsecure_port \
                -n \"$valid_agent_cert\" \
                kra-key-find --status $status --size 100 > $key_find_output" 0 "Search keys with junk status"
	rlAssertGrep "Number of entries returned 0" "$key_find_output"
        rlPhaseEnd

        rlPhaseStartTest "pki_kra_key_find-0018: Searching keys witn no status passed should fail with command help"
        local status=' '
        rlRun "pki -d $CERTDB_DIR \
                -c $CERTDB_DIR_PASSWORD \
                -h $tmp_kra_host \
                -p $target_unsecure_port \
                -n \"$valid_agent_cert\" \
                kra-key-find --status $status  > $key_find_output 2>&1" 255,1 "Search keys with no status passed to --status"
        ...
        rlPhaseStartTest "pki_kra_key_find-0019: Search keys starting from 0x6 using --start"
        local startsize=5
        rlRun "pki -d $CERTDB_DIR \
                -c $CERTDB_DIR_PASSWORD \
                -h $tmp_kra_host \
                -p $target_unsecure_port \
                -n \"$valid_agent_cert\" \
                kra-key-find --start $startsize > $key_find_output" 0 "Search keys with --size 5"
	...
	rlPhaseStartTest "pki_kra_key_find-0020: Search keys start from 0x6 with only 10 results returned using --size"
        local startsize=5
	local size=10
        rlRun "pki -d $CERTDB_DIR \
                -c $CERTDB_DIR_PASSWORD \
                ...
                -p $target_unsecure_port \
                -n \"$valid_agent_cert\" \
                kra-key-find --start $startsize --size $size > $key_find_output" 0 "Search keys with --size 5"
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_kra_key_find-0021: Search keys with negative values passed to  --start should not return any results"
        local startsize=-128
        rlRun "pki -d $CERTDB_DIR \
                -c $CERTDB_DIR_PASSWORD \
                -h $tmp_kra_host \
                -p $target_unsecure_port \
                -n \"$valid_agent_cert\" \
                kra-key-find --start $startsize > $key_find_output" 0 "Search keys with --size $startsize"
	rlAssertGrep "Number of entries returned 0" "$key_find_output"
        rlPhaseEnd

        rlPhaseStartTest "pki_kra_key_find-0022: Search keys with no value passed to --start should fail with command help"
        local startsize=' '
        rlRun "pki -d $CERTDB_DIR \
                -c $CERTDB_DIR_PASSWORD \
                -h $tmp_kra_host \
                -p $target_unsecure_port \
                -n \"$valid_agent_cert\" \
                kra-key-find --start $startsize > $key_find_output" 255,1 "Search keys with --size $startsize"
        ...
        rlPhaseStartTest "pki_kra_key_find-0023: Generate more than 100 keys and verify --size 105 returns 105 key results"
        local rand=$(cat /dev/urandom | tr -dc '0-9' | fold -w 5 | head -n 1)
        local client_id=temp$rand
        local size=105
        local algo=AES
        ...
        rlRun "pki -d $CERTDB_DIR \
                ...
                -h $tmp_kra_host \
                -n \"$valid_agent_cert\" \
                kra-key-find --maxResults $size --size $size  > $key_find_output"
        rlAssertGrep "Number of entries returned $size" "$key_find_output"
        rlPhaseEnd

        rlPhaseStartTest "pki_kra_key_find-0024: Generate more than 100 keys and verify --maxResults 105 returns 105 key results"
        local size=105
        rlRun "pki -d $CERTDB_DIR \
                -c $CERTDB_DIR_PASSWORD \
                -p $target_unsecure_port \
                -h $tmp_kra_host \
                -n \"$valid_agent_cert\" \
                kra-key-find --size $size --maxResults $size  > $key_find_output" 0 "Search keys with --maxResults $size"
        rlAssertGrep "Number of entries returned $size" "$key_find_output"
        rlPhaseEnd

	rlPhaseStartTest "pki_kra_key_find-0025: Search keys with --maxTime <positive value> and verify search results are returned"
        local maxTime=5
        rlRun "pki -d $CERTDB_DIR \
                -c $CERTDB_DIR_PASSWORD \
                -p $target_unsecure_port \
                -h $tmp_kra_host \
                -n \"$valid_agent_cert\" \
                kra-key-find --maxTime $maxTime --size 10  > $key_find_output" 0 "Search keys with --maxTime $maxTime"
        rlAssertGrep "Number of entries returned 10" "$key_find_output"
	rlPhaseEnd

        rlPhaseStartTest "pki_kra_key_find-0026: Searching keys with junk value passed to --MaxTime should fail"
        ...
        rlRun "pki -d $CERTDB_DIR \
                ...
                -h $tmp_kra_host \
                -n \"$valid_agent_cert\" \
                kra-key-find --maxTime $maxTime --size 10 > $key_find_output 2>&1" 255,1 "Search keys with junk value passed to --maxTime"
        rlAssertGrep "NumberFormatException: For input string: \"$tmp_junk_data\"" "$key_find_output"
        rlPhaseEnd
	
	rlPhaseStartTest "pki_kra_key_find-0027: Searching keys using valid admin cert should fail"
        rlLog "Executing pki kra-key-find as $valid_admin_cert"
        rlRun "pki -d $CERTDB_DIR \
                -c $CERTDB_DIR_PASSWORD \
                -h $tmp_kra_host \
                -p $target_unsecure_port \
                -n \"$valid_admin_cert\" \
                kra-key-find  > $key_find_output 2>&1" 255,1 "Search key as $valid_admin_cert"
        ...
        rlPhaseStartTest "pki_kra_key_find-0028: Searching keys using revoked Agent cert should fail"
        rlLog "Executing pki kra-key-find as $revoked Agent Cert"
        rlRun "pki -d $CERTDB_DIR \
                -c $CERTDB_DIR_PASSWORD \
                -h $tmp_kra_host \
                -h $tmp_kra_host \
                ...
                kra-key-find  > $key_find_output 2>&1" 255,1 "Search keys as $revoked_agent_cert"
        rlAssertNotGrep "Number of entries returned 20" "$key_find_output"
        rlAssertGrep "ForbiddenException: Authorization Error" "$key_find_output"
        rlPhaseEnd

	rlPhaseStartTest "pki_kra_key_find-0029: Searching keys using Expired admin(not a member of agents Group) cert should fail"
        ...
        rlRun "pki -d $CERTDB_DIR \
                ...
                kra-key-find  > $key_find_output 2>&1" 255,1 
        rlAssertNotGrep "Number of entries returned 20" "$key_find_output"
        ...
        rlPhaseStartTest "pki_kra_key_find-0030: Searching keys using Expired agent cert should fail"
        ...
        rlRun "pki -d $CERTDB_DIR \
                ...
                kra-key-find  > $key_find_output 2>&1" 255,1 "Search keys as $expired_agent_cert"
        rlAssertNotGrep "Number of entries returned 20" "$key_find_output"
        rlAssertGrep "ProcessingException: Unable to invoke request" "$key_find_output"
        rlLog "Set the date back to its original date & time"
        rlRun "chronyc -a -m 'settime $cur_date + 10 seconds' 'makestep' 'manual reset' 'online' 1> $TmpDir/chrony.out"
        ...
        rlPhaseStartTest "pki_kra_key_find-0031: Searching keys using normal user cert(without any privileges) should fail"
        local rand=$(cat /dev/urandom | tr -dc '0-9' | fold -w 5 | head -n 1)
        ...
        rlLog "Executing pki kra-key-find as $pki_user_fullName"
        rlRun "pki -d $TEMP_NSS_DB\
                -c $TEMP_NSS_DB_PWD \
                -h $tmp_kra_host \
                -p $target_unsecure_port \
                -n \"$pki_user\" \
                kra-key-find  > $key_find_output 2>&1" 255,1 "Search keys as $pki_user_fullName"
        rlAssertNotGrep "Number of entries returned 20" "$key_find_output"
        rlAssertGrep "ForbiddenException: Authorization Error" "$key_find_output"
        rlPhaseEnd

        rlPhaseStartTest "pki_kra_key_find-0032: Search keys using host URI parameter(https)"
        ...
        rlRun "pki -d $CERTDB_DIR \
                ...
                -c $CERTDB_DIR_PASSWORD \
                -n $valid_agent_cert \
                kra-key-find  > $key_find_output" 0 "Search keys as $valid_agent_cert"
        rlAssertGrep "Number of entries returned 20" "$key_find_output"
        rlPhaseEnd

        rlPhaseStartTest "pki_kra_key_find-0033: Search keys using valid user(Not a member of any group) should fail"
        rlRun "pki -d $CERTDB_DIR\
                -h $tmp_kra_host \
                -p $target_unsecure_port \
                -u $pki_user \
                -w $pki_pwd \
                kra-key-find  > $key_find_output 2>&1" 255,1 "Search key as $pki_user_fullName"
        rlAssertNotGrep "Number of entries returned 20" "$key_find_output"
        rlAssertGrep "ForbiddenException: Authentication method not allowed" "$key_find_output"

        rlPhaseStartTest "pki_kra_key_find-0034: Searching keys using in-valid user should fail"
        local invalid_pki_user=test1
        local invalid_pki_user_pwd=Secret123
        rlLog "Executing pki kra-key-find using user $pki_user"
        rlRun "pki -d $CERTDB_DIR \
                -h $tmp_kra_host -p $target_unsecure_port \
                -u $invalid_pki_user \
                -w $invalid_pki_user_pwd  \
                kra-key-find > $key_find_output 2>&1" 255,1 "Search key requests as $invalid_pki_user"
        rlAssertGrep "PKIException: Unauthorized" "$key_find_output"
        rlPhaseEnd

	rlPhaseStartCleanup "pki kra-key-find cleanup: Delete temp dir"
	rlRun "popd"
