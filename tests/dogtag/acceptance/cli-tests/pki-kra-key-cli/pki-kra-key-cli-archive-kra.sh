...
run_pki-kra-key-archive-kra_tests()
{
        ...
        # Creating Temporary Directory for pki kra-key-archive
	...
	local key_retrieve_output=$TmpDir/key-retrieve.out
        ...
        local tmp_junk_data=$(openssl rand -base64 200 |  perl -p -e 's/\n//')
	local tmp_passphrase=$(openssl rand -base64 10 |  perl -p -e 's/\n//')
        local profile=caUserCert


        # Config test of pki kra-key-archive
        rlPhaseStartTest "pki_key_cli-configtest: pki kra-key-archive --help configuration test"
        rlRun "pki kra-key-archive --help > $key_archive_output" 0 "pki kra-key-archive --help"
        rlAssertGrep "usage: kra-key-archive \[OPTIONS...\]" "$key_archive_output"
	...
	rlRun "pki -d $CERTDB_DIR \
		...
		-n \"$valid_agent_cert\" \
		kra-key-archive --clientKeyID $client_id \
