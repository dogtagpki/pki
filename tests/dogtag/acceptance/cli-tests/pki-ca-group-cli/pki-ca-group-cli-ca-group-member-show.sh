...
cert_info="$TmpDir/cert_info"
	...
	command="pki -d $UNTRUSTED_CERT_DB_LOCATION -n role_user_UTCA -c $UNTRUSTED_CERT_DB_PASSWORD  -h $CA_HOST -p $CA_PORT ca-group-member-show $group1 u1"
	errmsg="PKIException: Unauthorized"
	errorcode=255
    ...
    rlPhaseStartTest "pki_group_cli_group_member_show-ca-016: Should not be able to show group members using a user cert"
	#Create a user cert
        ...
        rlRun "pki -h $CA_HOST -p $CA_PORT cert-show $valid_pkcs10_serialNumber --encoded > $TmpDir/pki_ca_group_member_show_encoded_0029pkcs10.out" 0 "Executing pki cert-show $valid_pkcs10_serialNumber"
        rlRun "sed -n '/-----BEGIN CERTIFICATE-----/,/-----END CERTIFICATE-----/p' $TmpDir/pki_ca_group_member_show_encoded_0029pkcs10.out > $TmpDir/pki_ca_group_member_show_encoded_0029pkcs10.pem"
        rlRun "certutil -d $TEMP_NSS_DB -A -n \"casigningcert\" -i $CERTDB_DIR/ca_cert.pem -t \"CT,CT,CT\""
        rlRun "certutil -d $TEMP_NSS_DB -A -n pkiUser1 -i $TmpDir/pki_ca_group_member_show_encoded_0029pkcs10.pem  -t "u,u,u""
        ...
        rlRun "pki -d $TEMP_NSS_DB \
                   ...
                   -h $CA_HOST \
                   -p $CA_PORT \
                    ca-group-member-show $group1 u1 >  $TmpDir/pki-ca-group-member-show-pkiUser1-002.out 2>&1" 255 "Should not be able to show group members using a user cert"
        rlAssertGrep "PKIException: Unauthorized" "$TmpDir/pki-ca-group-member-show-pkiUser1-002.out"
    rlPhaseEnd
