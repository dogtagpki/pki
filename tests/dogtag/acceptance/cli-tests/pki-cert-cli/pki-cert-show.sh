...
run_pki-cert-show-ca_tests()
{
	...
	rlRun "pki -h $target_host -p $target_port cert-show --help > $TmpDir/cert-show.out 2>&1" 0 "pki cert-show --help"
	...
	rlRun "pki -h $target_host -p $target_port cert-show $valid_pkcs10_serialNumber > $temp_out" 0 "Executing pki cert-show $valid_pkcs10_serialNumber"
	...
	rlRun "pki -h $target_host -p $target_port cert-show  > $temp_out" 1,255 "pki cert-show without any serial number fails"
	...
	rlRun "pki -h $target_host -p $target_port cert-show $invalid_serialNumber 2> $temp_out" 1,255 "command pki cert-show $invalid_serialNumber"
	...
	rlRun "pki -h $target_host -p $target_port cert-show $decimal_valid_serialNumber_pkcs10 > $temp_out" 0 "Executing pki cert-show $decimal_valid_serialNumber_pkcs10"
	rlAssertGrep "Certificate \"$valid_pkcs10_serialNumber\"" "$temp_out"
	rlAssertGrep "Serial Number: $valid_pkcs10_serialNumber" "$temp_out"
	rlAssertGrep "Issuer: CN=PKI $CA_INST Signing Cert,O=redhat" "$temp_out"
	rlAssertGrep "Subject: $pkcs10_requestdn" "$temp_out"
	...
	rlRun "pki -h $target_host -p $target_port cert-show $invalid_hex_serialNumber 2> $temp_out" 1,255 "Executing pki cert-show $invalid_hex_serialNumber"
	rlAssertGrep "CertNotFoundException" "$temp_out"
	rlPhaseEnd
	...
	rlRun "pki -h $target_host -p $target_port cert-show \"$junk\" 2> $temp_out" 1,255 "Executing pki cert-show $junk"
	...
	rlLog "Running pki cert-show $valid_pkcs10_serialNumber"
        rlRun "pki -h $target_host -p $target_port cert-show $valid_pkcs10_serialNumber --encoded > $temp_out" 0 "command pki cert-show $valid_pkcs10_serialNumber --encoded"
	...
	rlRun "pki -h $target_host -p $target_port cert-show --encoded 1> $temp_out" 1,255 "Running pki cert-show <No-serial-Number> --encoded"
	rlAssertGrep "usage: cert-show" "$temp_out"
	rlPhaseEnd
	...
	rlLog "Running pki cert-show <invalid-serial-Number> --encoded"
	rlRun "pki -h $target_host -p $target_port cert-show $invalid_serialNumber --encoded 2> $temp_out" 1,255 "pki cert-show $serialNumber"
	...
	rlRun "pki -h $target_host -p $target_port cert-show $valid_pkcs10_serialNumber --output $temp_out" 0 "Executing pki cert-show $valid_pkcs10_serialNumber --output <file-name>"
	...
	rlRun "pki -h $target_host -p $target_port cert-show $valid_crmf_serialNumber --output $temp_out" 0 "Executing pki cert-show $valid_crmf_serialNumber --output <file-name>"
	rlAssertGrep "-----BEGIN CERTIFICATE-----" "$temp_out"
	rlAssertGrep "\-----END CERTIFICATE-----" "$temp_out"
	rlRun "openssl x509 -in $temp_out -noout -serial 1> $temp_out-openssl" 0 "Run openssl x509 on the output file"
	...
	rlRun "pki -h $target_host -p $target_port cert-show $invalid_serialNumber --output $temp_out" 1,255 "pki cert-show <invalid-serial-number> --output <file>"
	...
	rlRun "pki -h $target_host -p $target_port cert-show --output $temp_out13 >> $temp_out13_err 2>&1" 1,255
	...
	# Run pki cert-show <valid-serial-number> --pretty 
        ...
        rlRun "pki -h $target_host -p $target_port cert-show $valid_pkcs10_serialNumber --pretty > $temp_out" 0
	...
	rlRun "pki -h $target_host -p $target_port cert-show $invalid_hex_serialNumber --pretty 2> $temp_out1" 1,255 "Executing pki cert-show $invalid_hex_serialNumber --pretty"
	...
	rlRun "pki -h $target_host -p $target_port cert-show --pretty 1> $temp_out" 1,255
	...
	rlRun "pki -h $target_host -p $target_port cert-show $i18n_serialNumber 1> $temp_out" 0 "Executing pki cert-show $i18n_serialNumber"
