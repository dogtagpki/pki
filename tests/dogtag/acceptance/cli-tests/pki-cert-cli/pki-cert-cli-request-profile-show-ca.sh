...
run_pki-cert-request-profile-show-ca_tests()
{
	...
	rlRun "pki -h $target_host -p $target_port cert-request-profile-show --help 1> $cert_request_profile_show_info" 0 "pki -h $target_host -p $target_port cert-request-profile-show --help"
	...
	rlLog "Executing pki -h $target_host -p $target_port cert-request-profile-show"
	rlRun "pki -h $target_host -p $target_port cert-request-profile-show > $cert_request_profile_show_info 2>&1" 1,255
	...
	rlLog "Executing pki -h $target_host \
		-p $target_port \
		cert-request-profile-show $tmp_invalid_profile_name"
	rlRun "pki -h $target_host \
		-p $target_port \
		cert-request-profile-show $tmp_invalid_profile_name > $cert_request_profile_show_info 2>&1" 255,1 "Verify by passing invalid profile to cert-request-profile-show"
	...
	rlLog "Executing pki -h $target_host \
		-p $target_port \
		cert-request-profile-show $tmp_valid_profile_name > $cert_request_profile_show_info"
	rlRun "pki -h $target_host \
		-p $target_port \
		cert-request-profile-show $tmp_valid_profile_name > $cert_request_profile_show_info" 0 "Verify by passing valid profile to cert-request-profile-show"
	...
	rlRun "pki -h $target_host -p $target_port cert-request-profile-show $tmp_valid_profile_name --output $TmpDir/$tmp_valid_profile_name > $cert_request_profile_show_info"
	rlAssertGrep "Enrollment Template for Profile \"$tmp_valid_profile_name\"" "$cert_request_profile_show_info"
	...
	for i in ${profile_info[@]}; do
		        rlRun "pki -h $target_host -p $target_port cert-request-profile-show ${i} > /tmp/${i}-profile.out"
        RETVAL=$?
        if [ $RETVAL == 0 ]; then
	...
	rlPhaseStartTest "pki_cert_request_profile_show-006: Create a new profile and verify if the new profile's Enrollment template show up in pki -h $target_host -p $target_port cert-request-profile-show"
	...
	rlLog "Executing pki cert-request-profile-show $tmp_new_user_profile"
	rlRun "pki -h $target_host \
		-p $target_port \
		cert-request-profile-show $tmp_new_user_profile > $cert_request_profile_show_info" 0 "Verify cert-request-profile-show $tmp_new_user_profile"
        ...
        rlRun "pki -d $CERTDB_DIR -c $CERTDB_DIR_PASSWORD -n \"$CA_agentV_user\" -h $target_host -p $target_port  cert-request-profile-show $tmp_valid_profile_name 1> $cert_request_profile_show_info"
        rlAssertGrep "Enrollment Template for Profile \"$tmp_valid_profile_name\"" "$cert_request_profile_show_info"
        rlAssertGrep "Profile ID: $tmp_valid_profile_name" "$cert_request_profile_show_info"
        rlPhaseEnd
        ...
        rlRun "pki -d $CERTDB_DIR -c $CERTDB_DIR_PASSWORD -n \"$CA_agentR_user\" -h $target_host -p $target_port cert-request-profile-show $tmp_valid_profile_name > $cert_request_profile_show_info 2>&1" 1,255
        ...
        rlRun "pki -d $CERTDB_DIR -c $CERTDB_DIR_PASSWORD -n \"$CA_adminV_user\" -h $target_host -p $target_port cert-request-profile-show $tmp_valid_profile_name 1> $cert_request_profile_show_info"
        rlAssertGrep "Enrollment Template for Profile \"$tmp_valid_profile_name\"" "$cert_request_profile_show_info"
        rlAssertGrep "Profile ID: $tmp_valid_profile_name" "$cert_request_profile_show_info"
        rlPhaseEnd

        rlPhaseStartTest "pki_cert_request_profile_show-0010: Issue pki -h $target_host -p $target_port cert-request-profile-show using Expired admin cert"
        local cur_date=$(date)
		...
		-p $target_port \
		-n \"$CA_adminE_user\" \
		cert-request-profile-show $tmp_valid_profile_name > $cert_request_profile_show_info 2>&1" 1,255 "Executing cert-request-profile-show using Expired Admin cert"
	...
	rlPhaseStartTest "pki_cert_request_profile_show-0011: Issue pki -h $target_host -p $target_port cert-request-profile-show using valid audit cert"
        rlLog "Executing pki -d $CERTDB_DIR -c $CERTDB_DIR_PASSWORD -n \"$CA_auditV_user\" cert-request-profile-find"
        rlRun "pki -d $CERTDB_DIR \
		-c $CERTDB_DIR_PASSWORD \
		-n \"$CA_auditV_user\" \
		-h $target_host \
		-p $target_port  cert-request-profile-show $tmp_valid_profile_name 1> $cert_request_profile_show_info"
        rlAssertGrep "Enrollment Template for Profile \"$tmp_valid_profile_name\"" "$cert_request_profile_show_info"
        rlAssertGrep "Profile ID: $tmp_valid_profile_name" "$cert_request_profile_show_info"
        rlPhaseEnd

        rlPhaseStartTest "pki_cert_request_profile_show-0012: Issue pki -h $target_host -p $target_port cert-request-profile-show using valid operator cert"
       ...
       rlRun "pki -d $TEMP_NSS_DB \
        ...
        rlPhaseStartTest "pki_cert_request_profile_show-0014: Issue pki -h $target_host -p $target_port cert-request-profile-show using host URI parameter(https)"
        rlRun "pki -d $CERTDB_DIR \
        ...
        rlPhaseStartTest "pki_cert_request_profile_show-0015: Issue pki -h $target_host -p $target_port cert-request-profile-show using valid user"
        ...
        rlPhaseStartTest "pki_cert_request_profile_show-0016: Issue pki -h $target_host -p $target_port cert-request-profile-show using in-valid user"
	...
	rlLog "Executing pki cert-request-profile-show $tmp_new_user_profile"
        rlRun "pki -h $target_host \
		-p $target_port \
		cert-request-profile-show $tmp_new_user_profile > $cert_request_profile_show_info" 0 "cert-request-profile-show $tmp_new_user_profile"
        rlAssertGrep "Enrollment Template for Profile \"$tmp_new_user_profile\"" "$cert_request_profile_show_info"
        rlAssertGrep "Profile ID: $tmp_new_user_profile" "$cert_request_profile_show_info"
        rlPhaseEnd
