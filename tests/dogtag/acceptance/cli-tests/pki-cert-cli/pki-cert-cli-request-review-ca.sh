...
run_pki-cert-request-review-ca_tests()
{
	...
	# Creating Temporary Directory for pki cert-request-review
	...
	local cert_request_review=$TmpDir/cert-request-review.out
	...
	# Config test of pki cert-request-review
	rlPhaseStartTest "pki_cert_cli-configtest: pki cert-request-review --help configuration test"
	rlRun "pki cert-request-review --help > $cert_request_review" 0 "pki cert-request-find --help"
	rlAssertGrep "usage: cert-request-review <Request ID> \[OPTIONS...\]" "$cert_request_review"
	...
	rlPhaseStartTest "pki_cert_request_review-001: Approve enrollment request using Agent Cert(CA_agentV)"
	...
	local request_id=$(cat  $cert_request_submit | grep "Request ID:" | awk -F ": " '{print $2}')
		...
		-n \"$CA_agentV_user\" \
		cert-request-review $request_id \
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_cert_request_review-002: Reject enrollment request using Agent Cert(CA_agentV)"
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
        ...
        rlPhaseStartTest "pki_cert_request_review-003: Cancel enrollment request using Agent Cert(CA_agentV)"
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
        ...
        rlPhaseStartTest "pki_cert_request_review-004: Update enrollment request using Agent Cert(CA_agentV)"
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
	...
	local tmp_updated_date=$(date --date="$tmp_validity_period" +%Y-%m-%d)
        ...
        echo "spawn -noecho pki -d $CERTDB_DIR -h $target_host -p $target_port -n \"$CA_agentV_user\" -c $CERTDB_DIR_PASSWORD  cert-request-review $request_id --file $TEMP_NSS_DB/$requestid-req.xml" >> $exp
	...
	rlPhaseStartTest "pki_cert_request_review-005: Validate enrollment request by modifying request to have validity date beyond the default validity period(using CA_agentV) "
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
	...
	rlPhaseStartTest "pki_cert_request_review-006: Assign enrollment request to caadmin User(Member of Certificate Manager Agents)"
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
	...
	rlRun "ldapsearch -x -LLL -b \"ou=ca,ou=requests,o=pki-tomcat-CA\" -D \"cn=Directory Manager\" -w $LDAP_ROOTDNPWD \
		-h $target_host -p 1600 cn=$request_id requestOwner > $TmpDir/$request_id-ldap.out"
	rlAssertGrep "dn: cn=$request_id,ou=ca,ou=requests,o=pki-tomcat-CA" "$TmpDir/$request_id-ldap.out"
	rlAssertGrep "caadmin" "$TmpDir/$request_id-ldap.out"
	rlPhaseEnd

	rlPhaseStartTest "pki_cert_request_review-007: Unassign enrollment request assigned to caadmin(Member of Certificate Manager Agents)"
	...
	rlPhaseStartTest "pki_cert_request_review-008: Approve enrollment request containg i18n characters using Agent Cert(CA_agentV)"
        rlLog "Generate pkcs10 certificate request"
	...
	local i18n_uid=$(echo ${i}|cut -d: -f2)
        ...
        rlRun "pki -d $CERTDB_DIR \
                -c $CERTDB_DIR_PASSWORD \
                -n \"$CA_agentV_user\" \
                cert-request-review $i18n_request_id \
	...
	rlPhaseStartTest "pki_cert_request_review-009: Approve renewal requests using Agent Cert(CA_agentV)"
	...
	rlAssertGrep "Operation Result: success" "$cert_request_submit"
        rlRun "pki -d $CERTDB_DIR \
                -c $CERTDB_DIR_PASSWORD \
                -n \"$CA_agentV_user\" \
                cert-request-review $renewal_request_id \
               --action approve 1> $cert_request_review" 
        rlAssertGrep "Approved certificate request $renewal_request_id" "$cert_request_review"
        rlPhaseEnd

	rlPhaseStartTest "pki_cert_request_review-0010: Reject renewal requests using Agent Cert(CA_agentV)"
        rlLog "Generate cert with validity period of 1 Day"
        rlRun "generate_modified_cert validity_period:\"1 Day\" \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_pwd:$TEMP_NSS_DB_PWD \
	...
	rlPhaseStartTest "pki_cert_request_review-0011: Cancel renewal requests using Agent Cert(CA_agentV)"
        rlLog "Generate cert with validity period of 1 Day"
        rlRun "generate_modified_cert validity_period:\"1 Day\" \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_pwd:$TEMP_NSS_DB_PWD \
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_cert_request_review-0012: Update renewal request using Agent Cert(CA_agentV)"
        rlLog "Generate cert with validity period of 1 Day"
        rlRun "generate_modified_cert validity_period:\"1 Day\" \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_pwd:$TEMP_NSS_DB_PWD \
		...
		-c $CERTDB_DIR_PASSWORD  cert-request-review $renewal_request_id --file $TEMP_NSS_DB/$renewal_requestid-req.xml" >> $exp
        echo "expect \"Action \(approve/reject/cancel/update/validate/assign/unassign\):\"" >> $exp
        ...
        rlPhaseStartTest "pki_cert_request_review-0013: Validate renewal request by modifying request to have validity date beyond the default validity period(using CA_agentV) "
        rlLog "Generate cert with validity period of 1 Day"
        rlRun "generate_modified_cert validity_period:\"1 Day\" \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_pwd:$TEMP_NSS_DB_PWD \
	...
	rlPhaseStartTest "pki_cert_request_review-0014: Assign renewal request to caadmin User(Member of Certificate Manager Agents)"
        rlLog "Generate cert with validity period of 1 Day"
        rlRun "generate_modified_cert validity_period:\"1 Day\" \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_pwd:$TEMP_NSS_DB_PWD \
        ...
        rlRun "ldapsearch -x -LLL -b \"ou=ca,ou=requests,o=pki-tomcat-CA\" -D \"cn=Directory Manager\" -w $LDAP_ROOTDNPWD \
                -h $target_host -p 1600 cn=$renewal_request_id requestOwner > $TmpDir/$renewal_request_id-ldap.out"
        rlAssertGrep "dn: cn=$renewal_request_id,ou=ca,ou=requests,o=pki-tomcat-CA" "$TmpDir/$renewal_request_id-ldap.out"
        rlAssertGrep "caadmin" "$TmpDir/$renewal_request_id-ldap.out"
	rlPhaseEnd

        rlPhaseStartTest "pki_cert_request_review-0015: Unassign renewal request assigned using caadmin(Member of Certificate Manager Agents) "
        ...
        rlPhaseStartTest "pki_cert_request_review-0016: Approve renewal requests containg i18n characters using Agent Cert(CA_agentV)"
	rlLog "Generate cert with validity period of 1 Day"
        for i in "${i18n_array[@]}"; do
        local i18n_cn=$(echo ${i}|cut -d: -f1)
        local i18n_uid=$(echo ${i}|cut -d: -f2)
        ...
        rlPhaseStartTest "pki_cert_request_review-0017: Approving a approved Enrollment request should fail"
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
	...
	rlAssertGrep "BadRequestException: Request Not In Pending State" "$cert_request_review"
        ...
        rlPhaseStartTest "pki_cert_request_review-0018: Approving a approved Enrollment request should fail with --file option"
        rlLog "Approving an already approved Enrollment request $request_id with --file option"
        rlLog "Execuing pki cert-request-review $request_id --file $TEMP_NSS_DB/$request_id-req.xml"
        echo "set timeout 5" > $exp
        echo "set force_conservative 0" >> $exp
        echo "set send_slow {1 .1}" >> $exp
        echo "spawn -noecho pki -d $CERTDB_DIR -h $target_host -p $target_port -n \"$CA_agentV_user\" \
		-c $CERTDB_DIR_PASSWORD  cert-request-review $request_id --file $TEMP_NSS_DB/$request_id-req.xml" >> $exp
        echo "expect \"Action \(approve/reject/cancel/update/validate/assign/unassign\):\"" >> $exp
        echo "send -- \"approve\r\"" >> $exp
        echo "expect eof" >> $exp
        rlRun "/usr/bin/expect -f $exp > $expout 2>&1"
        ...
        rlPhaseStartTest "pki_cert_request_review-0019: Approving a approved renewal request should fail"
        rlLog "Generate cert with validity period of 1 Day"
        rlRun "generate_modified_cert validity_period:\"1 Day\" \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_pwd:$TEMP_NSS_DB_PWD \
	...
	rlLog "Approving an already approved Renewal request"
        ...
        rlPhaseStartTest "pki_cert_request_review-0020: Approving a approved renewal request should fail with --file option"
        rlLog "Approving an already approved renewal request $renewal_request_id with --file option"
        rlLog "Execuing pki cert-request-review $renewal_request_id --file $TEMP_NSS_DB/$renewal_request_id-req.xml"
        echo "set timeout 5" > $exp
        echo "set force_conservative 0" >> $exp
        echo "set send_slow {1 .1}" >> $exp
        echo "spawn -noecho pki -d $CERTDB_DIR -h $target_host -p $target_port -n \"$CA_agentV_user\" \
		-c $CERTDB_DIR_PASSWORD  cert-request-review $renewal_request_id --file $TEMP_NSS_DB/$renewal_request_id-req.xml" >> $exp
        echo "expect \"Action \(approve/reject/cancel/update/validate/assign/unassign\):\"" >> $exp
        echo "send -- \"approve\r\"" >> $exp
        echo "expect eof" >> $exp
        rlRun "/usr/bin/expect -f $exp > $expout 2>&1"
	...
	rlPhaseStartTest "pki_cert_request_review-0021: Approving a non-existent request should fail"
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_cert_request_review-0022: Approving a non-existent request should fail with --file option"
	...
	rlPhaseStartTest "pki_cert_request_review-0023: Approving a rejected Enrollment request should fail"
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_cert_request_review-0024: Approving a canceled Enrollment request should fail"
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
        ...
        rlPhaseStartTest "pki_cert_request_review-0025: Approving a canceled Enrollment request should fail with --file option"
        rlLog "Approving an rejected Enrollment request with --file option"
        rlLog "Execuing pki cert-request-review $request_id --file $TEMP_NSS_DB/$request_id-req.xml"
        echo "set timeout 5" > $exp
        echo "set force_conservative 0" >> $exp
        echo "set send_slow {1 .1}" >> $exp
        echo "spawn -noecho pki -d $CERTDB_DIR -h $target_host -p $target_port -n \"$CA_agentV_user\" -c $CERTDB_DIR_PASSWORD  cert-request-review $request_id --file $TEMP_NSS_DB/$request_id-req.xml" >> $exp
        echo "expect \"Action \(approve/reject/cancel/update/validate/assign/unassign\):\"" >> $exp
        echo "send -- \"approve\r\"" >> $exp
        echo "expect eof" >> $exp
        rlRun "/usr/bin/expect -f $exp > $expout 2>&1"
        ...
        rlPhaseStartTest "pki_cert_request_review-0026: Approving a rejected Enrollment request should fail with --file option"
	rlLog "Approving an rejected Enrollment request with --file option"
	rlLog "Execuing pki cert-request-review $request_id --file $TEMP_NSS_DB/$request_id-req.xml"
        echo "set timeout 5" > $exp
        echo "set force_conservative 0" >> $exp
	...
	rlPhaseStartTest "pki_cert_request_review-0027: Verify pki cert-request-review fails when no request-id is passed with --action approve"
	local request_id=" "
	rlLog "No request id is passed to pki cert-request-review"
        rlRun "pki -d $CERTDB_DIR \
                -c $CERTDB_DIR_PASSWORD \
                -n \"$CA_agentV_user\" \
                cert-request-review $request_id \
               --action approve > $cert_request_review 2>&1" 1,255 "No request id is passed to pki cert-request-review"
	...
	rlPhaseStartTest "pki_cert_request_review-0028: Approve an enrollment request as CA_agentV assigned to caadmin user(Member of Certificate Manager Agents)"
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
	...
	rlPhaseStartTest "pki_cert_request_review-0029: Approve an unassigned Enrollment request"
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
	...
	rlPhaseStartTest "pki_cert_request_review-0030: Approve a validated Enrollment request"
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_cert_request_review-0031: Verify pki cert-request-review fails when no request-id is passed with --file option"
        local request_id=" "
        rlLog "No request id is passed to pki cert-request-review"
        rlRun "pki -d $CERTDB_DIR \
                -c $CERTDB_DIR_PASSWORD \
                -n \"$CA_agentV_user\" \
                cert-request-review $request_id \
               --file $TmpDir/$rand\.xml  > $cert_request_review 2>&1" 1,255 "No request id is passed to pki cert-request-review"
        rlAssertGrep "Error: Missing Certificate Request ID" "$cert_request_review"
        rlAssertGrep "usage: cert-request-review <Request ID> \[OPTIONS...\]" "$cert_request_review"
        rlAssertGrep "    --action <action>   Action: approve, reject, cancel, update, validate," "$cert_request_review"
        rlAssertGrep "                        assign, unassign" "$cert_request_review"
	...
	rlPhaseStartTest "pki_cert_request_review-0032: Rejecting a approved Enrollment request should fail"
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
        ...
        rlPhaseStartTest "pki_cert_request_review-0033: Rejecting a approved Enrollment request should fail with --file option"
        ...
        rlPhaseStartTest "pki_cert_request_review-0034: Rejecting a approved renewal request should fail"
        rlLog "Generate cert with validity period of 1 Day"
        rlRun "generate_modified_cert validity_period:\"1 Day\" \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_pwd:$TEMP_NSS_DB_PWD \
        ...
        rlPhaseStartTest "pki_cert_request_review-0035: Rejecting approved renewal request should fail with --file option"
        rlLog "Rejecting an already approved renewal request $renewal_request_id with --file option"
        rlLog "Execuing pki cert-request-review $renewal_request_id --file $TEMP_NSS_DB/$renewal_request_id-req.xml"
        echo "set timeout 5" > $exp
        echo "set force_conservative 0" >> $exp
        echo "set send_slow {1 .1}" >> $exp
        echo "spawn -noecho pki -d $CERTDB_DIR -h $target_host -p $target_port -n \"$CA_agentV_user\" -c $CERTDB_DIR_PASSWORD  cert-request-review $renewal_request_id --file $TEMP_NSS_DB/$renewal_request_id-req.xml" >> $exp
        echo "expect \"Action \(approve/reject/cancel/update/validate/assign/unassign\):\"" >> $exp
        echo "send -- \"reject\r\"" >> $exp
        echo "expect eof" >> $exp
        rlRun "/usr/bin/expect -f $exp > $expout 2>&1"
        ...
        rlPhaseStartTest "pki_cert_request_review-0036: Rejecting an non-existent request should fail"
        local request_id=$request_id$rand
        local hex_requestid=$(echo "obase=16;$request_id"|bc)
        local conv_lower_val=${hex_requestid,,}
        local tmp_request_id=0x$conv_lower_val
        ...
        rlPhaseStartTest "pki_cert_request_review-0037: Rejecting a non-existent request should fail with --file option"
        ...
        rlPhaseStartTest "pki_cert_request_review-0038: Rejecting a rejected Enrollment request should fail"
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
        ...
        rlPhaseStartTest "pki_cert_request_review-0039: Rejecting a rejected Enrollment request with --file option should fail"
        ...
        rlPhaseStartTest "pki_cert_request_review-0040: Verify pki cert-request-review fails when no request-id is passed with --action reject"
        local request_id=" "
        rlLog "No request id is passed to pki cert-request-review"
        rlRun "pki -d $CERTDB_DIR \
                -c $CERTDB_DIR_PASSWORD \
                -n \"$CA_agentV_user\" \
                cert-request-review $request_id \
               --action reject > $cert_request_review 2>&1" 1,255 "No request id is passed to pki cert-request-review"
        rlAssertGrep "Error: Missing Certificate Request ID" "$cert_request_review"
        rlAssertGrep "usage: cert-request-review <Request ID> \[OPTIONS...\]" "$cert_request_review"
        rlAssertGrep "    --action <action>   Action: approve, reject, cancel, update, validate," "$cert_request_review"
        rlAssertGrep "                        assign, unassign" "$cert_request_review"
        ...
        rlPhaseStartTest "pki_cert_request_review-0041: Cancelling a approved Enrollment request should fail"
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
        ...
        rlPhaseStartTest "pki_cert_request_review-0042: Cancelling a approved Enrollment request should fail with --file option"
        ...
        rlPhaseStartTest "pki_cert_request_review-0043: Canceling a approved renewal request should fail"
        rlLog "Generate cert with validity period of 1 Day"
        rlRun "generate_modified_cert validity_period:\"1 Day\" \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_pwd:$TEMP_NSS_DB_PWD \
        ...
        rlPhaseStartTest "pki_cert_request_review-0044: Canceling a approved renewal request should fail with --file option"
        ...
        rlPhaseStartTest "pki_cert_request_review-0045: Cancel a non-existent request should fail"
        local request_id=$request_id$rand
        local hex_requestid=$(echo "obase=16;$request_id"|bc)
        local conv_lower_val=${hex_requestid,,}
        local tmp_request_id=0x$conv_lower_val
        ...
        rlPhaseStartTest "pki_cert_request_review-0046: cancelling a non-existent request should fail with --file option"
        ...
        rlPhaseStartTest "pki_cert_request_review-0047: Canceling a rejected Enrollment request should fail"
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
        ...
        rlPhaseStartTest "pki_cert_request_review-0048: Canceling a rejected Enrollment request should fail with --file option"
        ...
        rlPhaseStartTest "pki_cert_request_review-0049: Verify pki cert-request-review fails when no request-id is passed with --action cancel"
        local request_id=" "
        rlLog "No request id is passed to pki cert-request-review"
        rlRun "pki -d $CERTDB_DIR \
                -c $CERTDB_DIR_PASSWORD \
                -n \"$CA_agentV_user\" \
                cert-request-review $request_id \
               --action cancel > $cert_request_review 2>&1" 1,255 "No request id is passed to pki cert-request-review"
        rlAssertGrep "Error: Missing Certificate Request ID" "$cert_request_review"
        rlAssertGrep "usage: cert-request-review <Request ID> \[OPTIONS...\]" "$cert_request_review"
        rlAssertGrep "    --action <action>   Action: approve, reject, cancel, update, validate," "$cert_request_review"
        rlAssertGrep "                        assign, unassign" "$cert_request_review"
	...
	rlPhaseStartTest "pki_cert_request_review-0050: update a approve Enrollment request"
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
        ...
        echo "set send_slow {1 .1}" >> $exp
        echo "spawn -noecho pki -d $CERTDB_DIR -h $target_host -p $target_port -n \"$CA_agentV_user\" \
	 -c $CERTDB_DIR_PASSWORD  cert-request-review $request_id --file $TEMP_NSS_DB/$requestid-req.xml" >> $exp
        echo "expect \"Action \(approve/reject/cancel/update/validate/assign/unassign\):\"" >> $exp
        echo "system \"xmlstarlet ed -L -u \\\"certReviewResponse/ProfilePolicySet/policies/def/policyAttribute\[\@name='notAfter'\]/Value\\\" \
	...
	rlPhaseStartTest "pki_cert_request_review-0051: update a rejected Enrollment request"
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_cert_request_review-0052: Updating a non-existent request should fail"
        local request_id=$request_id$rand
        local hex_requestid=$(echo "obase=16;$request_id"|bc)
        local conv_lower_val=${hex_requestid,,}
        local tmp_request_id=0x$conv_lower_val
        ...
        rlPhaseStartTest "pki_cert_request_review-0053: Updating a non-existent request should fail with --file option"
        rlLog "Cancellig an non-existent Enrollment request with --file option"
        rlLog "Execuing pki cert-request-review $request_id --file $TEMP_NSS_DB/$request_id-req.xml"
        echo "set timeout 5" > $exp
        echo "set force_conservative 0" >> $exp
        ...
        rlPhaseStartTest "pki_cert_request_review-0054: Verify pki cert-request-review fails when no request-id is passed with --action update"
        local request_id=" "
        rlLog "No request id is passed to pki cert-request-review"
        rlRun "pki -d $CERTDB_DIR \
                -c $CERTDB_DIR_PASSWORD \
                -n \"$CA_agentV_user\" \
                cert-request-review $request_id \
               --action update > $cert_request_review 2>&1" 1,255 "No request id is passed to pki cert-request-review"
        rlAssertGrep "Error: Missing Certificate Request ID" "$cert_request_review"
        rlAssertGrep "usage: cert-request-review <Request ID> \[OPTIONS...\]" "$cert_request_review"
        rlAssertGrep "    --action <action>   Action: approve, reject, cancel, update, validate," "$cert_request_review"
        rlAssertGrep "                        assign, unassign" "$cert_request_review"
	...
	rlPhaseStartTest "pki_cert_request_review-0055: validate a approved Enrollment request"
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
	...
	rlPhaseStartTest "pki_cert_request_review-0056: validate a rejected Enrollment request"
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_cert_request_review-0057: Validate a non-existent request should fail"
        local request_id=$request_id$rand
        local hex_requestid=$(echo "obase=16;$request_id"|bc)
        local conv_lower_val=${hex_requestid,,}
        local tmp_request_id=0x$conv_lower_val
        ...
        rlPhaseStartTest "pki_cert_request_review-0058: Validating a non-existent request should fail with --file option"
	...
	rlAssertGrep "Request ID $tmp_request_id not found" "$expout"
        ...
        rlPhaseStartTest "pki_cert_request_review-0059: Verify pki cert-request-review fails when no request-id is passed with --action validate"
        local request_id=" "
        rlLog "No request id is passed to pki cert-request-review"
        rlRun "pki -d $CERTDB_DIR \
                -c $CERTDB_DIR_PASSWORD \
                -n \"$CA_agentV_user\" \
                cert-request-review $request_id \
               --action validate > $cert_request_review 2>&1" 1,255 "No request id is passed to pki cert-request-review"
        rlAssertGrep "Error: Missing Certificate Request ID" "$cert_request_review"
        rlAssertGrep "usage: cert-request-review <Request ID> \[OPTIONS...\]" "$cert_request_review"
        rlAssertGrep "    --action <action>   Action: approve, reject, cancel, update, validate," "$cert_request_review"
        rlAssertGrep "                        assign, unassign" "$cert_request_review"
        ...
        rlPhaseStartTest "pki_cert_request_review-0060: Assign a approved Enrollment request to caadmin(Member of Certificate Manager Agents) user"
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
	...
	rlLog "Assigning Approve certificate request $request_id to caadmin user"
        ...
        rlPhaseStartTest "pki_cert_request_review-0061: assign a rejected Enrollment request"
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_cert_request_review-0062: Assign a non-existent request should fail"
        local request_id=$request_id$rand
        local hex_requestid=$(echo "obase=16;$request_id"|bc)
        local conv_lower_val=${hex_requestid,,}
        local tmp_request_id=0x$conv_lower_val
        ...
        rlPhaseStartTest "pki_cert_request_review-0063: Assigning a non-existent request should fail with --file option"
	...
	rlAssertGrep "Request ID $tmp_request_id not found" "$expout"
        ...
        rlPhaseStartTest "pki_cert_request_review-0064: Verify pki cert-request-review fails when no request-id is passed with --action assign"
        local request_id=" "
        rlLog "No request id is passed to pki cert-request-review"
        rlRun "pki -d $CERTDB_DIR \
                -c $CERTDB_DIR_PASSWORD \
                -n \"$CA_agentV_user\" \
                cert-request-review $request_id \
               --action assign > $cert_request_review 2>&1" 1,255 "No request id is passed to pki cert-request-review"
        rlAssertGrep "Error: Missing Certificate Request ID" "$cert_request_review"
        rlAssertGrep "usage: cert-request-review <Request ID> \[OPTIONS...\]" "$cert_request_review"
        rlAssertGrep "    --action <action>   Action: approve, reject, cancel, update, validate," "$cert_request_review"
        rlAssertGrep "                        assign, unassign" "$cert_request_review"
        ...
        rlPhaseStartTest "pki_cert_request_review-0065: Unassign a approved Enrollment request to caadmin(Member of Certificate Manager Agents) user"
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
        ...
        rlPhaseStartTest "pki_cert_request_review-0066: Unassign a rejected Enrollment request"
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
        ...
        rlPhaseStartTest "pki_cert_request_review-0067: Unassign a non-existent request should fail"
        local request_id=$request_id$rand
        local hex_requestid=$(echo "obase=16;$request_id"|bc)
        local conv_lower_val=${hex_requestid,,}
        local tmp_request_id=0x$conv_lower_val
        ...
        rlPhaseStartTest "pki_cert_request_review-0069: Assigning a non-existent request should fail with --file option"
        ...
        rlPhaseStartTest "pki_cert_request_review-0069: Verify pki cert-request-review fails when no request-id is passed with --action unassign"
        local request_id=" "
        rlLog "No request id is passed to pki cert-request-review"
        rlRun "pki -d $CERTDB_DIR \
                -c $CERTDB_DIR_PASSWORD \
                -n \"$CA_agentV_user\" \
                cert-request-review $request_id \
               --action unassign > $cert_request_review 2>&1" 1,255 "No request id is passed to pki cert-request-review"
        rlAssertGrep "Error: Missing Certificate Request ID" "$cert_request_review"
        rlAssertGrep "usage: cert-request-review <Request ID> \[OPTIONS...\]" "$cert_request_review"
        rlAssertGrep "    --action <action>   Action: approve, reject, cancel, update, validate," "$cert_request_review"
        rlAssertGrep "                        assign, unassign" "$cert_request_review"
	...
	rlPhaseStartTest "pki_cert_request_review-0070: Approve an Enrollment request using admin cert(CA_adminV)"
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
	...
	rlLog "Approve Certificate request $request_id as $CA_adminV_user user"
        ...
        rlAssertGrep "Authorization failed on resource: certServer.ca.certrequests, operation: execute" "$cert_request_review"
	rlPhaseEnd

        rlPhaseStartTest "pki_cert_request_review-0071: Approve an Enrollment request using caadmin (Member of Certificate Manager Agents) cert"
	...
	rlPhaseStartTest "pki_cert_request_review-0072: Approve an Enrollment request using audit Cert(CA_auditV)"
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_cert_request_review-0073: Approve Enrollment request using operator Cert(CA_operatorV)"
	...
	rlPhaseStartTest "pki_cert_request_review-0074: Approve Enrollment request using Normal user cert who has no privileges"
        local profile=caUserCert
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_cert_request_review-0075: Approve Enrollment request using valid user"
        rlLog "Approving $request_id using user $pki_user"
        rlRun "pki -d $CERTDB_DIR \
                -u $pki_user \
                -w $pki_pwd \
                cert-request-review $request_id --action approve > $cert_request_review 2>&1" 1,255 
	rlAssertGrep "Authentication method not allowed" "$cert_request_review"
	rlPhaseEnd

        rlPhaseStartTest "pki_cert_request_review-0076: Approve Enrollment request using in-valid user"
	...
	rlPhaseStartTest "pki_cert_request_review-0077: Approve Enrollment request using Expired Admin cert"
        local cur_date=$(date)
	...
	rlLog "Approve $request_id using $CA_adminE_user"
        rlRun "pki -d $CERTDB_DIR \
		-c $CERTDB_DIR_PASSWORD \
		-n \"$CA_adminE_user\" cert-request-review $request_id --action approve > $cert_request_review 2>&1" 1,255
	...
	rlPhaseStartTest "pki_cert_request_review-0078: Approve Enrollment request using Expired Agent cert"
        local cur_date=$(date)
        ...
        rlRun "pki -d $CERTDB_DIR \
		-c $CERTDB_DIR_PASSWORD \
		-n \"$CA_agentE_user\" cert-request-review $request_id --action approve > $cert_request_review 2>&1" 1,255
        rlAssertGrep "ProcessingException: Unable to invoke request" "$cert_request_review"
        rlLog "Set the date back to it's original date & time"
        rlRun "chronyc -a -m 'settime $cur_date + 10 seconds' 'makestep' 'manual reset' 'online' 1> $TmpDir/chrony.out"
        rlAssertGrep "200 OK" "$TmpDir/chrony.out"
	...
	rlPhaseStartTest "pki_cert_request_review-0079: Approve Enrollment request using Revoked agent cert"
	...
	rlPhaseStartTest "pki_cert_request_review-0080: Issue pki cert-request-review using host URI parameter(https) and approve the request"
        rlRun "pki -d $CERTDB_DIR \
	...
	rlPhaseStartTest "pki_cert_request_review-0081: Reject an Enrollment request using admin cert(CA_adminV)"
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_cert_request_review-0082: Reject an Enrollment request using caadmin(Member of Certificate Manager Agents) cert"
	...
	rlPhaseStartTest "pki_cert_request_review-0083: Reject an Enrollment request using audit Cert(CA_auditV)"
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_cert_request_review-0084: Reject Enrollment request using operator Cert(CA_operatorV)"
	...
	rlPhaseStartTest "pki_cert_request_review-0085: Reject Enrollment request using Normal user cert who has no privileges"
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_cert_request_review-0086: Reject Enrollment request using valid user"
        rlLog "Rejecting $request_id using user $pki_user"
        rlRun "pki -d $CERTDB_DIR \
                -u $pki_user \
                -w $pki_pwd \
                cert-request-review $request_id --action reject > $cert_request_review 2>&1" 1,255 
	rlAssertGrep "Authentication method not allowed" "$cert_request_review"
	rlPhaseEnd

        rlPhaseStartTest "pki_cert_request_review-0087: Reject Enrollment request using in-valid user"
        local invalid_pki_user=test1
        local invalid_pki_user_pwd=Secret123
	...
	rlPhaseStartTest "pki_cert_request_review-0088: Reject Enrollment request using Expired Admin cert"
        local cur_date=$(date)
        local end_date=$(certutil -L -d $CERTDB_DIR -n CA_adminE | grep "Not After" | awk -F ": " '{print $2}')
        rlLog "Current Date/Time: $(date)"
        rlLog "Current Date/Time: before modifying using chrony $(date)"
	...
	rlLog "Reject $request_id using $CA_adminE_user"
        rlRun "pki -d $CERTDB_DIR \
		-c $CERTDB_DIR_PASSWORD \
		-n \"$CA_adminE_user\" cert-request-review $request_id \
	...
	rlPhaseStartTest "pki_cert_request_review-0089: Reject Enrollment request using Expired Agent cert"
        local cur_date=$(date)
        local end_date=$(certutil -L -d $CERTDB_DIR -n CA_agentE | grep "Not After" | awk -F ": " '{print $2}')
        rlLog "Current Date/Time: $(date)"
        rlLog "Current Date/Time: before modifying using chrony $(date)"
        ...
        rlRun "pki -d $CERTDB_DIR \
		-c $CERTDB_DIR_PASSWORD \
		-n \"$CA_agentE_user\" cert-request-review $request_id \
		--action reject> $cert_request_review 2>&1" 1,255
        rlAssertGrep "ProcessingException: Unable to invoke request" "$cert_request_review"
        rlLog "Set the date back to it's original date & time"
        rlRun "chronyc -a -m 'settime $cur_date + 10 seconds' 'makestep' 'manual reset' 'online' 1> $TmpDir/chrony.out"
	...
	rlPhaseStartTest "pki_cert_request_review-0090: Reject Enrollment request using Revoked agent cert"
	...
	rlPhaseStartTest "pki_cert_request_review-0091: Issue pki cert-request-review using host URI parameter(https) and reject the request"
        rlRun "pki -d $CERTDB_DIR \
		-U  https://$target_host:$target_https_port \
		-c $CERTDB_DIR_PASSWORD \
                -n \"$CA_agentV_user\" \
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_cert_request_review-0092: Cancel Enrollment request using Admin Cert(CA_adminV)"
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
	...
	rlPhaseStartTest "pki_cert_request_review-0093: Cancel Enrollment request using Admin Cert(caadmin) having agent privileges"
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_cert_request_review-0094: Cancel Enrollment request using Audit Cert(CA_auditV)"
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
        ...
        rlPhaseStartTest "pki_cert_request_review-0095: Cancel Enrollment request using Operator Cert(CA_operatorV)"
	...
	rlPhaseStartTest "pki_cert_request_review-0096: Cancel Enrollment request using Normal user cert who has no privileges"
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_cert_request_review-0097: Cancel Enrollment request using valid user"
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_cert_request_review-0098: Cancel Enrollment request using in-valid user"
        local invalid_pki_user=test1
        local invalid_pki_user_pwd=Secret123
        rlLog "Cancel $request_id using invalid user $invalid_pki_user"
        rlRun "pki -d $CERTDB_DIR \
                -u $invalid_pki_user \
                -w $invalid_pki_user_pwd cert-request-review $request_id \
		--action cancel > $cert_request_review 2>&1" 1,255 "Cancel $request_id using invalid user"
        rlAssertGrep "PKIException: Unauthorized" "$cert_request_review"
        rlPhaseEnd

	rlPhaseStartTest "pki_cert_request_review-0099: Cancel Enrollment request using Expired Admin cert"
        local cur_date=$(date)
        local end_date=$(certutil -L -d $CERTDB_DIR -n CA_adminE | grep "Not After" | awk -F ": " '{print $2}')
        rlLog "Current Date/Time: $(date)"
        rlLog "Current Date/Time: before modifying using chrony $(date)"
	...
	rlLog "Cancel $request_id using $CA_adminE_user"
        rlRun "pki -d $CERTDB_DIR \
		-c $CERTDB_DIR_PASSWORD \
		-n \"$CA_adminE_user\" cert-request-review $request_id --action cancel > $cert_request_review 2>&1" 1,255
        rlAssertGrep "ProcessingException: Unable to invoke request" "$cert_request_review"
        rlLog "Set the date back to it's original date & time"
        rlRun "chronyc -a -m 'settime $cur_date + 10 seconds' 'makestep' 'manual reset' 'online' 1> $TmpDir/chrony.out"
        rlAssertGrep "200 OK" "$TmpDir/chrony.out"
	...
	rlPhaseStartTest "pki_cert_request_review-0100: Cancel Enrollment request using Expired Agent cert"
        local cur_date=$(date)
        local end_date=$(certutil -L -d $CERTDB_DIR -n CA_agentE | grep "Not After" | awk -F ": " '{print $2}')
        rlLog "Current Date/Time: $(date)"
        rlLog "Current Date/Time: before modifying using chrony $(date)"
        ...
        rlRun "pki -d $CERTDB_DIR \
		-c $CERTDB_DIR_PASSWORD \
		-n \"$CA_agentE_user\" cert-request-review $request_id --action cancel > $cert_request_review 2>&1" 1,255
        rlAssertGrep "ProcessingException: Unable to invoke request" "$cert_request_review"
        rlLog "Set the date back to it's original date & time"
        rlRun "chronyc -a -m 'settime $cur_date + 10 seconds' 'makestep' 'manual reset' 'online' 1> $TmpDir/chrony.out"
        rlAssertGrep "200 OK" "$TmpDir/chrony.out"
	...
	rlPhaseStartTest "pki_cert_request_review-0101: Cancel Enrollment request using Revoked agent cert"
	...
	rlPhaseStartTest "pki_cert_request_review-0102: Issue pki cert-request-review using host URI parameter(https) and approve the request"
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_cert_request_review-0103: Update Enrollment request using Admin Cert(CA_admiV)"
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
	...
	rlLog "Update Enrollment request using $CA_adminV_user"
        ...
        echo "expect eof" >> $exp
        rlRun "/usr/bin/expect -f $exp > $expout 2>&1" 1
        rlAssertGrep "Authorization failed on resource: certServer.ca.certrequests, operation: execute" "$expout"
        rlPhaseEnd

	rlPhaseStartTest "pki_cert_request_review-0104: Update Enrollment request using caadmin(Member of Certificate Manager Agents) cert"
	...
	rlPhaseStartTest "pki_cert_request_review-0105: Update Enrollment request using Audit Cert(CA_auditV)"
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
	...
	rlPhaseStartTest "pki_cert_request_review-0106: Update Enrollment request using operator Cert(CA_operatorV)"
	...
	rlPhaseStartTest "pki_cert_request_review-0107: Update Enrollment request using Normal user cert with no privileges"
	rlLog "Update Enrollment request $request_id using $pki_user"
        ...
        echo "spawn -noecho pki -d $TEMP_NSS_DB -h $target_host -p $target_port -n \"$pki_user\" \
                -c $TEMP_NSS_DB_PWD  cert-request-review $request_id --file $TEMP_NSS_DB/$requestid-req.xml" >> $exp
        echo "expect \"Action \(approve/reject/cancel/update/validate/assign/unassign\):\"" >> $exp
        echo "system \"xmlstarlet ed -L -u \\\"certReviewResponse/ProfilePolicySet/policies/def/policyAttribute\[\@name='notAfter'\]/Value\\\" \
                -v \\\"$tmp_updated_date 00:00:10\\\" $TEMP_NSS_DB/$requestid-req.xml\"" >> $exp
        echo "send -- \"update\r\"" >> $exp
	...
	rlPhaseStartTest "pki_cert_request_review-0108: Update Enrollment request using valid user"
        rlLog "Update Enrollment request $request_id using $pki_user"
        local tmp_validity_period="1 day"
        local tmp_updated_date=$(date --date="$tmp_validity_period" +%Y-%m-%d)
        echo "set timeout 5" > $exp
        ...
        echo "spawn -noecho pki -d $CERTDB_DIR -h $target_host -p $target_port -u $pki_user \
                -w $pki_pwd cert-request-review $request_id --file $TEMP_NSS_DB/$requestid-req.xml" >> $exp
        echo "expect \"Action \(approve/reject/cancel/update/validate/assign/unassign\):\"" >> $exp
        echo "system \"xmlstarlet ed -L -u \\\"certReviewResponse/ProfilePolicySet/policies/def/policyAttribute\[\@name='notAfter'\]/Value\\\" \
                -v \\\"$tmp_updated_date 00:00:10\\\" $TEMP_NSS_DB/$requestid-req.xml\"" >> $exp
        echo "send -- \"update\r\"" >> $exp
	...
	rlPhaseStartTest "pki_cert_request_review-0109: Update Enrollment request using in-valid user"
        ...
        echo "spawn -noecho pki -d $CERTDB_DIR -h $target_host -p $target_port -u $invalid_pki_user \
                -w $invalid_pki_user_pwd cert-request-review $request_id --file $TEMP_NSS_DB/$requestid-req.xml" >> $exp
        echo "expect \"Action \(approve/reject/cancel/update/validate/assign/unassign\):\"" >> $exp
        echo "system \"xmlstarlet ed -L -u \\\"certReviewResponse/ProfilePolicySet/policies/def/policyAttribute\[\@name='notAfter'\]/Value\\\" \
                -v \\\"$tmp_updated_date 00:00:10\\\" $TEMP_NSS_DB/$requestid-req.xml\"" >> $exp
        echo "send -- \"update\r\"" >> $exp
	...
	rlPhaseStartTest "pki_cert_request_review-0110: Update Enrollment request using Expired Agent cert"
        local cur_date=$(date)
        local end_date=$(certutil -L -d $CERTDB_DIR -n CA_agentE | grep "Not After" | awk -F ": " '{print $2}')
        rlLog "Current Date/Time: $(date)"
        rlLog "Current Date/Time: before modifying using chrony $(date)"
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_cert_request_review-0111: Update Enrollment request using Expired Admin cert"
        local cur_date=$(date)
        local end_date=$(certutil -L -d $CERTDB_DIR -n CA_adminE | grep "Not After" | awk -F ": " '{print $2}')
        rlLog "Current Date/Time: $(date)"
        rlLog "Current Date/Time: before modifying using chrony $(date)"
	...
	rlPhaseStartTest "pki_cert_request_review-0112: Update Enrollment request using Revoked agent cert"
	...
	rlPhaseStartTest "pki_cert_request_review-0113: Issue pki cert-request-review using host URI parameter(https) and update the request"
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_cert_request_review-0114: Validate Enrollment request using Admin Cert(CA_adminV) "
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
	...
	rlPhaseStartTest "pki_cert_request_review-0115: Validate Enrollment request using caadmin(Member of Certificate Manager Agents) cert"
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_cert_request_review-0116: Validate Enrollment request using Audit Cert(CA_auditV) "
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
        ...
        rlPhaseStartTest "pki_cert_request_review-0117: Validate Enrollment request using Operator Cert(CA_operatorV)"
	...
	rlPhaseStartTest "pki_cert_request_review-0118: Validate Enrollment request using normal user cert with no privileges"
        ...
        rlPhaseStartTest "pki_cert_request_review-0119: Validate Enrollment request using valid user"
        rlLog "Validate $request_id using $pki_user"
        local tmp_validity_period="900 days"
        local tmp_updated_date=$(date --date="$tmp_validity_period" +%Y-%m-%d)
        echo "set timeout 5" > $exp
        ...
        rlPhaseStartTest "pki_cert_request_review-0120: Validate Enrollment request using In-valid user"
	...
	rlPhaseStartTest "pki_cert_request_review-0121: Validate Enrollment request using Expired admin Cert"
        local cur_date=$(date)
        local end_date=$(certutil -L -d $CERTDB_DIR -n CA_adminE | grep "Not After" | awk -F ": " '{print $2}')
        rlLog "Current Date/Time: $(date)"
        rlLog "Current Date/Time: before modifying using chrony $(date)"
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_cert_request_review-0122: Validate Enrollment request using expired agent Cert"
        local cur_date=$(date)
        local end_date=$(certutil -L -d $CERTDB_DIR -n CA_agentE | grep "Not After" | awk -F ": " '{print $2}')
        rlLog "Current Date/Time: $(date)"
        rlLog "Current Date/Time: before modifying using chrony $(date)"
	...
	rlPhaseStartTest "pki_cert_request_review-0123: Validate Enrollment request using Revoked agent cert"
	rlLog "Update $request_id as Normal user $CA_adminR_user"
        local tmp_validity_period="900 day"
        local tmp_updated_date=$(date --date="$tmp_validity_period" +%Y-%m-%d)
        echo "set timeout 5" > $exp
	...
	rlPhaseStartTest "pki_cert_request_review-0124: Issue pki cert-request-review using host URI parameter(https) and validate the request"
	rlLog "Update $request_id by connecting to CA using https port $https_port"
        local tmp_validity_period="900 days"
        local tmp_updated_date=$(date --date="$tmp_validity_period" +%Y-%m-%d)
        echo "set timeout 5" > $exp
	...
	rlPhaseStartTest "pki_cert_request_review-0125: Assign Enrollment request using Admin Cert(CA_adminV)"
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_cert_request_review-0126: Assign an Enrollment request to CA_agentV user"
	rlLog "Assign Enrollment request as $CA_agentV_user user"
        ...
        rlRun "ldapsearch -x -LLL -b \"ou=ca,ou=requests,o=pki-tomcat-CA\" -D \"cn=Directory Manager\" -w $LDAP_ROOTDNPWD \
                -h $target_host -p 1600 cn=$request_id requestOwner > $TmpDir/$request_id-ldap.out"
        rlAssertGrep "dn: cn=$request_id,ou=ca,ou=requests,o=pki-tomcat-CA" "$TmpDir/$request_id-ldap.out"
        rlAssertGrep "CA_agentV" "$TmpDir/$request_id-ldap.out"
        rlPhaseEnd

	rlPhaseStartTest "pki_cert_request_review-0127: Assign an Enrollment request using Audit Cert(CA_auditV)"
	rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
	...
	rlPhaseStartTest "pki_cert_request_review-0128: Assign Enrollment request using operator cert(CA_operatorV)"
	...
	rlPhaseStartTest "pki_cert_request_review-0129: Assign Enrollment request using normal user cert with no privileges"
	...
	rlPhaseStartTest "pki_cert_request_review-0130: Assign Enrollment request using valid user"
	...
	rlPhaseStartTest "pki_cert_request_review-0131: Assign Enrollment request using in-valid user"
	...
	rlPhaseStartTest "pki_cert_request_review-0132: Assign Enrollment request using Expired Admin cert"
        local cur_date=$(date)
        local end_date=$(certutil -L -d $CERTDB_DIR -n CA_adminE | grep "Not After" | awk -F ": " '{print $2}')
        rlLog "Current Date/Time: $(date)"
        rlLog "Current Date/Time: before modifying using chrony $(date)"
	...
	rlPhaseStartTest "pki_cert_request_review-0133: Assign Enrollment request using Expired Agent cert"
        local cur_date=$(date)
        local end_date=$(certutil -L -d $CERTDB_DIR -n CA_agentE | grep "Not After" | awk -F ": " '{print $2}')
        rlLog "Current Date/Time: $(date)"
        rlLog "Current Date/Time: before modifying using chrony $(date)"
	...
	rlPhaseStartTest "pki_cert_request_review-0134: Assign Enrollment request using Revoked agent cert"
	...
	rlPhaseStartTest "pki_cert_request_review-0135: Issue pki cert-request-review using host URI parameter(https) and assign the request"
        rlRun "pki -d $CERTDB_DIR \
		-U  https://$target_host:$target_https_port \
		-c $CERTDB_DIR_PASSWORD \
                -n \"$CA_agentV_user\" \
	...
	rlPhaseStartTest "pki_cert_request_review-0136: Unassign Enrollment request using Admin Cert(CA_adminV)"
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_cert_request_review-0137: Unassign an Enrollment request to CA_agentV user"
	...
	rlPhaseStartTest "pki_cert_request_review-0138: Unassign an Enrollment request using Audit Cert(CA_auditV)"
	rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
	...
	rlPhaseStartTest "pki_cert_request_review-0139: Unassign Enrollment request using operator cert(CA_operatorV)"
	...
	rlPhaseStartTest "pki_cert_request_review-0140: Unassign Enrollment request using normal user cert with no privileges"
	...
	rlPhaseStartTest "pki_cert_request_review-0141: Unassign Enrollment request using valid user"
	...
	rlPhaseStartTest "pki_cert_request_review-0142: Unassign Enrollment request using in-valid user"
	...
	rlPhaseStartTest "pki_cert_request_review-0143: Unassign Enrollment request using Expired Admin cert"
        local cur_date=$(date)
        local end_date=$(certutil -L -d $CERTDB_DIR -n CA_adminE | grep "Not After" | awk -F ": " '{print $2}')
        rlLog "Current Date/Time: $(date)"
        rlLog "Current Date/Time: before modifying using chrony $(date)"
	...
	rlPhaseStartTest "pki_cert_request_review-0144: Unassign Enrollment request using Expired Agent cert"
        local cur_date=$(date)
        local end_date=$(certutil -L -d $CERTDB_DIR -n CA_agentE | grep "Not After" | awk -F ": " '{print $2}')
        rlLog "Current Date/Time: $(date)"
        rlLog "Current Date/Time: before modifying using chrony $(date)"
	...
	rlPhaseStartTest "pki_cert_request_review-0145: Unassign Enrollment request using Revoked agent cert"
	...
	rlPhaseStartTest "pki_cert_request_review-0146: Issue pki cert-request-review using host URI parameter(https) and unassign the request"
        rlRun "pki -d $CERTDB_DIR \
		-U  https://$target_host:$target_https_port \
		-c $CERTDB_DIR_PASSWORD \
                -n \"$CA_agentV_user\" \
	...
	local cert_serialNumber=$(pki cert-request-show $request_id | grep "Certificate ID:" | awk -F ": " '{print $2}')
	rlRun "pki cert-show $cert_serialNumber --pretty | grep  -A 4 \"Critical: yes\" > $TmpDir/cert-show.out "
	...
	rlPhaseStartTest "pki_cert_request_review-0148: Approve Enrollment request from caUserSMIMEcapCert profile"
        ...
        local cert_serialNumber=$(pki cert-request-show $request_id | grep "Certificate ID:" | awk -F ": " '{print $2}')
        rlRun "pki cert-show $cert_serialNumber --pretty | grep  -A 14 \"Critical: yes\" > $TmpDir/cert-show.out "
        rlLog "Verify only Key Encipherment extension is only added"
        rlAssertGrep "Key Encipherment" "$TmpDir/cert-show.out"
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_cert_request_review-0149: Approve Enrollment request from AdminCert profile"
	...
	rlPhaseStartTest "pki_cert_request_review-0150: Approve Enrollment request from caSignedLogCert profile"
        ...
        rlLog "Update $tmp_profile xml with certificate request details"
        rlRun "generate_cert_request_xml $TEMP_NSS_DB/$rand-request.pem $TEMP_NSS_DB/$rand-subject.out $TEMP_NSS_DB/$rand-profile.xml $profile"
        rlRun "pki cert-request-submit $TEMP_NSS_DB/$rand-profile.xml 1> $cert_request_submit" 
        rlAssertGrep "Request Status: pending" "$cert_request_submit"
        rlAssertGrep "Operation Result: success" "$cert_request_submit"
        local request_id=$(cat  $cert_request_submit | grep "Request ID:" | awk -F ": " '{print $2}')
        rlRun "pki -d $CERTDB_DIR \
        ...
        rlLog "Verify Certificate has Key Encipherment extension added"
        local cert_serialNumber=$(pki cert-request-show $request_id | grep "Certificate ID:" | awk -F ": " '{print $2}')
        rlRun "pki cert-show $cert_serialNumber --pretty | grep  -A 5 \"Critical: yes\" > $TmpDir/cert-show.out "
	...
	rlRun "pki cert-show $cert_serialNumber --pretty | grep  -A 4 \"Extended Key Usage:\" > $TmpDir/cert-show.out"
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_cert_request_review-0152: Approve Enrollment request from caServerCert profile"
        ...
        rlPhaseStartTest "pki_cert_request_review-0153: Approve Enrollment request from caSubsystemCert profile"
        ...
        rlPhaseStartTest "pki_cert_request_review-0154: Approve Enrollment request from caOtherCert profile"
	...
	rlPhaseStartTest "pki_cert_request_review-0155: Approve Enrollment request from caCACert profile"
        ...
        rlLog "Verify Certificate has Key Encipherment extension added"
        local cert_serialNumber=$(pki cert-request-show $request_id | grep "Certificate ID:" | awk -F ": " '{print $2}')
        rlRun "pki cert-show $cert_serialNumber --pretty | grep  -A 15 \"Critical: yes\" > $TmpDir/cert-show.out"
        rlLog "Verify Key Usage extensions are added"
        rlAssertGrep "Digital Signature" "$TmpDir/cert-show.out"
        rlAssertGrep "Non Repudiation" "$TmpDir/cert-show.out"
        rlAssertNotGrep "Key Encipherment" "$TmpDir/cert-show.out"
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_cert_request_review-0156: Approve Enrollment request from caCrossSignedCACert profile"
	...
	rlAssertGrep "Is CA: yes" "$TmpDir/cert-show.out"
        ...
        rlPhaseStartTest "pki_cert_request_review-0157: Approve Enrollment request from caInstallCACert profile"
	...
	rlPhaseStartTest "pki_cert_request_review-0158: Approve Enrollment request from caStorageCert profile"
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_cert_request_review-0159: Approve Enrollment request from caTransportCert profile"
	...
	rlPhaseStartTest "pki_cert_request_review-0160: Cancel a canceled Enrollment request"
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
	...
	rlPhaseStartTest "pki_cert_request_review-0161: canceling a Unassigned Enrollment request should fail"
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
	...
	rlPhaseStartTest "pki_cert_request_review-0162: canceling an enrollment request as CA_agentV assigned to caadmin(Member of Certificate Manager Agents) should fail"
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
	...
	rlPhaseStartTest "pki_cert_request_review-0163: cancel a validated Enrollment request"
	rlLog "validate enrollment request $request_id"
        echo "set timeout 5" > $exp
        echo "set force_conservative 0" >> $exp
        echo "set send_slow {1 .1}" >> $exp
        echo "spawn -noecho pki -d $CERTDB_DIR -h $target_host -p $target_port -n \"$admin_cert_nickname\" -c $CERTDB_DIR_PASSWORD  cert-request-review $request_id --file $TEMP_NSS_DB/$requestid-req.xml" >> $exp
        echo "expect \"Action \(approve/reject/cancel/update/validate/assign/unassign\):\"" >> $exp
        echo "send -- \"validate\r\"" >> $exp
        echo "expect eof" >> $exp
        rlRun "/usr/bin/expect -f $exp > $expout 2>&1"
	...
	rlRun "pki -d $CERTDB_DIR \
		...
		-n \"$CA_agentV_user\" \
		-c $CERTDB_DIR_PASSWORD \
		cert-request-review --action validate $request_id"
	rlAssertGrep "Validated certificate request $request_id" "$expout"
	rlPhaseEnd

	rlPhaseStartTest "pki_cert_request_review-0165: validate Enrollment request as CA_agentV assigned to caadmin(Member of Certificate Manager Agents)"
	rlLog "Assign $request_id to caadmin"
        ...
        rlRun "pki -d $CERTDB_DIR \
                ...
                -n \"$CA_agentV_user\" \
                -c $CERTDB_DIR_PASSWORD \
                cert-request-review --action validate $request_id > $cert_request_review 2>&1" 255,1
	rlAssertGrep "PKIException: Problem approving request in CertRequestResource.assignRequest! Not authorized to do this operation" "$cert_request_review"
	rlPhaseEnd

	rlPhaseStartTest "pki_cert_request_review-0166: validate unassigned Enrollment request"
        rlLog "Generate pkcs10 certificate request"
        rlRun "create_new_cert_request \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_password:$TEMP_NSS_DB_PWD \
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_cert_request_review-0167: Assign renewal request to CA_agentV User which was initially assigned to caadmin(Member of Certificate Manager Agents)"
        rlLog "Generate cert with validity period of 1 Day"
        rlRun "generate_modified_cert validity_period:\"1 Day\" \
                tmp_nss_db:$TEMP_NSS_DB \
                tmp_nss_db_pwd:$TEMP_NSS_DB_PWD \
	...
	rlAssertGrep "PKIException: Problem approving request in CertRequestResource.assignRequest! Not authorized to do this operation" "$cert_request_review"
        ...
        rlPhaseStartTest "pki_cert_request_review-0168: Unassign renewal request assigned to CA_adminV using CA_agentV"
        rlLog "Un assign $renewal_request_id"
        rlRun "pki -d $CERTDB_DIR \
                -c $CERTDB_DIR_PASSWORD \
                -n \"$CA_agentV_user\" \
	...
	rlPhaseStartCleanup "pki cert-request-review cleanup: Delete temp dir"
	rlRun "popd"
