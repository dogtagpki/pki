...
#   Description: PKI tks-group-find CLI tests
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# The following pki cli commands needs to be tested:
#  pki-tks-group-cli-tks-group-find  To  list  groups in TKS.
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#
...
run_pki-tks-group-cli-tks-group-find_tests(){
...
#### Create Temporary directory ####    
     ...
     rlPhaseStartSetup "pki_tks_group_cli_tks_group_add-startup: Create temporary directory"
...
ROOTCA_agent_user=${caId}_agentV
    rlPhaseStartSetup "pki_tks_group_cli_tks_group_find-startup: Create temporary directory and add groups"
	i=1
        while [ $i -lt 25 ] ; do
			   ...
			   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -h $TKS_HOST \
                    -p $TKS_PORT \
                           tks-group-add --description=test_group g$i"
                let i=$i+1
        done
    rlPhaseEnd
...
local cert_info="$TmpDir/cert_info"
    ...
    rlPhaseStartTest "pki_tks_group_cli_tks_group_find-configtest-001: pki tks-group-find --help configuration test"
        rlRun "pki tks-group-find --help > $TmpDir/tks_group_find.out 2>&1" 0 "pki tks-group-find --help"
        rlAssertGrep "usage: tks-group-find \[FILTER\] \[OPTIONS...\]" "$TmpDir/tks_group_find.out"
        rlAssertGrep "\--size <size>     Page size" "$TmpDir/tks_group_find.out"
        rlAssertGrep "\--start <start>   Page start" "$TmpDir/tks_group_find.out"
        rlAssertGrep "\--help            Show help options" "$TmpDir/tks_group_find.out"
    rlPhaseEnd

    rlPhaseStartTest "pki_tks_group_cli_tks_group_find-configtest-002: pki tks-group-find configuration test"
	command="pki tks-group-find"
	errmsg="ProcessingException: Unable to invoke request"
	errorcode=255
	rlRun "verifyErrorMsg \"$command\" \"$errmsg\" \"$errorcode\"" 0 "Verify expected error message - pki tks-group-find"
    rlPhaseEnd

    rlPhaseStartTest "pki_tks_group_cli_tks_group_find-003: Find 5 groups, --size=5"
	rlRun "pki -d $CERTDB_DIR \
		   -n $(eval echo \$${subsystemId}_adminV_user) \
                    -c $CERTDB_DIR_PASSWORD \
                    -h $TKS_HOST \
                    -p $TKS_PORT \
                    tks-group-find --size=5  > $TmpDir/pki-tks-group-find-001.out 2>&1" \
                         0 \
                        "Found 5 groups"
	rlAssertGrep "Number of entries returned 5" "$TmpDir/pki-tks-group-find-001.out"
    rlPhaseEnd

    rlPhaseStartTest "pki_tks_group_cli_tks_group_find-004: Find no group, --size=0"
        rlRun "pki -d $CERTDB_DIR \
		   -n $(eval echo \$${subsystemId}_adminV_user) \
                    -c $CERTDB_DIR_PASSWORD \
                    -h $TKS_HOST \
                    -p $TKS_PORT \
                    tks-group-find --size=0  > $TmpDir/pki-tks-group-find-002.out 2>&1" \
                    0 \
                    "Found no groups"
        rlAssertGrep "Number of entries returned 0" "$TmpDir/pki-tks-group-find-002.out"
    rlPhaseEnd

    rlPhaseStartTest "pki_tks_group_cli_tks_group_find-005: Find all groups, large value as input"
        large_num=1000000
	rlRun "pki -d $CERTDB_DIR \
		   -n $(eval echo \$${subsystemId}_adminV_user) \
                    -c $CERTDB_DIR_PASSWORD \
                    -h $TKS_HOST \
                    -p $TKS_PORT \
                   tks-group-find --size=$large_num  > $TmpDir/pki-tks-group-find-003.out 2>&1" \
                         0 \ 
    ...
    rlPhaseStartTest "pki_tks_group_cli_tks_group_find-006: Find all groups, --size with maximum possible value as input"
	...
	rlLog "pki -d $CERTDB_DIR \
		   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -h $TKS_HOST \
                    -p $TKS_PORT \
                    tks-group-find --size=$maximum_check"
        rlRun "pki -d $CERTDB_DIR \
		   -n $(eval echo \$${subsystemId}_adminV_user) \
                    -c $CERTDB_DIR_PASSWORD \
                    -h $TKS_HOST \
                    -p $TKS_PORT \
                   tks-group-find --size=$maximum_check  > $TmpDir/pki-tks-group-find-003_2.out 2>&1" \
                   0 \
    ...
    rlPhaseStartTest "pki_tks_group_cli_tks_group_find-007: Find all groups, --size more than maximum possible value"
	randhex=$(openssl rand -hex 12 |  perl -p -e 's/\n//')
	randhex_covup=${randhex^^}
        maximum_check=$(echo "ibase=16;$randhex_covup"|bc)
	command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_adminV_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT tks-group-find --size=$maximum_check"
    ...
    rlPhaseStartTest "pki_tks_group_cli_tks_group_find-008: Find groups, check for negative input --size=-1"
	command="pki -d $CERTDB_DIR  -n $(eval echo \$${subsystemId}_adminV_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT tks-group-find --size=-1"
    ...
    rlPhaseStartTest "pki_tks_group_cli_tks_group_find-009: Find groups for size input as noninteger, --size=abc"
        size_noninteger="abc"
	command="pki -d $CERTDB_DIR  -n $(eval echo \$${subsystemId}_adminV_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT tks-group-find --size=$size_noninteger"
    ...
    rlPhaseStartTest "pki_tks_group_cli_tks_group_find-010: Find groups, check for no input --size="
	command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_adminV_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT tks-group-find --size="
    ...
    rlPhaseStartTest "pki_tks_group_cli_tks_group_find-011: Find groups, --start=10"
	#Find the 10th group
        rlRun "pki -d $CERTDB_DIR \
		   -n $(eval echo \$${subsystemId}_adminV_user) \
                    -c $CERTDB_DIR_PASSWORD \
                    -h $TKS_HOST \
                    -p $TKS_PORT \
                   tks-group-find  > $TmpDir/pki-tks-group-find-007_1.out 2>&1" \
                   0 \
	...
	rlLog "pki -d $CERTDB_DIR \
		   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -h $TKS_HOST \
                    -p $TKS_PORT \
                    tks-group-find --start=10"
        rlRun "pki -d $CERTDB_DIR \
		   -n $(eval echo \$${subsystemId}_adminV_user) \
                    -c $CERTDB_DIR_PASSWORD \
                    -h $TKS_HOST \
                    -p $TKS_PORT \
                   tks-group-find --start=10  > $TmpDir/pki-tks-group-find-007.out 2>&1" \
                   0 \
    ...
    rlPhaseStartTest "pki_tks_group_cli_tks_group_find-012: Find groups, --start=10000, large possible input"
        rlRun "pki -d $CERTDB_DIR \
		   -n $(eval echo \$${subsystemId}_adminV_user) \
                    -c $CERTDB_DIR_PASSWORD \
                    -h $TKS_HOST \
                    -p $TKS_PORT \
                   tks-group-find --start=10000  > $TmpDir/pki-tks-group-find-008.out 2>&1" \
                    0 \
                   "Find users, --start=10000, large possible input"
        rlAssertGrep "Number of entries returned 0" "$TmpDir/pki-tks-group-find-008.out"
    rlPhaseEnd

    rlPhaseStartTest "pki_tks_group_cli_tks_group_find-013: Find groups, --start with maximum possible input"
	randhex=$(openssl rand -hex 3 |  perl -p -e 's/\n//')
        randhex_covup=${randhex^^}
        maximum_check=$(echo "ibase=16;$randhex_covup"|bc)
	rlLog "pki -d $CERTDB_DIR \
		   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -h $TKS_HOST \
                    -p $TKS_PORT \
                   tks-group-find --start=$maximum_check"
        rlRun "pki -d $CERTDB_DIR \
		   -n $(eval echo \$${subsystemId}_adminV_user) \
                    -c $CERTDB_DIR_PASSWORD \
                    -h $TKS_HOST \
                    -p $TKS_PORT \
                   tks-group-find --start=$maximum_check  > $TmpDir/pki-tks-group-find-008_2.out 2>&1" \
                    0 \
    ...
    rlPhaseStartTest "pki_tks_group_cli_tks_group_find-014: Find groups, --start with more than maximum possible input"
	randhex=$(openssl rand -hex 12 |  perl -p -e 's/\n//')
        randhex_covup=${randhex^^}
        maximum_check=$(echo "ibase=16;$randhex_covup"|bc)
	command="pki -d $CERTDB_DIR  -n $(eval echo \$${subsystemId}_adminV_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT tks-group-find --start=$maximum_check"
	errmsg="NumberFormatException: For input string: \"$maximum_check\""
	errorcode=255
    ...
    rlPhaseStartTest "pki_tks_group_cli_tks_group_find-015: Find groups, --start=0"
        rlRun "pki -d $CERTDB_DIR \
		   -n $(eval echo \$${subsystemId}_adminV_user) \
                    -c $CERTDB_DIR_PASSWORD \
                    -h $TKS_HOST \
                    -p $TKS_PORT \
                    tks-group-find --start=0  > $TmpDir/pki-tks-group-find-009.out 2>&1" \
                     0 \
    ...
    rlPhaseStartTest "pki_tks_group_cli_tks_group_find-016: Find groups, --start=-1"
	command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_adminV_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT tks-group-find --start=-1"
    ...
    rlPhaseStartTest "pki_tks_group_cli_tks_group_find-017: Find groups for size input as noninteger, --start=abc"
        size_noninteger="abc"
	command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_adminV_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT tks-group-find --start=$size_noninteger"
	errmsg="NumberFormatException: For input string: \"$size_noninteger\""
	errorcode=255
	rlRun "verifyErrorMsg \"$command\" \"$errmsg\" \"$errorcode\"" 0 "Verify expected error message - start with non integer value should fail"
    rlPhaseEnd

    rlPhaseStartTest "pki_tks_group_cli_tks_group_find-018: Find groups, check for no input --start= "
	command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_adminV_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT tks-group-find --start="
	errmsg="NumberFormatException: For input string: \"""\""
	errorcode=255
	rlRun "verifyErrorMsg \"$command\" \"$errmsg\" \"$errorcode\"" 0 "Verify expected error message - start with empty value should fail"
    rlPhaseEnd

    rlPhaseStartTest "pki_tks_group_cli_tks_group_find-019: Find groups, --size=12 --start=12"
        ...
        rlRun "pki -d $CERTDB_DIR \
		   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -h $TKS_HOST \
                    -p $TKS_PORT \
                   tks-group-find  > $TmpDir/pki-tks-group-find-00_13_1.out 2>&1" \
                     0 \
                   "Get all groups in TKS"
        ...
        rlRun "pki -d $CERTDB_DIR \
		   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -h $TKS_HOST \
                    -p $TKS_PORT \
                   tks-group-find --start=12 --size=12  > $TmpDir/pki-tks-group-find-0013.out 2>&1" \
                   0 \
    ...
    rlPhaseStartTest "pki_tks_group_cli_tks_group_find-020: Find groups, --size=0 --start=12"
        #Find 12 groups starting from 12th group
        rlRun "pki -d $CERTDB_DIR \
		   -n $(eval echo \$${subsystemId}_adminV_user) \
                    -c $CERTDB_DIR_PASSWORD \
                    -h $TKS_HOST \
                    -p $TKS_PORT \
                    tks-group-find  > $TmpDir/pki-tks-group-find-00_14_1.out 2>&1" \
                  0 \
                        "Get all groups in TKS"
        ...
        rlRun "pki -d $CERTDB_DIR \
		   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -h $TKS_HOST \
                    -p $TKS_PORT \
                   tks-group-find --start=12 --size=0  > $TmpDir/pki-tks-group-find-0014.out 2>&1" \
                    0 \
    ...
    rlPhaseStartTest "pki_tks_group_cli_tks_group_find-021: Should not be able to find group using a revoked cert TKS_adminR"
	command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_adminR_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT tks-group-find --start=1 --size=5"
    ...
    rlPhaseStartTest "pki_tks_group_cli_tks_group_find-022: Should not be able to find groups using an agent with revoked cert TKS_agentR"
	command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_agentR_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT tks-group-find --start=1 --size=5"
	errmsg="PKIException: Unauthorized"
	errorcode=255
    ...
    rlPhaseStartTest "pki_tks_group_cli_tks_group_find-023: Should not be able to find groups using a valid agent TKS_agentV user"
	command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_agentV_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT tks-group-find --start=1 --size=5"
    ...
    rlPhaseStartTest "pki_tks_group_cli_tks_group_find-024: Should not be able to find groups using admin user with expired cert TKS_adminE"
        rlRun "date --set='next day'" 0 "Set System date a day ahead"
	rlRun "date --set='next day'" 0 "Set System date a day ahead"
	rlRun "date"
	command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_adminE_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT tks-group-find --start=1 --size=5"
	errmsg="ForbiddenException: Authorization Error"
	errorcode=255
    ...
    rlPhaseStartTest "pki_tks_group_cli_tks_group_find-025: Should not be able to find groups using TKS_agentE cert"
        rlRun "date --set='next day'" 0 "Set System date a day ahead"
	rlRun "date --set='next day'" 0 "Set System date a day ahead"
	rlRun "date"
	command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_agentE_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT tks-group-find --start=1 --size=5"
	errmsg="ForbiddenException: Authorization Error"
	errorcode=255
    ...
    rlPhaseEnd
     ...
     rlPhaseStartTest "pki_tks_group_cli_tks_group_find-026: Should not be able to find groups using a TKS_auditV"
	command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_auditV_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT tks-group-find --start=1 --size=5"
	errmsg="ForbiddenException: Authorization Error"
	errorcode=255
    ...
    rlPhaseStartTest "pki_tks_group_cli_tks_group_find-027: Should not be able to find groups using a TKS_operatorV"
	command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_operatorV_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT tks-group-find --start=1 --size=5"
	errmsg="ForbiddenException: Authorization Error"
	errorcode=255
    ...
    rlPhaseStartTest "pki_tks_group_cli_tks_group_find-028: Should not be able to find groups using a cert created from a untrusted CA TKS_adminUTCA"
	command="pki -d $UNTRUSTED_CERT_DB_LOCATION -n role_user_UTCA -c $UNTRUSTED_CERT_DB_PASSWORD -h $TKS_HOST -p $TKS_PORT tks-group-find --start=1 --size=5"
	errmsg="PKIException: Unauthorized"
	errocode=255
    ...
    rlPhaseStartTest "pki_tks_group_cli_tks_group_find-029: Should not be able to find groups using a user cert"
	#Create a user cert
	...
	rlLog "Executing: pki -d $TEMP_NSS_DB \
                   ...
                   -h $TKS_HOST \
                   -p $TKS_PORT \
                    tks-group-find --start=1 --size=5"
        rlRun "pki -d $TEMP_NSS_DB \
                   -n pkiUser1 \
                   -c $TEMP_NSS_DB_PASSWD \
		   -h $TKS_HOST \
		   -p $TKS_PORT \
                    tks-group-find --start=1 --size=5 >  $TmpDir/pki-tks-group-find-pkiUser1-002.out 2>&1" 255 "Should not be able to find groups using a user cert"
        rlAssertGrep "PKIException: Unauthorized" "$TmpDir/pki-tks-group-find-pkiUser1-002.out"
    rlPhaseEnd

    rlPhaseStartTest "pki_tks_group_cli_tks_group_find-030: find groups when group id has i18n characters"
	rlLog "pki -d $CERTDB_DIR \
                   -n $(eval echo \$${subsystemId}_adminV_user) \
                    -c $CERTDB_DIR_PASSWORD \
                    -h $TKS_HOST \
                    -p $TKS_PORT \
                    tks-group-add --description='Örjan Äke' 'ÖrjanÄke'"
        rlRun "pki -d $CERTDB_DIR \
		   -n $(eval echo \$${subsystemId}_adminV_user) \
                    -c $CERTDB_DIR_PASSWORD \
                    -h $TKS_HOST \
                    -p $TKS_PORT \
                    tks-group-add --description='Örjan Äke' 'ÖrjanÄke' > $TmpDir/pki-tks-group-find-001_31.out 2>&1" \
                    0 \
        ...
        rlLog "pki -d $CERTDB_DIR \
		   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -h $TKS_HOST \
                    -p $TKS_PORT \
                    tks-group-find --size=1000"
        rlRun "pki -d $CERTDB_DIR \
		   -n $(eval echo \$${subsystemId}_adminV_user) \
                    -c $CERTDB_DIR_PASSWORD \
                    -h $TKS_HOST \
                    -p $TKS_PORT \
                    tks-group-find --size=1000 > $TmpDir/pki-tks-group-show-001_31_2.out" \
                    0 \
    ...
    rlPhaseStartTest "pki_tks_group_cli_tks_group_find-031: find group when group id has i18n characters"
	rlLog "pki -d $CERTDB_DIR \
                   -n $(eval echo \$${subsystemId}_adminV_user) \
                    -c $CERTDB_DIR_PASSWORD \
                    -h $TKS_HOST \
                    -p $TKS_PORT \
                    tks-group-add --description='Éric Têko' 'ÉricTêko'"
        rlRun "pki -d $CERTDB_DIR \
		   -n $(eval echo \$${subsystemId}_adminV_user) \
                    -c $CERTDB_DIR_PASSWORD \
                    -h $TKS_HOST \
                    -p $TKS_PORT \
                    tks-group-add --description='Éric Têko' 'ÉricTêko' > $TmpDir/pki-tks-group-show-001_32.out 2>&1" \
                    0 \
        ...
        rlRun "pki -d $CERTDB_DIR \
		   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -h $TKS_HOST \
                    -p $TKS_PORT \
                    tks-group-find --size=1000 > $TmpDir/pki-tks-group-show-001_32_2.out" \
                    0 \
                    "Find group with max size"
	...
	rlPhaseStartTest "pki_tks_group_cli_tks_group_find-032: find group - filter 'Administrator'"
        rlLog "pki -d $CERTDB_DIR \
		   -n $(eval echo \$${subsystemId}_adminV_user) \
                    -c $CERTDB_DIR_PASSWORD \
                    -h $TKS_HOST \
                    -p $TKS_PORT \
                    tks-group-find Administrator"
        rlRun "pki -d $CERTDB_DIR \
		   -n $(eval echo \$${subsystemId}_adminV_user) \
                    -c $CERTDB_DIR_PASSWORD \
                    -h $TKS_HOST \
                    -p $TKS_PORT \
                    tks-group-find Administrator > $TmpDir/pki-tks-group-show-033.out" \
                    0 \
                    "Find group with Keyword Administrator"
	rlAssertGrep "Group ID: Administrators" "$TmpDir/pki-tks-group-show-033.out"
    rlPhaseEnd

	rlPhaseStartTest "pki_tks_group_cli_tks_group_find-033: find group should fail when filter keyword has less than 3 characters"
	command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_adminV_user) -c $CERTDB_DIR_PASSWORD -h $TKS_HOST -p $TKS_PORT tks-group-find CA"
	errmsg="BadRequestException: Filter is too short."
	errorcode=255
	rlRun "verifyErrorMsg \"$command\" \"$errmsg\" \"$errorcode\"" 0 "Verify expected error message - pki tks-group-find should fail if the filter has less than 3 characters"	
    rlPhaseEnd
	...
	#===Deleting i18n groups created using TKS_adminV cert===#
		...
		-n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -h $TKS_HOST \
                    -p $TKS_PORT \
                tks-group-del 'ÖrjanÄke' > $TmpDir/pki-group-del-tks-group-i18n_1.out" \
                0 \
		...
		-n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -h $TKS_HOST \
                    -p $TKS_PORT \
              tks-group-del 'ÉricTêko' > $TmpDir/pki-group-del-tks-group-i18n_2.out" \
                0 \
    ...
    rlPhaseEnd
        ...
        rlPhaseStartCleanup "pki tks-group-find cleanup: Delete temp dir"
        rlRun "popd"
        rlRun "rm -r $TmpDir" 0 "Removing tmp directory"
