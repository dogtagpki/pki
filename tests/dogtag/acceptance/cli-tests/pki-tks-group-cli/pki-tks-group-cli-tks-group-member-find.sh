	...
	rlPhaseStartTest "pki_tks_group_cli_tks_group_member-find-031:Find group-member for group id with i18n characters"
		...
		rlLog "Create a group dadministʁasjɔ̃ with i18n characters"
			   ...
			   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -h $TKS_HOST \
                    -p $TKS_PORT \
                            tks-group-add 'dadministʁasjɔ̃' --description \"Admininstartors in French\" 2>&1 > $TmpDir/pki-tks-group-member-add-groupadd-031_1.out" \
                            0 \
		...
		rlLog "pki -d $CERTDB_DIR \
			   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -h $TKS_HOST \
                    -p $TKS_PORT \
                            tks-group-member-add \"dadministʁasjɔ̃\" u9"
                rlRun "pki -d $CERTDB_DIR \
			   -n $(eval echo \$${subsystemId}_adminV_user) \
                    -c $CERTDB_DIR_PASSWORD \
                    -h $TKS_HOST \
                    -p $TKS_PORT \
                           tks-group-member-add \"dadministʁasjɔ̃\" u9 > $TmpDir/pki-tks-group-member-find-groupadd-031_2.out" \
                            0 \
			   ...
			   -n $(eval echo \$${subsystemId}_adminV_user) \
                    ...
                    -h $TKS_HOST \
                    -p $TKS_PORT \
                            tks-group-member-find \"dadministʁasjɔ̃\" > $TmpDir/pki-tks-group-member-find-groupadd-find-031_3.out" \
                            0 \
