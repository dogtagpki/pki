...
if [ "$tps_instance_created" = "TRUE" ] ;  then
	...
	rlPhaseStartTest "pki_tps_user_cli_tps_user_membership-add-009: Should be able to tps-user-membership-add user name with i18n characters"
		rlLog "tps-user-add user fullname ÖrjanÄke with i18n characters"
	        rlLog "pki -d $CERTDB_DIR \
	 		   ...
	 		   -h $SUBSYSTEM_HOST \
 			   -p $(eval echo \$${subsystemId}_UNSECURE_PORT) \
	                    tps-user-add --fullName='ÖrjanÄke' u5"
        	rlRun "pki -d $CERTDB_DIR \
                	   -n ${TPS_INST}_adminV \
	                   -c $CERTDB_DIR_PASSWORD \
	 		   -h $SUBSYSTEM_HOST \
	...
	rlPhaseStartTest "pki_tps_user_cli_tps_user_membership-add-010: Should be able to tps-user-membership-add user to group id with i18n characters"
	 		   ...
	 		   -h $SUBSYSTEM_HOST \
	 		   -p $(eval echo \$${subsystemId}_UNSECURE_PORT) \
                            tps-user-add --fullName='Éric Têko' u6"
                rlRun "pki -d $CERTDB_DIR \
                           -n ${TPS_INST}_adminV \
                           -c $CERTDB_DIR_PASSWORD \
	 		   -h $SUBSYSTEM_HOST \
		...
		rlRun "pki -d $CERTDB_DIR \
	 		   ...
	 		   -h $SUBSYSTEM_HOST \
	 		   -p $(eval echo \$${subsystemId}_UNSECURE_PORT) \
                            group-add 'dadministʁasjɔ̃' --description \"Admininstartors in French\" 2>&1 > $TmpDir/pki-tps-user-membership-add-groupadd-010_1.out" \
                            0 \
                            "Adding group dadministʁasjɔ̃ with i18n characters"
                rlAssertGrep "Added group \"dadministʁasjɔ̃\"" "$TmpDir/pki-tps-user-membership-add-groupadd-010_1.out"   
                rlAssertGrep "Group ID: dadministʁasjɔ̃" "$TmpDir/pki-tps-user-membership-add-groupadd-010_1.out"   
			   ...
			   -h $SUBSYSTEM_HOST \
 			   -p $(eval echo \$${subsystemId}_UNSECURE_PORT) \
                            tps-user-membership-add u6 \"dadministʁasjɔ̃\" > $TmpDir/pki-tps-user-membership-add-groupadd-010_2.out" \
                            0 \
                            "Adding user ÉricTêko to group \"dadministʁasjɔ̃\""
                rlAssertGrep "Added membership in \"dadministʁasjɔ̃\"" "$TmpDir/pki-tps-user-membership-add-groupadd-010_2.out"    
                rlAssertGrep "Group: dadministʁasjɔ̃" "$TmpDir/pki-tps-user-membership-add-groupadd-010_2.out"
                        rlLog "Check if the user is added to the group"
                        rlRun "pki -d $CERTDB_DIR \
                                   -n ${TPS_INST}_adminV \
                                   -c $CERTDB_DIR_PASSWORD \
 				   ...
 				   -p $(eval echo \$${subsystemId}_UNSECURE_PORT) \
                        ...
                        rlAssertGrep "Group: dadministʁasjɔ̃" "$TmpDir/pki-tps-user-membership-add-groupadd-find-tps-010_3.out"
        rlPhaseEnd
...
Import CA certificate (Y/n)? \"" >> $expfile
	...
	rlPhaseStartCleanup "pki_tps_user_cli_tps_user_membership-add-cleanup-001: Deleting the temp directory and users"
		...
		rlRun "pki -d $CERTDB_DIR \
 			...
 			-h $SUBSYSTEM_HOST \
	 		-p $(eval echo \$${subsystemId}_UNSECURE_PORT) \
	                tps-group-del 'dadministʁasjɔ̃' > $TmpDir/pki-tps-user-del-tps-group-i18n_1.out" \
        	        0 \
                	"Deleting group dadministʁasjɔ̃"
	        rlAssertGrep "Deleted group \"dadministʁasjɔ̃\"" "$TmpDir/pki-tps-user-del-tps-group-i18n_1.out"
