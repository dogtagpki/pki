...
#   Description: PKI ocsp-user-cert-add CLI tests
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# The following pki cli commands needs to be tested:
#  pki-ocsp-user-cli-ocsp-user-cert-add    Add certs to users in the pki ocsp subsystem.
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#
...
######################################################################################
#create_role_users.sh should be first executed prior to pki-ocsp-user-cli-ocsp-user-cert-add.sh
######################################################################################
...
run_pki-ocsp-user-cli-ocsp-user-cert-add_tests(){
	subsystemId=$1
	SUBSYSTEM_TYPE=$2
	MYROLE=$3
	caId=$4
...
rlPhaseStartTest "pki_ocsp_user_cli_ocsp_user_cert-add-002: Add one cert to a user should succeed"
	...
	rlRun "pki -h $CA_HOST -p $CA_PORT cert-show $valid_pkcs10_serialNumber --encoded > $TmpDir/pki_ocsp_user_cert_add_encoded_002pkcs10.out" 0 "Executing pki cert-show $valid_pkcs10_serialNumber"
        ...
        rlLog "Executing pki -d $CERTDB_DIR/ \
			   ...
			   -h $OCSP_HOST \
                   	   -p $OCSP_PORT \
                            ocsp-user-cert-add $user2 --input $TmpDir/pki_ocsp_user_cert_add_validcert_002pkcs10.pem"
        rlRun "pki -d $CERTDB_DIR/ \
                           -n $(eval echo \$${subsystemId}_adminV_user) \
                           -c $CERTDB_DIR_PASSWORD \
			   -h $OCSP_HOST \
                   	   -p $OCSP_PORT \
                            ocsp-user-cert-add $user2 --input $TmpDir/pki_ocsp_user_cert_add_validcert_002pkcs10.pem  > $TmpDir/pki_ocsp_user_cert_add_useraddcert_002pkcs10.out" \
                            0 \
	...
	rlRun "generate_new_cert tmp_nss_db:$TEMP_NSS_DB tmp_nss_db_pwd:$TEMP_NSS_DB_PASSWD request_type:crmf \
        ...
        rlLog "Executing pki -d $CERTDB_DIR/ \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $user2 --input $TmpDir/pki_ocsp_user_cert_add_validcert_002crmf.pem"
        rlRun "pki -d $CERTDB_DIR/ \
                           -n $(eval echo \$${subsystemId}_adminV_user) \
                           -c $CERTDB_DIR_PASSWORD \
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $user2 --input $TmpDir/pki_ocsp_user_cert_add_validcert_002crmf.pem  > $TmpDir/pki_ocsp_user_cert_add_useraddcert_002crmf.out" \
                            0 \
...
##### Add multiple certs to a user #####
    ...
    rlPhaseStartTest "pki_ocsp_user_cli_ocsp_user_cert-add-003: Add multiple certs to a user should succeed"
        i=0
        rlRun "pki -d $CERTDB_DIR \
			    -n $(eval echo \$${subsystemId}_adminV_user) \
                           -c $CERTDB_DIR_PASSWORD \
		...
		rlRun "generate_new_cert tmp_nss_db:$TEMP_NSS_DB tmp_nss_db_pwd:$TEMP_NSS_DB_PASSWD request_type:pkcs10 \
        	...
        	rlLog "Executing pki -d $CERTDB_DIR/ \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $user1 --input $TmpDir/pki_ocsp_user_cert_add_validcert_003pkcs10$i.pem"
        	rlRun "pki -d $CERTDB_DIR/ \
                           -n $(eval echo \$${subsystemId}_adminV_user) \
                           -c $CERTDB_DIR_PASSWORD \
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $user1 --input $TmpDir/pki_ocsp_user_cert_add_validcert_003pkcs10$i.pem  > $TmpDir/pki_ocsp_user_cert_add_useraddcert_003pkcs10$i.out" \
                            0 \
		...
		rlRun "generate_new_cert tmp_nss_db:$TEMP_NSS_DB tmp_nss_db_pwd:$TEMP_NSS_DB_PASSWD request_type:crmf \
                ...
                rlRun "pki -d $CERTDB_DIR/ \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $user1 --input $TmpDir/pki_ocsp_user_cert_add_validcert_003crmf$i.pem  > $TmpDir/pki_ocsp_user_cert_add_useraddcert_003crmf$i.out 2>&1" \
                            0 \
...
rlPhaseStartTest "pki_ocsp_user_cli_ocsp_user_cert-add-004: Adding expired cert to a user should fail"
        rlRun "pki -d $CERTDB_DIR \
		    -n $(eval echo \$${subsystemId}_adminV_user) \
                    -c $CERTDB_DIR_PASSWORD \
                    -h $OCSP_HOST \
	...
	command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_adminV_user) -c $CERTDB_DIR_PASSWORD -h $OCSP_HOST -p $OCSP_PORT ocsp-user-cert-add $user2 --input $TmpDir/pki_ocsp_user_cert_add_expiredcert_004pkcs10.pem"
	...
	rlLog "Generate cert with validity period of $validityperiod"
        ...
        command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_adminV_user) -c $CERTDB_DIR_PASSWORD -h $OCSP_HOST -p $OCSP_PORT ocsp-user-cert-add $user2 --input $TmpDir/pki_ocsp_user_cert_add_expiredcert_004crmf.pem"
        errmsg="BadRequestException: Certificate expired"
        errorcode=255
        rlRun "verifyErrorMsg \"$command\" \"$errmsg\" \"$errorcode\"" 0 "Verify expected error message - Adding an expired cert to a user should fail"
        rlLog "Set the date back to it's original date & time"
...
rlPhaseStartTest "pki_ocsp_user_cli_ocsp_user_cert-add-005: Add revoked cert to a user should succeed"
        rlRun "generate_new_cert tmp_nss_db:$TEMP_NSS_DB tmp_nss_db_pwd:$TEMP_NSS_DB_PASSWD request_type:pkcs10 \
        algo:rsa key_size:2048 subject_cn:\"$user2fullname\" subject_uid:$user2 subject_email:$user2@example.org \
        organizationalunit:Engineering organization:Example.Inc country:US archive:false req_profile:caUserCert \
        target_host:$CA_HOST protocol: port:$CA_PORT cert_db_dir:$CERTDB_DIR cert_db_pwd:$CERTDB_DIR_PASSWORD \
	...
	rlRun "pki -d $CERTDB_DIR/ \
        ...
        rlLog "Executing pki -d $CERTDB_DIR/ \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $user2 --input $TmpDir/pki_ocsp_user_cert_add_validcert_005pkcs10.pem"
        rlRun "pki -d $CERTDB_DIR/ \
                           -n $(eval echo \$${subsystemId}_adminV_user) \
                           -c $CERTDB_DIR_PASSWORD \
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $user2 --input $TmpDir/pki_ocsp_user_cert_add_validcert_005pkcs10.pem  > $TmpDir/pki_ocsp_user_cert_add_useraddcert_005pkcs10.out" \
                            0 \
                            "PKCS10 Cert is added to the user $user2"
	...
	rlRun "pki -d $CERTDB_DIR/ \
        ...
        rlLog "Executing pki -d $CERTDB_DIR/ \
                           ...
                           -h $OCSP_HOST \
			   -p $OCSP_PORT \
                            ocsp-user-cert-add $user2 --input $TmpDir/pki_ocsp_user_cert_add-CA_validcert_005crmf.pem"
        rlRun "pki -d $CERTDB_DIR/ \
                           -n $(eval echo \$${subsystemId}_adminV_user) \
                           -c $CERTDB_DIR_PASSWORD \
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $user2 --input $TmpDir/pki_ocsp_user_cert_add_validcert_005crmf.pem  > $TmpDir/pki_ocsp_user_cert_add_useraddcert_005crmf.out" \
                            0 \
                            "CRMF Cert is added to the user $user2"
...
rlPhaseStartTest "pki_ocsp_user_cli_ocsp_user_cert-add-006-tier1: Add one cert to a user should fail when USER ID is missing"
	...
	rlRun "generate_new_cert tmp_nss_db:$TEMP_NSS_DB tmp_nss_db_pwd:$TEMP_NSS_DB_PASSWD myreq_type:crmf \
        ...
        command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_adminV_user) -c $CERTDB_DIR_PASSWORD -h $OCSP_HOST -p $OCSP_PORT ocsp-user-cert-add --input $TmpDir/pki_ocsp_user_cert_add_validcert_006pkcs10.pem"
        ...
        command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_adminV_user) -c $CERTDB_DIR_PASSWORD -h $OCSP_HOST -p $OCSP_PORT ocsp-user-cert-add --input $TmpDir/pki_ocsp_user_cert_add_validcert_006crmf.pem"
        errmsg="Error: No User ID specified."
        errorcode=255
        rlRun "verifyErrorMsg \"$command\" \"$errmsg\" \"$errorcode\"" 0 "Verify expected error message - USER ID missing"
rlPhaseEnd
...
rlPhaseStartTest "pki_ocsp_user_cli_ocsp_user_cert-add-007: Add one cert to a user should fail when --input parameter is missing"
        rlRun "pki -d $CERTDB_DIR \
		   -n $(eval echo \$${subsystemId}_adminV_user) \
                   -c $CERTDB_DIR_PASSWORD \
                   -h $OCSP_HOST \
        ...
        command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_adminV_user) -c $CERTDB_DIR_PASSWORD -h $OCSP_HOST -p $OCSP_PORT ocsp-user-cert-add u1"
...
rlPhaseStartTest "pki_ocsp_user_cli_ocsp_user_cert-add-008: Add one cert to a user should fail when argument for the --input param is missing"
        command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_adminV_user) -c $CERTDB_DIR_PASSWORD -h $OCSP_HOST -p $OCSP_PORT  ocsp-user-cert-add $user2 --input"
...
rlPhaseStartTest "pki_ocsp_user_cli_ocsp_user_cert-add-009: Add one cert to a user should fail when the cert is invalid"
	rlRun "generate_new_cert tmp_nss_db:$TEMP_NSS_DB tmp_nss_db_pwd:$TEMP_NSS_DB_PASSWD myreq_type:pkcs10 \
        algo:rsa key_size:2048 subject_cn: subject_uid: subject_email: subject_ou: org: country: archive:false \
        req_profile: target_host:$CA_HOST protocol: port:$CA_PORT cert_db_dir:$CERTDB_DIR cert_db_pwd:$CERTDB_DIR_PASSWORD \
        certdb_nick:\"$ROOTCA_agent_user\" cert_info:$cert_info" 0 "Generate certificate based on pkcs10 request"
        ...
        command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_adminV_user) -c $CERTDB_DIR_PASSWORD -h $OCSP_HOST -p $OCSP_PORT  ocsp-user-cert-add $user2 --input $TmpDir/pki_ocsp_user_cert_add_validcert_009pkcs10.pem"
        ...
        command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_adminV_user) -c $CERTDB_DIR_PASSWORD -h $OCSP_HOST -p $OCSP_PORT ocsp-user-cert-add $user2 --input $TmpDir/pki_ocsp_user_cert_add_validcert_009crmf.pem"
        errmsg="PKIException: Certificate exception"
        errorcode=255
        rlRun "verifyErrorMsg \"$command\" \"$errmsg\" \"$errorcode\"" 0 "Verify expected error message - Invalid Certificate cannot be added to a user"
rlPhaseEnd
...
rlPhaseStartTest "pki_ocsp_user_cli_ocsp_user_cert-add-0010: Add one cert to a user should fail when Input file does not exist "
                command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_adminV_user) -c $CERTDB_DIR_PASSWORD -h $OCSP_HOST -p $OCSP_PORT ocsp-user-cert-add $user2 --input $TmpDir/tempfile.pem"
...
rlPhaseStartTest "pki_ocsp_user_cli_ocsp_user_cert-add-0011: Add one cert to a user - Should be able to add certs with i18n characters in the Subject name of the cert"
        rlRun "generate_new_cert tmp_nss_db:$TEMP_NSS_DB tmp_nss_db_pwd:$TEMP_NSS_DB_PASSWD request_type:pkcs10 \
        ...
        rlLog "Executing pki -d $CERTDB_DIR/ \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $user2 --input $TmpDir/pki_ocsp_user_cert_add_validcert_0011pkcs10.pem"
        rlRun "pki -d $CERTDB_DIR/ \
                           -n $(eval echo \$${subsystemId}_adminV_user) \
                           -c $CERTDB_DIR_PASSWORD \
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $user2 --input $TmpDir/pki_ocsp_user_cert_add_validcert_0011pkcs10.pem  > $TmpDir/pki_ocsp_user_cert_add_useraddcert_0011pkcs10.out" \
                            0 \
                            "PKCS10 Cert is added to the user $user2"
        ...
        rlLog "Executing pki -d $CERTDB_DIR/ \
			   ...
			   -c $CERTDB_DIR_PASSWORD \
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $user2 --input $TmpDir/pki_ocsp_user_cert_add_validcert_0011crmf.pem"
        rlRun "pki -d $CERTDB_DIR/ \
                           -n $(eval echo \$${subsystemId}_adminV_user) \
                           -c $CERTDB_DIR_PASSWORD \
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $user2 --input $TmpDir/pki_ocsp_user_cert_add_validcert_0011crmf.pem  > $TmpDir/pki_ocsp_user_cert_add_useraddcert_0011crmf.out" \
                            0 \
                            "CRMF Cert is added to the user $user2"
...
rlPhaseStartTest "pki_ocsp_user_cli_ocsp_user_cert-add-0012: Add cert to a user of type 'Auditors'"
	...
	rlRun "pki -d $CERTDB_DIR \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-add --fullName=\"$userFullname\" --type=Auditors $userid"
        rlRun "generate_new_cert tmp_nss_db:$TEMP_NSS_DB tmp_nss_db_pwd:$TEMP_NSS_DB_PASSWD request_type:pkcs10 \
        ...
        rlLog "Executing pki -d $CERTDB_DIR/ \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0012pkcs10.pem"
        rlRun "pki -d $CERTDB_DIR/ \
                           -n $(eval echo \$${subsystemId}_adminV_user) \
                           -c $CERTDB_DIR_PASSWORD \
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0012pkcs10.pem  > $TmpDir/pki_ocsp_user_cert_add_useraddcert_0012pkcs10.out" \
                            0 \
        ...
        rlLog "Executing pki -d $CERTDB_DIR/ \
                           ...
                           -h $OCSP_HOST \
			   -p $OCSP_PORT \
                            ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0012crmf.pem"
        rlRun "pki -d $CERTDB_DIR/ \
                           -n $(eval echo \$${subsystemId}_adminV_user) \
                           -c $CERTDB_DIR_PASSWORD \
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0012crmf.pem  > $TmpDir/pki_ocsp_user_cert_add_useraddcert_0012crmf.out" \
                            0 \
        ...
        rlRun "pki -d $CERTDB_DIR \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                           ocsp-user-del $userid"
        rlPhaseEnd

##### Add one cert to a user - User type 'Certificate Manager Agents' #####
rlPhaseStartTest "pki_ocsp_user_cli_ocsp_user_cert-add-0013: Add cert to a user of type 'Certificate Manager Agents'"
        ...
        rlRun "pki -d $CERTDB_DIR \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-add --fullName=\"$userFullname\" --type=\"Certificate Manager Agents\" $userid"
        rlRun "generate_new_cert tmp_nss_db:$TEMP_NSS_DB tmp_nss_db_pwd:$TEMP_NSS_DB_PASSWD request_type:pkcs10 \
        algo:rsa key_size:2048 subject_cn:\"$userFullname\" subject_uid:$userid subject_email:$userid@example.org \
        organizationalunit:Engineering organization:Example.Inc country:US archive:false req_profile:caUserCert \
        target_host:$CA_HOST protocol: port:$CA_PORT cert_db_dir:$CERTDB_DIR cert_db_pwd:$CERTDB_DIR_PASSWORD \
        ...
        rlLog "Executing pki -d $CERTDB_DIR/ \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0013pkcs10.pem"
        rlRun "pki -d $CERTDB_DIR/ \
                           -n $(eval echo \$${subsystemId}_adminV_user) \
                           -c $CERTDB_DIR_PASSWORD \
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0013pkcs10.pem  > $TmpDir/pki_ocsp_user_cert_add_useraddcert_0013pkcs10.out" \
                            0 \
                            "PKCS10 Cert is added to the user $userid"
	...
	rlLog "Executing pki -d $CERTDB_DIR/ \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0013crmf.pem"
        rlRun "pki -d $CERTDB_DIR/ \
                           -n $(eval echo \$${subsystemId}_adminV_user) \
                           -c $CERTDB_DIR_PASSWORD \
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0013crmf.pem  > $TmpDir/pki_ocsp_user_cert_add_useraddcert_0013crmf.out" \
                            0 \
                            "CRMF Cert is added to the user $userid"
...
rlPhaseStartTest "pki_ocsp_user_cli_ocsp_user_cert-add-0014: Add cert to a user of type 'Registration Manager Agents'"
        ...
        rlRun "pki -d $CERTDB_DIR \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-add --fullName=\"$userFullname\" --type=\"Registration Manager Agents\" $userid"
        rlRun "generate_new_cert tmp_nss_db:$TEMP_NSS_DB tmp_nss_db_pwd:$TEMP_NSS_DB_PASSWD request_type:pkcs10 \
        algo:rsa key_size:2048 subject_cn:\"$userFullname\" subject_uid:$userid subject_email:$userid@example.org \
        organizationalunit:Engineering organization:Example.Inc country:US archive:false req_profile:caUserCert \
        target_host:$CA_HOST protocol: port:$CA_PORT cert_db_dir:$CERTDB_DIR cert_db_pwd:$CERTDB_DIR_PASSWORD \
        ...
        rlLog "Executing pki -d $CERTDB_DIR/ \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0014pkcs10.pem"
        rlRun "pki -d $CERTDB_DIR/ \
                           -n $(eval echo \$${subsystemId}_adminV_user) \
                           -c $CERTDB_DIR_PASSWORD \
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0014pkcs10.pem  > $TmpDir/pki_ocsp_user_cert_add_useraddcert_0014pkcs10.out" \
                            0 \
                            "PKCS10 Cert is added to the user $userid"
	...
	rlLog "Executing pki -d $CERTDB_DIR/ \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0014crmf.pem"
        rlRun "pki -d $CERTDB_DIR/ \
                           -n $(eval echo \$${subsystemId}_adminV_user) \
                           -c $CERTDB_DIR_PASSWORD \
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0014crmf.pem  > $TmpDir/pki_ocsp_user_cert_add_useraddcert_0014crmf.out" \
                            0 \
                            "CRMF Cert is added to the user $userid"
...
rlPhaseStartTest "pki_ocsp_user_cli_ocsp_user_cert-add-0015: Add cert to a user of type 'Subsystem Group'"
        ...
        rlRun "pki -d $CERTDB_DIR \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-add --fullName=\"$userFullname\" --type=\"Subsystem Group\" $userid"
        rlRun "generate_new_cert tmp_nss_db:$TEMP_NSS_DB tmp_nss_db_pwd:$TEMP_NSS_DB_PASSWD request_type:pkcs10 \
        algo:rsa key_size:2048 subject_cn:\"$userFullname\" subject_uid:$userid subject_email:$userid@example.org \
        organizationalunit:Engineering organization:Example.Inc country:US archive:false req_profile:caUserCert \
        target_host:$CA_HOST protocol: port:$CA_PORT cert_db_dir:$CERTDB_DIR cert_db_pwd:$CERTDB_DIR_PASSWORD \
        ...
        rlLog "Executing pki -d $CERTDB_DIR/ \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0015pkcs10.pem"
        rlRun "pki -d $CERTDB_DIR/ \
                           -n $(eval echo \$${subsystemId}_adminV_user) \
                           -c $CERTDB_DIR_PASSWORD \
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0015pkcs10.pem  > $TmpDir/pki_ocsp_user_cert_add_useraddcert_0015pkcs10.out" \
                            0 \
                            "PKCS10 Cert is added to the user $userid"
        ...
        rlLog "Executing pki -d $CERTDB_DIR/ \
			-n $(eval echo \$${subsystemId}_adminV_user) \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0015crmf.pem"
        rlRun "pki -d $CERTDB_DIR/ \
                           -n $(eval echo \$${subsystemId}_adminV_user) \
                           -c $CERTDB_DIR_PASSWORD \
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0015crmf.pem  > $TmpDir/pki_ocsp_user_cert_add_useraddcert_0015crmf.out 2>&1" \
                            0 \
                            "CRMF Cert is added to the user $userid"
...
rlPhaseStartTest "pki_ocsp_user_cli_ocsp_user_cert-add-0016: Add cert to a user of type 'Security Domain Administrators'"
        ...
        rlRun "pki -d $CERTDB_DIR \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-add --fullName=\"$userFullname\" --type=\"Security Domain Administrators\" $userid"
        rlRun "generate_new_cert tmp_nss_db:$TEMP_NSS_DB tmp_nss_db_pwd:$TEMP_NSS_DB_PASSWD request_type:pkcs10 \
        algo:rsa key_size:2048 subject_cn:\"$userFullname\" subject_uid:$userid subject_email:$userid@example.org \
        organizationalunit:Engineering organization:Example.Inc country:US archive:false req_profile:caUserCert \
        target_host:$CA_HOST protocol: port:$CA_PORT cert_db_dir:$CERTDB_DIR cert_db_pwd:$CERTDB_DIR_PASSWORD \
        ...
        rlLog "Executing pki -d $CERTDB_DIR/ \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0016pkcs10.pem"
        rlRun "pki -d $CERTDB_DIR/ \
                           -n $(eval echo \$${subsystemId}_adminV_user) \
                           -c $CERTDB_DIR_PASSWORD \
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0016pkcs10.pem  > $TmpDir/pki_ocsp_user_cert_add_useraddcert_0016pkcs10.out" \
                            0 \
                            "PKCS10 Cert is added to the user $userid"
	...
	rlLog "Executing pki -d $CERTDB_DIR/ \
                        -n $(eval echo \$${subsystemId}_adminV_user) \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0016crmf.pem"
        rlRun "pki -d $CERTDB_DIR/ \
                           -n $(eval echo \$${subsystemId}_adminV_user) \
                           -c $CERTDB_DIR_PASSWORD \
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0016crmf.pem  > $TmpDir/pki_ocsp_user_cert_add_useraddcert_0016crmf.out" \
                            0 \
                            "CRMF Cert is added to the user $userid"
...
rlPhaseStartTest "pki_ocsp_user_cli_ocsp_user_cert-add-0017: Add cert to a user of type 'ClonedSubsystems'"
        ...
        rlRun "pki -d $CERTDB_DIR \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-add --fullName=\"$userFullname\" --type=\"ClonedSubsystems\" $userid"
        rlRun "generate_new_cert tmp_nss_db:$TEMP_NSS_DB tmp_nss_db_pwd:$TEMP_NSS_DB_PASSWD request_type:pkcs10 \
        algo:rsa key_size:2048 subject_cn:\"$userFullname\" subject_uid:$userid subject_email:$userid@example.org \
        organizationalunit:Engineering organization:Example.Inc country:US archive:false req_profile:caUserCert \
        target_host:$CA_HOST protocol: port:$CA_PORT cert_db_dir:$CERTDB_DIR cert_db_pwd:$CERTDB_DIR_PASSWORD \
        ...
        rlLog "Executing pki -d $CERTDB_DIR/ \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0017pkcs10.pem"
        rlRun "pki -d $CERTDB_DIR/ \
                           -n $(eval echo \$${subsystemId}_adminV_user) \
                           -c $CERTDB_DIR_PASSWORD \
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0017pkcs10.pem  > $TmpDir/pki_ocsp_user_cert_add_useraddcert_0017pkcs10.out" \
                            0 \
                            "PKCS10 Cert is added to the user $userid"
        ...
        rlLog "Executing pki -d $CERTDB_DIR/ \
			-n $(eval echo \$${subsystemId}_adminV_user) \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0017crmf.pem"
        rlRun "pki -d $CERTDB_DIR/ \
                           -n $(eval echo \$${subsystemId}_adminV_user) \
                           -c $CERTDB_DIR_PASSWORD \
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0017crmf.pem  > $TmpDir/pki_ocsp_user_cert_add_useraddcert_0017crmf.out" \
                            0 \
                            "CRMF Cert is added to the user $userid"
...
rlPhaseStartTest "pki_ocsp_user_cli_ocsp_user_cert-add-0018: Add cert to a user of type 'Trusted Managers'"
        ...
        rlRun "pki -d $CERTDB_DIR \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-add --fullName=\"$userFullname\" --type=\"Trusted Managers\" $userid"
        rlRun "generate_new_cert tmp_nss_db:$TEMP_NSS_DB tmp_nss_db_pwd:$TEMP_NSS_DB_PASSWD request_type:pkcs10 \
        algo:rsa key_size:2048 subject_cn:\"$userFullname\" subject_uid:$userid subject_email:$userid@example.org \
        organizationalunit:Engineering organization:Example.Inc country:US archive:false req_profile:caUserCert \
        target_host:$CA_HOST protocol: port:$CA_PORT cert_db_dir:$CERTDB_DIR cert_db_pwd:$CERTDB_DIR_PASSWORD \
        ...
        rlLog "Executing pki -d $CERTDB_DIR/ \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0018pkcs10.pem"
        rlRun "pki -d $CERTDB_DIR/ \
                           -n $(eval echo \$${subsystemId}_adminV_user) \
                           -c $CERTDB_DIR_PASSWORD \
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0018pkcs10.pem  > $TmpDir/pki_ocsp_user_cert_add_useraddcert_0018pkcs10.out" \
                            0 \
                            "PKCS10 Cert is added to the user $userid"
        ...
        rlLog "Executing pki -d $CERTDB_DIR/ \
			 -n $(eval echo \$${subsystemId}_adminV_user) \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                           ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0018crmf.pem"
        rlRun "pki -d $CERTDB_DIR/ \
                           -n $(eval echo \$${subsystemId}_adminV_user) \
                           -c $CERTDB_DIR_PASSWORD \
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0018crmf.pem  > $TmpDir/pki_ocsp_user_cert_add_useraddcert_0018crmf.out" \
                            0 \
                            "CRMF Cert is added to the user $userid"
...
rlPhaseStartTest "pki_ocsp_user_cli_ocsp_user_cert-add-0019: Add an Admin user \"admin_user\", add a cert to admin_user, add a new user as admin_user"
        ...
        rlRun "pki -d $CERTDB_DIR \
			    -n $(eval echo \$${subsystemId}_adminV_user) \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                           ocsp-group-member-add Administrators admin_user > $TmpDir/pki-ocsp-user-add-group0019.out"
        ...
        rlRun "pki -d $CERTDB_DIR \
			    -n $(eval echo \$${subsystemId}_adminV_user) \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-group-member-add Administrators admin_user1 > $TmpDir/pki-ocsp-user-add-group00191.out"
	...
	rlLog "Executing pki -d $CERTDB_DIR/ \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add admin_user --input $TmpDir/pki_ocsp_user_cert_add_validcert_0019pkcs10.pem"
        rlRun "pki -d $CERTDB_DIR/ \
                           -n $(eval echo \$${subsystemId}_adminV_user) \
                           -c $CERTDB_DIR_PASSWORD \
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add admin_user --input $TmpDir/pki_ocsp_user_cert_add_validcert_0019pkcs10.pem  > $TmpDir/pki_ocsp_user_cert_add_useraddcert_0019pkcs10.out" \
                            0 \
	...
	rlLog "pki -d $TEMP_NSS_DB/ \
        ...
        rlRun "pki -d $TEMP_NSS_DB/ \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-add --fullName=\"New Test User1\" new_test_user1 > $TmpDir/pki_ocsp_user_cert_add_useradd_0019.out 2>&1" \
                            0 \
...
rlPhaseStartTest "pki_ocsp_user_cli_ocsp_user_cert-add-OCSP-0020: Adding a cert as a OCSP agent user should fail"
        ...
        command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_agentV_user) -c $CERTDB_DIR_PASSWORD -h $OCSP_HOST -p $OCSP_PORT ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0021pkcs10.pem"
        ...
        command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_agentV_user) -c $CERTDB_DIR_PASSWORD -h $OCSP_HOST -p $OCSP_PORT  ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0021crmf.pem"
        errmsg="ForbiddenException: Authorization Error"
        errorcode=255
...
rlPhaseStartTest "pki_ocsp_user_cli_ocsp_user_cert-add-0021: Adding a cert as valid OCSP auditor user should fail"
	...
	command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_auditV_user) -c $CERTDB_DIR_PASSWORD -h $OCSP_HOST -p $OCSP_PORT ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0022pkcs10.pem"
        errmsg="ForbiddenException: Authorization Error"
        errorcode=255
        ...
        command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_auditV_user) -c $CERTDB_DIR_PASSWORD -h $OCSP_HOST -p $OCSP_PORT ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0022crmf.pem"
        errmsg="ForbiddenException: Authorization Error"
        errorcode=255
...
rlPhaseStartTest "pki_ocsp_user_cli_ocsp_user_cert-add-0022: Adding a cert as OCSP_adminE should fail"
	...
	rlRun "date --set='next day'" 0 "Set System date a day ahead"
        ...
        command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_adminE_user) -c $CERTDB_DIR_PASSWORD -h $OCSP_HOST -p $OCSP_PORT ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0023pkcs10.pem"
        errmsg="ForbiddenException: Authorization Error"
        errorcode=255
        ...
        command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_adminE_user) -c $CERTDB_DIR_PASSWORD -h $OCSP_HOST -p $OCSP_PORT ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0023crmf.pem"
        errmsg="ForbiddenException: Authorization Error"
        errorcode=255
...
rlPhaseStartTest "pki_ocsp_user_cli_ocsp_user_cert-add-0023: Adding a cert as an admin user with revoked cert should fail"
	...
	command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_adminR_user) -c $CERTDB_DIR_PASSWORD -h $OCSP_HOST -p $OCSP_PORT  ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0024pkcs10.pem"
        ...
        command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_adminR_user) -c $CERTDB_DIR_PASSWORD -h $OCSP_HOST -p $OCSP_PORT ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0024crmf.pem"
        errmsg="PKIException: Unauthorized"
        errorcode=255
        rlRun "verifyErrorMsg \"$command\" \"$errmsg\" \"$errorcode\"" 0 "Verify expected error message - Adding cert to a user as admin user with revoked cert"
...
rlPhaseStartTest "pki_ocsp_user_cli_ocsp_user_cert-add-0024: Adding a cert as an agent user with revoked cert should fail"
	local userid="new_user5"
        ...
        command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_agentR_user) -c $CERTDB_DIR_PASSWORD -h $OCSP_HOST -p $OCSP_PORT ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0025pkcs10.pem"
        errmsg="PKIException: Unauthorized"
        errorcode=255
        ...
        command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_agentR_user) -c $CERTDB_DIR_PASSWORD -h $OCSP_HOST -p $OCSP_PORT ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0025crmf.pem"
        errmsg="PKIException: Unauthorized"
        errorcode=255
        rlRun "verifyErrorMsg \"$command\" \"$errmsg\" \"$errorcode\"" 0 "Verify expected error message - Adding cert to a user as an agent user with revoked cert"
	rlLog "PKI Ticket: https://fedorahosted.org/pki/ticket/1134"
...
rlPhaseStartTest "pki_ocsp_user_cli_ocsp_user_cert-add-0025: Adding a cert as agent user with expired cert should fail"
	local userid="new_user6"
        ...
        command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_agentE_user) -c $CERTDB_DIR_PASSWORD -h $OCSP_HOST -p $OCSP_PORT ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0026pkcs10.pem"
        errmsg="ForbiddenException: Authorization Error"
        errorcode=255
        ...
        command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_agentE_user) -c $CERTDB_DIR_PASSWORD -h $OCSP_HOST -p $OCSP_PORT  ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0026crmf.pem"
        errmsg="ForbiddenException: Authorization Error"
        errorcode=255
        rlRun "verifyErrorMsg \"$command\" \"$errmsg\" \"$errorcode\"" 0 "Verify expected error message - Adding cert to a user as an agent user with expired cert"
...
rlPhaseStartTest "pki_ocsp_user_cli_ocsp_user_cert-add-0026: Adding a cert as role_user_UTCA should fail"
	local userid="new_user7"
        ...
        command="pki -d $UNTRUSTED_CERT_DB_LOCATION -n role_user_UTCA -c $UNTRUSTED_CERT_DB_PASSWORD -h $OCSP_HOST -p $OCSP_PORT ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0027pkcs10.pem"
        errmsg="PKIException: Unauthorized"
        errorcode=255
        rlRun "verifyErrorMsg \"$command\" \"$errmsg\" \"$errorcode\"" 0 "Verify expected error message - Adding cert to a user as OCSP_adminUTCA"

        command="pki -d $UNTRUSTED_CERT_DB_LOCATION -n role_user_UTCA -c $UNTRUSTED_CERT_DB_PASSWORD -h $OCSP_HOST -p $OCSP_PORT  ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0027crmf.pem"
        errmsg="PKIException: Unauthorized"
        errorcode=255
        rlRun "verifyErrorMsg \"$command\" \"$errmsg\" \"$errorcode\"" 0 "Verify expected error message - Adding cert to a user as OCSP_adminUTCA"
...
rlPhaseStartTest "pki_ocsp_user_cli_ocsp_user_cert-add-0027: Adding a cert as OCSP_agentUTCA should fail"
        ...
        command="pki -d $UNTRUSTED_CERT_DB_LOCATION -n role_user_UTCA -c $UNTRUSTED_CERT_DB_PASSWORD -h $OCSP_HOST -p $OCSP_PORT  ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0028pkcs10.pem"
        errmsg="PKIException: Unauthorized"
        errorcode=255
        rlRun "verifyErrorMsg \"$command\" \"$errmsg\" \"$errorcode\"" 0 "Verify expected error message - Adding cert to a user as OCSP_agentUTCA"

        command="pki -d $UNTRUSTED_CERT_DB_LOCATION -n role_user_UTCA -c $UNTRUSTED_CERT_DB_PASSWORD -h $OCSP_HOST -p $OCSP_PORT  ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0028crmf.pem"
        errmsg="PKIException: Unauthorized"
        errorcode=255
...
rlPhaseStartTest "pki_ocsp_user_cli_ocsp_user_cert-add-0028: Adding a cert as OCSP_operatorV should fail"
	local userid="new_user8"
        ...
        command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_operatorV_user) -c $CERTDB_DIR_PASSWORD -h $OCSP_HOST -p $OCSP_PORT  ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0029pkcs10.pem"
        errmsg="ForbiddenException: Authorization Error"
        errorcode=255
        rlRun "verifyErrorMsg \"$command\" \"$errmsg\" \"$errorcode\"" 0 "Verify expected error message - Adding cert to a user as OCSP_operatorV"

        command="pki -d $CERTDB_DIR -n $(eval echo \$${subsystemId}_operatorV_user) -c $CERTDB_DIR_PASSWORD -h $OCSP_HOST -p $OCSP_PORT  ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0029crmf.pem"
        errmsg="ForbiddenException: Authorization Error"
        errorcode=255
        rlRun "verifyErrorMsg \"$command\" \"$errmsg\" \"$errorcode\"" 0 "Verify expected error message - Adding cert to a user as OCSP_operatorV"
...
rlPhaseStartTest "pki_ocsp_user_cli_ocsp_user_cert-add-0029: Adding a cert as user not associated with an group, should fail"
	...
	command="pki -d $CERTDB_DIR -n $userid -c $CERTDB_DIR_PASSWORD -h $OCSP_HOST -p $OCSP_PORT ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0030pkcs10.pem"
        errmsg="ForbiddenException: Authorization Error"
        errorcode=255
        ...
        command="pki -d $CERTDB_DIR -n $userid -c $CERTDB_DIR_PASSWORD -h $OCSP_HOST -p $OCSP_PORT ocsp-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add_validcert_0030crmf.pem"
        errmsg="ForbiddenException: Authorization Error"
        errorcode=255
        rlRun "verifyErrorMsg \"$command\" \"$errmsg\" \"$errorcode\"" 0 "Verify expected error message -  Adding cert to $userid as a user not associated with any group"
...
rlPhaseStartTest "pki_ocsp_user_cli_ocsp_user_cert-add-0030: Add one cert to a user - switching position of options should succeed"
	rlRun "generate_new_cert tmp_nss_db:$TEMP_NSS_DB tmp_nss_db_pwd:$TEMP_NSS_DB_PASSWD request_type:pkcs10 \
        algo:rsa key_size:2048 subject_cn:\"$user2fullname\" subject_uid:$user2 subject_email:$user2@example.org \
        organizationalunit:Engineering organization:Example.Inc country:US archive:false req_profile:caUserCert \
        target_host:$CA_HOST protocol: port:$CA_PORT cert_db_dir:$CERTDB_DIR cert_db_pwd:$CERTDB_DIR_PASSWORD \
        ...
        rlLog "Executing pki -d $CERTDB_DIR/ \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add --input $TmpDir/pki_ocsp_user_cert_add_validcert_0031pkcs10.pem $user2"
        rlRun "pki -d $CERTDB_DIR/ \
                           -n $(eval echo \$${subsystemId}_adminV_user) \
                           -c $CERTDB_DIR_PASSWORD \
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add --input $TmpDir/pki_ocsp_user_cert_add_validcert_0031pkcs10.pem $user2 > $TmpDir/pki_ocsp_user_cert_add_useraddcert_0031pkcs10.out" \
                            0 \
                            "PKCS10 Cert is added to the user $user2"
        ...
        rlLog "Executing pki -d $CERTDB_DIR/ \
			   ...
			   -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add --input $TmpDir/pki_ocsp_user_cert_add_validcert_0031crmf.pem $user2"
        rlRun "pki -d $CERTDB_DIR/ \
                           -n $(eval echo \$${subsystemId}_adminV_user) \
                           -c $CERTDB_DIR_PASSWORD \
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add --input $TmpDir/pki_ocsp_user_cert_add_validcert_0031crmf.pem $user2 > $TmpDir/pki_ocsp_user_cert_add_useraddcert_0031crmf.out" \
                            0 \
                            "CRMF Cert is added to the user $user2"
...
rlPhaseStartTest "pki_ocsp_user_cli_ocsp_user_cert-add-0031: Add one cert to a user with --serial option hex"
        ...
        rlRun "pki -d $CERTDB_DIR \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-add --fullName=\"$username\" $userid"
        rlRun "generate_new_cert tmp_nss_db:$TEMP_NSS_DB tmp_nss_db_pwd:$TEMP_NSS_DB_PASSWD request_type:pkcs10 \
        ...
        rlLog "Executing pki -d $CERTDB_DIR/ \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $userid --serial=$valid_pkcs10_serialNumber"
        rlRun "pki -d $CERTDB_DIR/ \
                           -n $(eval echo \$${subsystemId}_adminV_user) \
                           -c $CERTDB_DIR_PASSWORD \
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $userid --serial=$valid_pkcs10_serialNumber  > $TmpDir/pki_ocsp_user_cert_add_useraddcert_0032pkcs10.out" \
                            0 \
                            "PKCS10 Cert is added to the user $userid"
        ...
        rlLog "Executing pki -d $CERTDB_DIR/ \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                           ocsp-user-cert-add $userid --serial=$valid_crmf_serialNumber"
        rlRun "pki -d $CERTDB_DIR/ \
                           -n $(eval echo \$${subsystemId}_adminV_user) \
                           -c $CERTDB_DIR_PASSWORD \
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $userid --serial=$valid_crmf_serialNumber > $TmpDir/pki_ocsp_user_cert_add_useraddcert_0032crmf.out" \
                            0 \
                            "CRMF Cert is added to the user $userid"
...
rlPhaseStartTest "pki_ocsp_user_cli_ocsp_user_cert-add-0032: Add one cert to a user with --serial option decimal"
        local userid="testuser4"
        local username="Test User4"
        rlRun "pki -d $CERTDB_DIR \
                           -n $(eval echo \$${subsystemId}_adminV_user) \
        ...
        rlLog "Executing pki -d $CERTDB_DIR/ \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $userid --serial=$valid_decimal_pkcs10_serialNumber"
        rlRun "pki -d $CERTDB_DIR/ \
                           -n $(eval echo \$${subsystemId}_adminV_user) \
                           -c $CERTDB_DIR_PASSWORD \
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $userid --serial=$valid_decimal_pkcs10_serialNumber  > $TmpDir/pki_ocsp_user_cert_add_useraddcert_0033pkcs10.out" \
                            0 \
                            "PKCS10 Cert is added to the user $userid"
        ...
        rlLog "Executing pki -d $CERTDB_DIR/ \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $userid --serial=$valid_decimal_crmf_serialNumber"
        rlRun "pki -d $CERTDB_DIR/ \
                           -n $(eval echo \$${subsystemId}_adminV_user) \
                           -c $CERTDB_DIR_PASSWORD \
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $userid --serial=$valid_decimal_crmf_serialNumber > $TmpDir/pki_ocsp_user_cert_add_useraddcert_0033crmf.out" \
                            0 \
                            "CRMF Cert is added to the user $userid"
...
rlPhaseStartTest "pki_ocsp_user_cli_ocsp_user_cert-add-0033: Add one cert to a user with --serial and --input options should fail"
        local userid="testuser4"
        local username="Test User4"
        rlRun "pki -d $CERTDB_DIR \
                           -n $(eval echo \$${subsystemId}_adminV_user) \
        ...
        rlLog "Executing pki -d $CERTDB_DIR/ \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $userid --serial=$valid_decimal_pkcs10_serialNumber --input=$TmpDir/pki_ocsp_user_cert_add_validcert_0034pkcs10.pem"
        ...
        rlLog "Executing pki -d $CERTDB_DIR/ \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                            ocsp-user-cert-add $userid --serial=$valid_decimal_crmf_serialNumber --input=$TmpDir/pki_ocsp_user_cert_add_validcert_0034crmf.pem"
...
rlPhaseStartTest "pki_ocsp_user_cli_ocsp_user_cert-add-0034: Add one cert to a user with negative serial should fail"
        local userid="testuser4"
        local username="Test User4"
        ...
        rlRun "pki -d $CERTDB_DIR \
                           ...
                           -p $OCSP_PORT \
                            ocsp-user-add --fullName=\"$username\" $userid"
        command="pki -d $CERTDB_DIR/ -n $(eval echo \$${subsystemId}_adminV_user) -c $CERTDB_DIR_PASSWORD -h $OCSP_HOST -p $OCSP_PORT ocsp-user-cert-add $userid --serial=-100"
...
rlPhaseStartTest "pki_ocsp_user_cli_ocsp_user_cert-add-0035: Add one cert to a user with missing argument for --serial"
        local userid="testuser4"
        local username="Test User4"
        rlRun "pki -d $CERTDB_DIR \
                           -n $(eval echo \$${subsystemId}_adminV_user) \
                           ...
                           -p $OCSP_PORT \
                           ocsp-user-add --fullName=\"$username\" $userid"
        command="pki -d $CERTDB_DIR/ -n $(eval echo \$${subsystemId}_adminV_user) -c $CERTDB_DIR_PASSWORD -h $OCSP_HOST -p $OCSP_PORT ocsp-user-cert-add $userid --serial"
...
rlPhaseStartTest "pki_ocsp_user_cli_ocsp_user_cert-add-0036: Add one cert to a user with character passed as argument to --serial"
        local userid="testuser4"
        local username="Test User4"
        rlRun "pki -d $CERTDB_DIR \
                           -n $(eval echo \$${subsystemId}_adminV_user) \
                           ...
                           -p $OCSP_PORT \
                            ocsp-user-add --fullName=\"$username\" $userid"
        command="pki -d $CERTDB_DIR/ -n $(eval echo \$${subsystemId}_adminV_user) -c $CERTDB_DIR_PASSWORD -h $OCSP_HOST -p $OCSP_PORT ocsp-user-cert-add $userid --serial='abc'"
        ...
        rlRun "pki -d $CERTDB_DIR \
                           ...
                           ocsp-user-del $userid"
rlPhaseEnd
#rlPhaseStartTest "pki_ca_user_cli_ocsp_user_cert-add-0038: client cert authentication using cross certification"
...
#        rlRun "sleep 10"
#        rlLog "Executing: pki -d $NEWCA_CERTDB_DIR -n \"PKI Administrator for $ROOTCA_DOMAIN\" -c $NEWCA_CERTDB_DIR_PASSWORD -h $CA_HOST -t $SUBSYSTEM_TYPE -p $NEWCA_HTTP_PORT ocsp-user-add --fullName=\"$username\" $userid"
...
#                          -t $SUBSYSTEM_TYPE \
#                          -p $NEWCA_HTTP_PORT \
#                           ocsp-user-add --fullName=\"$username\" $userid > $TmpDir/newcanewuser.out 2>&1" 0 "Added a user to new CA"
#
#        rlRun "pki -d $NEWCA_CERTDB_DIR \
#                           -n \"PKI Administrator for $ROOTCA_DOMAIN\" \
#                           -c $NEWCA_CERTDB_DIR_PASSWORD \
...
#                           -t $SUBSYSTEM_TYPE \
#                           -p $NEWCA_HTTP_PORT \
#                            ocsp-group-member-add Administrators $userid > $TmpDir/pki-ocsp-user-add-newca-group001.out 2>&1"  \
#                            0 \
...
#        local valid_pkcs10_serialNumber=$(cat $cert_info| grep cert_serialNumber | cut -d- -f2)
#        local valid_decimal_pkcs10_serialNumber=$(cat $cert_info| grep decimal_valid_serialNumber | cut -d- -f2)
#        rlRun "pki -h $CA_HOST -p $CA_PORT cert-show $valid_pkcs10_serialNumber --encoded > $TmpDir/pki_ocsp_user_cert_add-CA_encoded_0038pkcs10.out" 0 "Executing pki cert-show $valid_pkcs10_serialNumber"
#        rlRun "sed -n '/-----BEGIN CERTIFICATE-----/,/-----END CERTIFICATE-----/p' $TmpDir/pki_ocsp_user_cert_add-CA_encoded_0038pkcs10.out > $TmpDir/pki_ocsp_user_cert_add-CA_validcert_0038pkcs10.pem"
...
#                           -t $SUBSYSTEM_TYPE \
#                           -p $NEWCA_HTTP_PORT \
#                          ca-user-cert-add $userid --input $TmpDir/pki_ocsp_user_cert_add-CA_validcert_0038pkcs10.pem > $TmpDir/pki-ca_ocsp-user-cert-add-newca.out 2>&1"  \
#                            0 \
#                            "Added cert to user $userid"

#        rlRun "certutil -d $NEWCA_CERTDB_DIR -A -n \"$userid\" -i $TmpDir/pki_ocsp_user_cert_add-CA_validcert_0038pkcs10.pem -t "u,u,u""
#        rlRun "sleep 10"
#        rlRun "certutil -d $CERTDB_DIR -A -n \"$userid\" -i $TmpDir/pki_ocsp_user_cert_add-CA_validcert_0038pkcs10.pem -t "u,u,u""
#        rlRun "sleep 10"
...
#                          -t $SUBSYSTEM_TYPE \
#                          -p $NEWCA_HTTP_PORT \
#                           ocsp-user-add --fullName=\"New Test User\" new_test_user > /tmp/newcanewuser.out 2>&1" 0 "Added a user to new CA"
...
rlPhaseStartCleanup "pki_ocsp_user_cli_user_cleanup: Deleting role users"
        ...
        while [ $j -lt 11 ] ; do
			  ...
			  -n $(eval echo \$${subsystemId}_adminV_user) \
                           ...
                           -h $OCSP_HOST \
                           -p $OCSP_PORT \
                           ocsp-user-del  $usr > $TmpDir/pki-ocsp-user-del-ocsp-new-user-00$j.out" \
                           0 \
                           "Deleted user $usr"
                rlAssertGrep "Deleted user \"$usr\"" "$TmpDir/pki-ocsp-user-del-ocsp-new-user-00$j.out"
                let j=$j+1
        done
