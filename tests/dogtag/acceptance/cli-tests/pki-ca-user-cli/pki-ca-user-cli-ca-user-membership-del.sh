...
run_pki-ca-user-cli-ca-user-membership-del_tests(){
  ...
  if [ "$ca_instance_created" = "TRUE" ] ;  then
	...
	rlPhaseStartTest "pki_ca_user_cli_ca_user_membership-del-008: Should not be able to ca-user-membership-del using a revoked cert CA_adminR"
                command="pki -d $CERTDB_DIR -h $SUBSYSTEM_HOST -p $(eval echo \$${subsystemId}_UNSECURE_PORT) -n ${prefix}_adminR -c $CERTDB_DIR_PASSWORD ca-user-membership-del user2 \"Administrators\""
	...
	rlPhaseStartTest "pki_ca_user_cli_ca_user_membership-del-009:  Should not be able to ca-user-membership-del using an agent with revoked cert CA_agentR"
		command="pki -d $CERTDB_DIR -h $SUBSYSTEM_HOST -p $(eval echo \$${subsystemId}_UNSECURE_PORT) -n ${prefix}_agentR -c $CERTDB_DIR_PASSWORD ca-user-membership-del user2 \"Administrators\""
		rlLog "Executing $command"
                errmsg="PKIException: Unauthorized"
                errorcode=255
	...
	rlPhaseStartTest "pki_ca_user_cli_ca_user_membership-del-010: Should not be able to ca-user-membership-del using a valid agent CA_agentV user"
		command="pki -d $CERTDB_DIR -h $SUBSYSTEM_HOST -p $(eval echo \$${subsystemId}_UNSECURE_PORT) -n ${prefix}_agentV -c $CERTDB_DIR_PASSWORD ca-user-membership-del user2 \"Administrators\""
		rlLog "Executing $command"
	...
	rlPhaseStartTest "pki_ca_user_cli_ca_user_membership-del-011: Should not be able to ca-user-membership-del using admin user with expired cert CA_adminE"
		rlRun "date --set='+2 days'" 0 "Set System date 2 days ahead"
                rlRun "date"
                command="pki -d $CERTDB_DIR -h $SUBSYSTEM_HOST -p $(eval echo \$${subsystemId}_UNSECURE_PORT) -n ${prefix}_adminE -c $CERTDB_DIR_PASSWORD ca-user-membership-del user2 \"Administrators\""
		rlLog "Executing $command"
	...
	rlPhaseStartTest "pki_ca_user_cli_ca_user_membership-del-012: Should not be able to ca-user-membership-del using CA_agentE cert"
		rlRun "date --set='+2 days'" 0 "Set System date 2 days ahead"
                rlRun "date"
                command="pki -d $CERTDB_DIR -h $SUBSYSTEM_HOST -p $(eval echo \$${subsystemId}_UNSECURE_PORT) -n ${prefix}_agentE -c $CERTDB_DIR_PASSWORD ca-user-membership-del user2 \"Administrators\""
		rlLog "Executing $command"
                errmsg="ProcessingException: Unable to invoke request"
                errorcode=255
	...
	rlPhaseStartTest "pki_ca_user_cli_ca_user_membership-del-013: Should not be able to ca-user-membership-del using CA_auditV cert"
                command="pki -d $CERTDB_DIR -h $SUBSYSTEM_HOST -p $(eval echo \$${subsystemId}_UNSECURE_PORT) -n ${prefix}_auditV -c $CERTDB_DIR_PASSWORD ca-user-membership-del user2 \"Administrators\""
		rlLog "Executing $command"
                errmsg="ForbiddenException: Authorization Error"
                errorcode=255
	...
	rlPhaseStartTest "pki_ca_user_cli_ca_user_membership-del-014: Should not be able to ca-user-membership-del using CA_operatorV cert"
		command="pki -d $CERTDB_DIR -h $SUBSYSTEM_HOST -p $(eval echo \$${subsystemId}_UNSECURE_PORT) -n ${prefix}_operatorV -c $CERTDB_DIR_PASSWORD ca-user-membership-del user2 \"Administrators\""
		rlLog "Executing $command"
                errmsg="ForbiddenException: Authorization Error"
                errorcode=255
	...
	rlPhaseStartTest "pki_ca_user_cli_ca_user_membership-del-016: Delete user-membership for user fullname with i18n characters"
                rlLog "ca-user-add user fullname Éric Têko with i18n characters"
                rlLog "pki -d $CERTDB_DIR \
                           -n ${prefix}_adminV \
                           -c $CERTDB_DIR_PASSWORD \
			   -h $SUBSYSTEM_HOST \
			   ...
			   -h $SUBSYSTEM_HOST \
                           -p $(eval echo \$${subsystemId}_UNSECURE_PORT) \
                            ca-user-add --fullName='Éric Têko' u16" \
                            0 \
                            "Adding user with fullname ÉricTêko with i18n characters"
			   ...
			   -h $SUBSYSTEM_HOST \
                           -p $(eval echo \$${subsystemId}_UNSECURE_PORT) \
                            ca-group-add 'dadministʁasjɔ̃' --description \"Admininstartors in French\" 2>&1 > $TmpDir/pki-ca-user-membership-add-groupadd-ca-017_1.out" \
                            0 \
			   ...
			   -h $SUBSYSTEM_HOST \
                           -p $(eval echo \$${subsystemId}_UNSECURE_PORT) \
                            ca-user-membership-add u16 \"dadministʁasjɔ̃\" > $TmpDir/pki-ca-user-membership-del-groupadd-ca-017_2.out" \
                            0 \
		...
		rlLog "Delete user-membership from the group"
			   ...
			   -h $SUBSYSTEM_HOST \
                           -p $(eval echo \$${subsystemId}_UNSECURE_PORT) \
                            ca-user-membership-del u16 'dadministʁasjɔ̃' > $TmpDir/pki-ca-user-ca-membership-del-017_3.out" \
                            0 \
		...
		rlLog "Check if the user is removed from the group"
			   ...
			   -h $SUBSYSTEM_HOST \
                           -p $(eval echo \$${subsystemId}_UNSECURE_PORT) \
                            user-membership-find u16 > $TmpDir/pki-ca-user-membership-find-groupadd-find-ca-017_4.out" \
                            0 \
	...
	rlPhaseStartTest "pki_ca_user_cli_ca_user_membership-del-017: Delete user-membership for user fullname with i18n characters"
                rlLog "ca-user-add user fullname ÖrjanÄke with i18n characters"
                rlRun "pki -d $CERTDB_DIR \
                           -n ${prefix}_adminV \
                           -c $CERTDB_DIR_PASSWORD \
			   -h $SUBSYSTEM_HOST \
                           -p $(eval echo \$${subsystemId}_UNSECURE_PORT) \
                            ca-user-add --fullName='ÖrjanÄke' u17 > $TmpDir/pki-ca-user-add-ca-018.out 2>&1" \
                            0 \
                            "Adding user fullname ÖrjanÄke with i18n characters"
                rlAssertGrep "Added user \"u17\"" "$TmpDir/pki-ca-user-add-ca-018.out"
                rlAssertGrep "User ID: u17" "$TmpDir/pki-ca-user-add-ca-018.out"
                rlLog "pki -d $CERTDB_DIR \
                           -n ${prefix}_adminV \
                           -c $CERTDB_DIR_PASSWORD \
			   -h $SUBSYSTEM_HOST \
                           -p $(eval echo \$${subsystemId}_UNSECURE_PORT) \
                            ca-user-membership-add u17 \"dadministʁasjɔ̃\""
                rlRun "pki -d $CERTDB_DIR \
                           -n ${prefix}_adminV \
                           -c $CERTDB_DIR_PASSWORD \
			   -h $SUBSYSTEM_HOST \
                           -p $(eval echo \$${subsystemId}_UNSECURE_PORT) \
                            ca-user-membership-add u17 \"dadministʁasjɔ̃\" > $TmpDir/pki-ca-user-membership-del-groupadd-ca-018_2.out" \
                            0 \
		...
		rlLog "Delete user from the group"
			   ...
			   -h $SUBSYSTEM_HOST \
                           -p $(eval echo \$${subsystemId}_UNSECURE_PORT) \
                            ca-user-membership-del u17 \"dadministʁasjɔ̃\" > $TmpDir/pki-ca-user-membership-del-groupadd-del-ca-018_3.out" \
                            0 \
		...
		rlAssertGrep "Deleted membership in group \"dadministʁasjɔ̃\"" "$TmpDir/pki-ca-user-membership-del-groupadd-del-ca-018_3.out"
			   ...
			   -h $SUBSYSTEM_HOST \
                           -p $(eval echo \$${subsystemId}_UNSECURE_PORT) \
                            ca-user-membership-find u17 > $TmpDir/pki-ca-user-membership-del-groupadd-del-ca-018_4.out" \
                            0 \
                            "Find user-membership with group \"dadministʁasjɔ̃\""
                rlAssertGrep "0 entries matched" "$TmpDir/pki-ca-user-membership-del-groupadd-del-ca-018_4.out"
        rlPhaseEnd

	rlPhaseStartTest "pki_ca_user_cli_ca_user_membership-del-018: Delete user-membership when uid is not associated with a group"
		rlLog "pki -d $CERTDB_DIR \
                           -n ${prefix}_adminV \
                           -c $CERTDB_DIR_PASSWORD \
			   -h $SUBSYSTEM_HOST \
			   ...
			   -h $SUBSYSTEM_HOST \
                ...
                command="pki -d $CERTDB_DIR  -h $SUBSYSTEM_HOST -p $(eval echo \$${subsystemId}_UNSECURE_PORT) -n ${prefix}_adminV -c $CERTDB_DIR_PASSWORD ca-user-membership-del user123 \"Administrators\""
                rlLog "Executing $command"
	...
	rlPhaseStartTest "pki_ca_user_cli_ca_user_membership-del-019: Deleting a user that has membership with groups removes the user from the groups"
		rlLog "pki -d $CERTDB_DIR \
                           -n ${prefix}_adminV \
                           -c $CERTDB_DIR_PASSWORD \
			   -h $SUBSYSTEM_HOST \
		...
		rlRun "pki -d $CERTDB_DIR \
			   ...
			   -h $SUBSYSTEM_HOST \
                           -p $(eval echo \$${subsystemId}_UNSECURE_PORT) \
                            ca-group-member-find  Administrators > $TmpDir/pki-ca-user-del-ca-user-membership-find-user-del-ca-20_4.out" \
                            0 \
			   ...
			   -h $SUBSYSTEM_HOST \
                           -p $(eval echo \$${subsystemId}_UNSECURE_PORT) \
                            ca-group-member-find \"Certificate Manager Agents\" > $TmpDir/pki-ca-user-del-ca-user-membership-find-user-del-ca-20_5.out" \
                            0 \
	...
	#Usability tests
	rlPhaseStartTest "pki_ca_user_cli_ca_user_membership-del-020: User deleted from  Administrators group cannot create a new user"
		rlRun "pki -d $CERTDB_DIR \
                           -n ${prefix}_adminV \
                           -c $CERTDB_DIR_PASSWORD \
			   -h $SUBSYSTEM_HOST \
		...
		local expfile="$TmpDir/expfile_testuser1.out"
                echo "spawn -noecho pki -d $TEMP_NSS_DB -n testuser1 -c Password -h $SUBSYSTEM_HOST -p $(eval echo \$${subsystemId}_UNSECURE_PORT) ca-user-add --fullName=test_user u15" > $expfile
...
Import CA certificate (Y/n)? \"" >> $expfile
		...
		command="pki -d $TEMP_NSS_DB -h $SUBSYSTEM_HOST -p $(eval echo \$${subsystemId}_UNSECURE_PORT) -n testuser1 -c Password ca-user-add --fullName=test_user u212"
		rlLog "Executing $command"
		errmsg="ForbiddenException: Authorization Error"
		errorcode=255
	...
	#Usability tests
	rlPhaseStartTest "pki_ca_user_cli_ca_user_membership-del-021: User deleted from the Certificate Manager Agents group can not approve certificate requests"
		rlRun "pki -d $CERTDB_DIR \
                           -n ${prefix}_adminV \
                           -c $CERTDB_DIR_PASSWORD \
			   -h $SUBSYSTEM_HOST \
