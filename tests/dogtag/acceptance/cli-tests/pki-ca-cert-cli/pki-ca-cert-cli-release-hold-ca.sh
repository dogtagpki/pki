...
run_pki-ca-cert-release-hold-ca_tests()
{
	local cs_Type=$1
	local cs_Role=$2

        # Creating Temporary Directory for pki ca-cert-release-hold
        rlPhaseStartSetup "pki ca-cert-release-hold Temporary Directory"
	...
	rlPhaseStartTest "pki_cert_release_hold-configtest: pki ca-cert-release-hold --help configuration test"
	rlRun "pki ca-cert-release-hold --help > $TmpDir/cert-release-hold.out 2>&1" 0 "pki cert-release-hold --help"
	rlAssertGrep "usage: ca-cert-release-hold <Serial Number> \[OPTIONS...]" "$TmpDir/cert-release-hold.out"	
	rlAssertGrep "--force   Force" "$TmpDir/cert-release-hold.out"
	rlAssertGrep "--help    Show help options" "$TmpDir/cert-release-hold.out"
	rlPhaseEnd
	
        rlPhaseStartTest "pki_ca_cert_release_hold_001: Release a valid cert on Hold using Agent Certificate"
	...
	local cmd="pki -d $CERTDB_DIR -c $CERTDB_DIR_PASSWORD -n \"$CA_agentV_user\" -h $target_host -p $target_port ca-cert-release-hold $cert_serialNumber"
	...
	rlPhaseStartTest "pki_ca_cert_release_hold_002: Release a Valid cert not on Hold using Agent Certificate"	
        rlLog "Generate Temporary certificate"
        rlRun "generate_new_cert tmp_nss_db:$TEMP_NSS_DB tmp_nss_db_pwd:$TEMP_NSS_DB_PWD \
                myreq_type:pkcs10 algo:rsa key_size:1024 subject_cn: subject_uid: \
                subject_email: subject_ou: subject_o: subject_c: archive:false \
	...
	rlPhaseStartTest "pki_ca_cert_release_hold_003: Release a invalid cert using Agent Certificate"
	...
	rlPhaseStartTest "pki_ca_cert_release_hold_004: Release a valid Cert on Hold (in decimalNumber) using Agent Certificate"
        rlLog "Generate Temporary certificate"
        rlRun "generate_new_cert tmp_nss_db:$TEMP_NSS_DB tmp_nss_db_pwd:$TEMP_NSS_DB_PWD \
                myreq_type:pkcs10 algo:rsa key_size:1024 subject_cn: subject_uid: \
                subject_email: subject_ou: subject_o: subject_c: archive:false \
	...
	local cmd="pki -d $CERTDB_DIR -c $CERTDB_DIR_PASSWORD -n \"$CA_agentV_user\" -h $target_host -p $target_port ca-cert-release-hold $decimal_cert_serialNumber"
        rlRun "cert-release-hold_expect_data $exp $cert_info \"$cmd\""
        rlRun "/usr/bin/expect -f $exp > $expout 2>&1"
        rlAssertGrep "Placed certificate \"$cert_serialNumber\" off-hold" "$expout"
        rlAssertGrep "Serial Number: $cert_serialNumber" "$expout"
	...
	rlPhaseStartTest "pki_ca_cert_release_hold_005: Test-1 Release a valid cert with subject name having i18n characters on Hold using Agent Certificate"
        rlLog "Generate Temporary certificate"
        rlRun "generate_new_cert tmp_nss_db:$TEMP_NSS_DB tmp_nss_db_pwd:$TEMP_NSS_DB_PWD \
	...
	rlPhaseStartTest "pki_ca_cert_release_hold_006: Test-2 Release a valid cert with subject name having i18n characters on Hold using Agent Certificate"
        rlLog "Generate Temporary certificate"
        rlRun "generate_new_cert tmp_nss_db:$TEMP_NSS_DB tmp_nss_db_pwd:$TEMP_NSS_DB_PWD \
	...
	rlPhaseStartTest "pki_ca_cert_release_hold_007: Test-3 Release a valid cert with subject name having i18n characters on Hold using Agent Certificate"
        rlLog "Generate Temporary certificate"
        rlRun "generate_new_cert tmp_nss_db:$TEMP_NSS_DB tmp_nss_db_pwd:$TEMP_NSS_DB_PWD \
	...
	rlPhaseStartTest "pki_ca_cert_release_hold_008: Test-4 Release a valid cert with subject name having i18n characters on Hold using Agent Certificate"
        rlLog "Generate Temporary certificate"
        rlRun "generate_new_cert tmp_nss_db:$TEMP_NSS_DB tmp_nss_db_pwd:$TEMP_NSS_DB_PWD \
	...
	rlPhaseStartTest "pki_ca_cert_release_hold_009: Test-5 Release a valid cert with subject name having i18n characters on Hold using Agent Certificate"
        rlLog "Generate Temporary certificate"
        rlRun "generate_new_cert tmp_nss_db:$TEMP_NSS_DB tmp_nss_db_pwd:$TEMP_NSS_DB_PWD \
	...
	rlPhaseStartTest "pki_ca_cert_release_hold_0010: Release a Invalid certificate (in decimalNumber) using Agent Certificate"
	rlRun "pki -d $CERTDB_DIR \
		-c $CERTDB_DIR_PASSWORD \
		-n \"$CA_agentV_user\" \
		-h $target_host \
	...
	rlPhaseStartTest "pki_ca_cert_release_hold_0011: Release a valid certificate(hexadecimal) on Hold using Agent Certificate (--force)"
        rlLog "Generate Temporary certificate"
        rlRun "generate_new_cert tmp_nss_db:$TEMP_NSS_DB tmp_nss_db_pwd:$TEMP_NSS_DB_PWD \
                myreq_type:pkcs10 algo:rsa key_size:1024 subject_cn: subject_uid: \
                subject_email: subject_ou: subject_o: subject_c: archive:false \
	...
	local cert_serialNumber=$(cat $cert_info| grep cert_serialNumber | cut -d- -f2)
		...
		-n \"$CA_agentV_user\" \
		-h $target_host \
		-p $target_port ca-cert-release-hold $cert_serialNumber \
	...
	rlPhaseStartTest "pki_ca_cert_release_hold_0012: Release a valid certificate(Decimal) on Hold using Agent Certificate  (--force)"
        rlLog "Generate Temporary certificate"
        rlRun "generate_new_cert tmp_nss_db:$TEMP_NSS_DB tmp_nss_db_pwd:$TEMP_NSS_DB_PWD \
                myreq_type:pkcs10 algo:rsa key_size:1024 subject_cn: subject_uid: \
                subject_email: subject_ou: subject_o: subject_c: archive:false \
	...
	local decimal_cert_serialNumber=$(cat $cert_info| grep decimal_valid_serialNumber | cut -d- -f2)
		...
		-n \"$CA_agentV_user\" \
		-h $target_host \
		-p $target_port ca-cert-release-hold $decimal_cert_serialNumber \
	...
	rlPhaseStartTest "pki_ca_cert_release_hold_0013: Release a invalid certificate (Junk characters) using Agent Certificate"
	rlRun "pki -d $CERTDB_DIR \
		-c $CERTDB_DIR_PASSWORD \
		-h $target_host \
		-p $target_port \
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_ca_cert_release_hold_0014: Release a invalid certificate (Junk characters) using Agent Certificate(--force)"
        rlRun "pki -d $CERTDB_DIR \
		-c $CERTDB_DIR_PASSWORD \
		-n \"$CA_agentV_user\" \
		-h $target_host \
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_ca_cert_release_hold_0015: Release a valid certificate(hexadecimal) on Hold using Admin Certificate (--force)"
        rlLog "Generate Temporary certificate"
        rlRun "generate_new_cert tmp_nss_db:$TEMP_NSS_DB tmp_nss_db_pwd:$TEMP_NSS_DB_PWD \
                myreq_type:pkcs10 algo:rsa key_size:1024 subject_cn: subject_uid: \
                subject_email: subject_ou: subject_o: subject_c: archive:false \
	...
	rlAssertGrep "Authorization Error" "$certout"
        ...
        rlPhaseStartTest "pki_ca_cert_release_hold_0016: Release a valid certificate(hexadecimal) on Hold using Admin Cert(--force)"
        rlLog "Generate Temporary certificate"
        rlRun "generate_new_cert tmp_nss_db:$TEMP_NSS_DB tmp_nss_db_pwd:$TEMP_NSS_DB_PWD \
                myreq_type:pkcs10 algo:rsa key_size:1024 subject_cn: subject_uid: \
                subject_email: subject_ou: subject_o: subject_c: archive:false \
        ...
        rlPhaseStartTest "pki_ca_cert_release_hold_0017: Release valid certificate(hexadecimal) on Hold using Revoked Agent cert (--force)"
        rlLog "Generate Temporary certificate"
        rlRun "generate_new_cert tmp_nss_db:$TEMP_NSS_DB tmp_nss_db_pwd:$TEMP_NSS_DB_PWD \
                myreq_type:pkcs10 algo:rsa key_size:1024 subject_cn: subject_uid: \
                subject_email: subject_ou: subject_o: subject_c: archive:false \
        ...
        rlPhaseStartTest "pki_ca_cert_release_hold_0018: Release valid certificate(hexadecimal) on Hold using Audit cert (--force)"
        rlLog "Generate Temporary certificate"
        rlRun "generate_new_cert tmp_nss_db:$TEMP_NSS_DB tmp_nss_db_pwd:$TEMP_NSS_DB_PWD \
                myreq_type:pkcs10 algo:rsa key_size:1024 subject_cn: subject_uid: \
                subject_email: subject_ou: subject_o: subject_c: archive:false \
        ...
        rlPhaseStartTest "pki_ca_cert_release_hold_0019: Release valid certificate(hexadecimal) on Hold using CA Operator cert (--force)"
        rlLog "Generate Temporary certificate"
        rlRun "generate_new_cert tmp_nss_db:$TEMP_NSS_DB tmp_nss_db_pwd:$TEMP_NSS_DB_PWD \
                myreq_type:pkcs10 algo:rsa key_size:1024 subject_cn: subject_uid: \
                subject_email: subject_ou: subject_o: subject_c: archive:false \
		...
		-h $target_host \
		-p $target_port \
		ca-cert-release-hold $cert_serialNumber \
		--force 2> $certout" 1,255 "Release hold using valid operator cert"
        rlAssertGrep "Authorization Error" "$certout"
        rlPhaseEnd

        rlPhaseStartTest "pki_ca_cert_release_hold_0020: Release a cert revoked with reason key compromise using Agent cert"
        rlLog "Generate Temporary certificate"
        rlRun "generate_new_cert tmp_nss_db:$TEMP_NSS_DB tmp_nss_db_pwd:$TEMP_NSS_DB_PWD \
                myreq_type:pkcs10 algo:rsa key_size:1024 subject_cn: subject_uid: \
                subject_email: subject_ou: subject_o: subject_c: archive:false \
	...
	rlLog "With released Agent Cert hold a user cert"
		...
		-h $target_host \
		-p $target_port \
		ca-cert-release-hold $decimal_cert_serialNumber \
                 --force 1> $certout" 0 "Release hold a cert with serialNumber given in decimals"
        rlAssertGrep "Placed certificate \"$cert_serialNumber\" off-hold" "$certout"
        rlAssertGrep "Serial Number: $cert_serialNumber" "$certout"
        rlAssertGrep "Issuer: CN=PKI $CA_INST Signing Cert,O=redhat" "$certout"
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_ca_cert_release_hold_0022: Release valid certificate(hexadecimal) on Hold using Expired Admin Certificate (--force)"
        rlLog "Generate Temporary certificate"
        rlRun "generate_new_cert tmp_nss_db:$TEMP_NSS_DB tmp_nss_db_pwd:$TEMP_NSS_DB_PWD \
                myreq_type:pkcs10 algo:rsa key_size:1024 subject_cn: subject_uid: \
                subject_email: subject_ou: subject_o: subject_c: archive:false \
	...
	rlAssertGrep "200 OK" "$TmpDir/chrony.out"
        ...
        rlPhaseStartTest "pki_ca_cert_release_hold_0023: Release valid certificate(hexadecimal) on Hold using Expired Agent cert(--force)"
        rlLog "Generate Temporary certificate"
        rlRun "generate_new_cert tmp_nss_db:$TEMP_NSS_DB tmp_nss_db_pwd:$TEMP_NSS_DB_PWD \
                myreq_type:pkcs10 algo:rsa key_size:1024 subject_cn: subject_uid: \
                subject_email: subject_ou: subject_o: subject_c: archive:false \
