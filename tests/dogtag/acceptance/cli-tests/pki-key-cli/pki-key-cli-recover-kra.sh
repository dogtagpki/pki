...
run_pki-key-recover-kra_tests()
{
	...
	rlPhaseStartTest "pki_key_cli-configtest: pki key-recover --help configuration test"
        rlRun "pki key-recover --help > $key_recover_output" 0 "pki key-recover --help"
	...
	rlPhaseStartTest "pki_key_recover-001: Generating symmetric key recovery request using valid agent cert should succeed"
	rlLog "Executing pki -d -d $CERTDB_DIR \
                -c $CERTDB_DIR_PASSWORD \
                -h $tmp_kra_host \
                -p $target_unsecure_port \
                -n \"$valid_agent_cert\" \
                key-recover --keyID $key_id > $key_recover_output"
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_key_recover-002: Generating Asymmetric key recovery request using valid agent cert should succeed"
	rlLog "Executing pki -d $CERTDB_DIR \
        ...
        rlRun "pki -d $CERTDB_DIR \
                ...
                -p $target_unsecure_port \
                -n \"$valid_agent_cert\" \
                key-recover --keyID $key_id > $key_recover_output" 0 "Recover key $key_id as $valid_agent_user"
        local recover_request_id=$(cat $key_recover_output | grep "Request ID" | awk -F ": " '{print $2}')
        rlAssertGrep "Key ID: $key_id" "$key_recover_output"
        rlAssertGrep "Type: securityDataRecovery" "$key_recover_output"
        rlAssertGrep "Status: svc_pending" "$key_recover_output"
	...
	rlPhaseStartTest "pki_key_recover-003: Issue a passphrase recovery request using valid agent cert should succeed"
	rlLog "Executing pki -d $CERTDB_DIR \
                -c $CERTDB_DIR_PASSWORD \
                -h $tmp_kra_host \
                -p $target_unsecure_port \
	...
	rlPhaseStartTest "pki_key_recover-004: Verify when no keyID is passed key-recover fails with command help"
	local key_id=''
        rlLog "Create a recovery of key $key_id request"
        rlRun "pki -d $CERTDB_DIR \
                -c $CERTDB_DIR_PASSWORD \
                ...
                -p $target_unsecure_port \
                -n \"$valid_agent_cert\" \
                key-recover --keyID $key_id > $key_recover_output 2>&1" 255,1 "Recover key with No data passed to --keyID"
        rlAssertGrep "usage: key-recover \[OPTIONS...\]" "$key_recover_output"
        rlAssertGrep "    --help                      Show help options" "$key_recover_output"
        rlAssertGrep "    --input <Input file path>   Location of the request file." "$key_recover_output"
	rlAssertGrep "    --keyID <Key Identifier>    Key Identifier for the secret to be" "$key_recover_output"
	...
	rlPhaseEnd
        ...
        rlRun "pki -d $CERTDB_DIR \
                ...
                -p $target_unsecure_port \
                -n \"$valid_admin_cert\" \
                key-recover --keyID $key_id > $key_recover_output 2>&1" 255,1 "Create Approve request for  ID $key_id"
	...
	rlLog "Executing pki -d <CERTDB_DIR> -c <CERTDB_PWD> -h $tmp_kra_host -c $target_unsecure_port -n $revoked_admin_cert key-recover --key $Key_id"
        rlRun "pki -d $CERTDB_DIR \
                ...
                key-recover --keyID $key_id > $key_recover_output 2>&1" 255,1 "Create Approve request for Key ID $key_id"
        rlAssertGrep "Authorization Error" "$key_recover_output"
        rlPhaseEnd
	...
	rlLog "Executing pki -d <CERTDB_DIR> -c <CERTDB_PWD> -h $tmp_kra_host -c $target_unsecure_port -n $expired_admin_cert key-recover --key $Key_id"
        rlRun "pki -d $CERTDB_DIR \
                -c $CERTDB_DIR_PASSWORD \
                -h $tmp_kra_host \
                -p $target_unsecure_port \
        ...
        rlAssertGrep "200 OK" "$TmpDir/chrony.out"
        rlLog "Date after modifying using chrony: $(date)"
	rlLog "Executing pki -d <CERTDB_DIR> -c <CERTDB_PWD> -h $tmp_kra_host -c $target_unsecure_port -n $expired_agent_cert key-recover --key $Key_id"
        rlRun "pki -d $CERTDB_DIR \
                -c $CERTDB_DIR_PASSWORD \
                -h $tmp_kra_host \
                -p $target_unsecure_port \
	...
	rlLog "Executing pki -d $TEMP_NSS_DB -c $TEMP_NSS_DB_PWD -h $tmp_kra_host -c $target_unsecure_port -n $pki_user key-recover --key $Key_id"
        rlRun "pki -d $TEMP_NSS_DB \
                ...
                -p $target_unsecure_port \
                -n \"$pki_user\" \
                key-recover --keyID $key_id > $key_recover_output 2>&1" 255,1 "Issuing Symmetric key recovery request user $pki_user cert"
	rlAssertGrep "ForbiddenException: Authorization Error" "$key_recover_output"
	rlPhaseEnd
	...
	rlLog "Executing pki -d $CERTDB_DIR -c $CERTDB_DIR_PASSWORD -U https://$tmp_kra_host:$target_secure_port -n $valid_agent_cert key-recover --keyID $key_id"
        rlRun "pki -d $CERTDB_DIR \
                ...
                -n \"$valid_agent_cert\" \
                key-recover --keyID $key_id > $key_recover_output" 0 "Issue recover request for $key_id"
        local recover_request_id=$(cat $key_recover_output | grep "Request ID" | awk -F ": " '{print $2}')
        rlAssertGrep "Key ID: $key_id" "$key_recover_output"
        rlAssertGrep "Type: securityDataRecovery" "$key_recover_output"
        rlAssertGrep "Status: svc_pending" "$key_recover_output"
	...
	rlRun "pki -d $CERTDB_DIR \
		...
		-u $pki_user \
		-w $pki_pwd \
		key-recover --keyID $key_id > $key_recover_output 2>&1" 255,1 "Issue a key recovery request as $pki_user"
	...
	rlLog "Executing pki -d $CERTDB_DIR -c $CERTDB_DIR_PASSWORD \
        ...
        rlRun "pki -d $CERTDB_DIR \
                ...
                -u $invalid_pki_user \
                -w $invalid_pki_user_pwd \
                key-recover --keyID $key_id > $key_recover_output 2>&1" 255,1 "key recovery request as $invalid_pki_user"
