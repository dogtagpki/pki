...
. /opt/rhqa_pki/env.sh
run_pki-key-mod-kra_tests()
{
	
        local cs_Type=$1
        local cs_Role=$2

        # Creating Temporary Directory for pki key-mod
	...
	local key_archive_output=$TmpDir/key-archive.out
        ...
        rlPhaseStartTest "pki_key_cli-configtest: pki key-mod --help configuration test"
        rlRun "pki key-mod --help > $key_mod_output" 0 "pki key-mod --help"
        rlAssertGrep "usage: key-mod <Key ID> --status <status> \[OPTIONS...\]" "$key_mod_output"
	...
	rlPhaseStartTest "pki_key_mod-0026: Modifying the status of key using Invalid user should fail"
        ...
        rlRun "pki -d $CERTDB_DIR \
                ...
                key-mod $tmp_key_id --status $state_change > $key_mod_output 2>&1" 255,1 "Modify status of key $tmp_key_id as $invalid_pki_user"
	...
	rlPhaseStartTest "pki_key_mod-0027: Modifying the status of key using Normal user(Not a member of any group) should fail"
       	rlRun "pki -d $CERTDB_DIR\
                ...
                -u $pki_user \
                -w $pki_pwd \
                key-mod $tmp_key_id --status $state_change > $key_mod_output 2>&1" 255,1 "Search key requests as $pki_user_fullName"
	...
	rlPhaseEnd
        ...
        rlPhaseStartCleanup "pki key-mod cleanup: Delete temp dir"
        rlRun "popd"
