...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#
#   runtest.sh of /CoreOS/rhcs/acceptance/legacy/ocsp-tests/ocsp-ad-usergroups
#   Description: OCSP Admin Console User groups tests
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#   The following legacy test is being tested:
#   OCSP Admin Console Usergroups tests
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#
...
run_ocsp-ad_usergroups()
{
	...
	rlPhaseStartTest "pki_ocsp_ad_usergroups-001: OCSP Console: List Users"
	...
	rlLog "curl --capath $CERTDB_DIR \
		...
		-k \"https://$tmp_ocsp_host:$target_secure_port/ocsp/ug\" >> $TmpDir/$test_out"
	rlRun "curl --capath $CERTDB_DIR \
		--dump-header $admin_out \
		--basic --user "$valid_admin:$valid_admin_pwd" \
		-d \"OP_TYPE=$OP_TYPE&OP_SCOPE=$OP_SCOPE\" \
		-k \"https://$tmp_ocsp_host:$target_secure_port/ocsp/ug\" >> $TmpDir/$test_out" 0 "List Users"
	...
	rlPhaseStartTest "pki_ocsp_ad_usergroups-002: OCSP Console: Add Users"
	local user1=$OCSP_INST-$RANDOM
	...
	local phone=''
	local state=''
	local groups='Online Certificate Status Manager Agents'
	local userType=''
	...
	rlPhaseStartTest "pki_ocsp_ad_usergroups-003: OCSP Console: Edit Users"
	...
	rlPhaseStartTest "pki_ocsp_ad_usergroups-004: OCSP Console: Import Cert to the user"
	local OP_TYPE='OP_ADD'
	...
	rlLog "curl --capath $CERTDB_DIR \
                --dump-header $admin_out \
                --basic --user "$valid_admin:$valid_admin_pwd" \
		-d \"OP_TYPE=$OP_TYPE&OP_SCOPE=$OP_SCOPE&RS_ID=$RS_ID\" --data-urlencode \"cert=$certificate_in_base64\" -k \"https://$tmp_ocsp_host:$target_secure_port/ocsp/ug\" > $TmpDir/$test_out" 0 "Import cert to $user1"
	rlRun "curl --capath $CERTDB_DIR \
                --dump-header $admin_out \
                --basic --user "$valid_admin:$valid_admin_pwd" \
                -d \"OP_TYPE=$OP_TYPE&OP_SCOPE=$OP_SCOPE&RS_ID=$RS_ID\" --data-urlencode \"cert=$certificate_in_base64\" -k \"https://$tmp_ocsp_host:$target_secure_port/ocsp/ug\" > $TmpDir/$test_out" 0 "Import cert to $user1"
	...
	rlRun "curl --capath $CERTDB_DIR \
                --dump-header $admin_out \
                --basic --user "$valid_admin:$valid_admin_pwd" \
                -d \"OP_TYPE=$OP_TYPE&OP_SCOPE=$OP_SCOPE&RS_ID=$RS_ID\" -k \"https://$tmp_ocsp_host:$target_secure_port/ocsp/ug\" > $TmpDir/$test_out" 0 "Search certificate"
	rlAssertGrep "HTTP/1.1 200 OK" "$admin_out"
	rlAssertGrep "-----BEGIN+CERTIFICATE-----" "$TmpDir/$test_out"
	rlAssertGrep "-----END+CERTIFICATE-----" "$TmpDir/$test_out"
	rlPhaseEnd

	rlPhaseStartTest "pki_ocsp_ad_usergroups-005: OCSP Console: Add Group"
	local group1=$OCSP_INST-group-$RANDOM
	local OP_TYPE='OP_ADD'
	...
	rlLog "curl --capath $CERTDB_DIR \
		--dump-header $admin_out \
		--basic --user "$valid_admin:$valid_admin_pwd" \
		-d \"OP_TYPE=$OP_TYPE&OP_SCOPE=$OP_SCOPE&RS_ID=$RS_ID\" -k \"https://$tmp_ocsp_host:$target_secure_port/ocsp/ug\" > $TmpDir/$test_out" 0 "Add Group $group1"
	rlRun "curl --capath $CERTDB_DIR \
                --dump-header $admin_out \
                --basic --user "$valid_admin:$valid_admin_pwd" \
                -d \"OP_TYPE=$OP_TYPE&OP_SCOPE=$OP_SCOPE&RS_ID=$RS_ID\" -k \"https://$tmp_ocsp_host:$target_secure_port/ocsp/ug\" > $TmpDir/$test_out" 0 "Add Group $group1"
	...
	rlLog "curl --capath $CERTDB_DIR \
		--dump-header $admin_out \
		--basic --user "$valid_admin:$valid_admin_pwd" \
		-d \"OP_TYPE=$OP_TYPE&OP_SCOPE=$OP_SCOPE\" -k \"https://$tmp_ocsp_host:$target_secure_port/ocsp/ug\" > $TmpDir/$test_out" 0 "Search for $group1"
	rlRun "curl --capath $CERTDB_DIR \
                --dump-header $admin_out \
                --basic --user "$valid_admin:$valid_admin_pwd" \
                -d \"OP_TYPE=$OP_TYPE&OP_SCOPE=$OP_SCOPE\" -k \"https://$tmp_ocsp_host:$target_secure_port/ocsp/ug\" > $TmpDir/$test_out" 0 "Search for $group1"
	...
	rlPhaseStartTest "pki_ocsp_ad_usergroups-006: OCSP Console: Edit Group"
	local OP_TYPE='OP_MODIFY'
	local OP_SCOPE='groups'
	local RS_ID=$group1
	local desc=$group1
	...
	rlLog "curl --capath $CERTDB_DIR \
		--dump-header $admin_out \
		--basic --user "$valid_admin:$valid_admin_pwd" \
		-d \"OP_TYPE=$OP_TYPE&OP_SCOPE=$OP_SCOPE&RS_ID=$RS_ID&desc=$desc&user=$user\" -k \"https://$tmp_ocsp_host:$target_secure_port/ocsp/ug\" > $TmpDir/$test_out" 0 "Edit $group1"
	rlRun "curl --capath $CERTDB_DIR \
                --dump-header $admin_out \
                --basic --user "$valid_admin:$valid_admin_pwd" \
                -d \"OP_TYPE=$OP_TYPE&OP_SCOPE=$OP_SCOPE&RS_ID=$RS_ID&desc=$desc&user=$user\" -k \"https://$tmp_ocsp_host:$target_secure_port/ocsp/ug\" > $TmpDir/$test_out" 0 "Edit $group1"
	...
	rlPhaseStartTest "pki_ocsp_ad_usergroups-007: OCSP Console: Delete Group"
	...
	rlLog "curl --capath $CERTDB_DIR \
		--dump-header $admin_out \
		--basic --user "$valid_admin:$valid_admin_pwd" -d \"OP_TYPE=$OP_TYPE&OP_SCOPE=$OP_SCOPE&RS_ID=$RS_ID\" -k \"https://$tmp_ocsp_host:$target_secure_port/ocsp/ug\" > $TmpDir/$test_out" 0 "Delete $group1"
	rlRun "curl --capath $CERTDB_DIR \
                --dump-header $admin_out \
                --basic --user "$valid_admin:$valid_admin_pwd" -d \"OP_TYPE=$OP_TYPE&OP_SCOPE=$OP_SCOPE&RS_ID=$RS_ID\" -k \"https://$tmp_ocsp_host:$target_secure_port/ocsp/ug\" > $TmpDir/$test_out" 0 "Delete $group1"
	rlAssertGrep "HTTP/1.1 200 OK" "$admin_out"
	...
	rlPhaseStartTest "pki_ocsp_ad_usergroups-008: OCSP Console: Delete User"
        local OP_TYPE='OP_DELETE'
        local OP_SCOPE='users'
	...
	rlLog "curl --capath $CERTDB_DIR \
                --dump-header $admin_out \
                --basic --user "$valid_admin:$valid_admin_pwd" -d \"OP_TYPE=$OP_TYPE&OP_SCOPE=$OP_SCOPE&RS_ID=$RS_ID\" -k \"https://$tmp_ocsp_host:$target_secure_port/ocsp/ug\" > $TmpDir/$test_out" 0 "Delete $user1"
        rlRun "curl --capath $CERTDB_DIR \
                --dump-header $admin_out \
                --basic --user "$valid_admin:$valid_admin_pwd" -d \"OP_TYPE=$OP_TYPE&OP_SCOPE=$OP_SCOPE&RS_ID=$RS_ID\" -k \"https://$tmp_ocsp_host:$target_secure_port/ocsp/ug\" > $TmpDir/$test_out" 0 "Delete $user1"
	rlAssertGrep "HTTP/1.1 200 OK" "$admin_out"
	...
	rlPhaseStartTest "pki_ocsp_ad_usergroups-009: OCSP Console: Verify Agent user cannot add new user"
        local user2=$OCSP_INST-$RANDOM
        local OP_TYPE='OP_ADD'
        local OP_SCOPE='users'
        local RS_ID=$user2
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_ocsp_ad_usergroups-0010: OCSP Console: Verify Audit user cannot add new user"
        local user2=$OCSP_INST-$RANDOM
        local OP_TYPE='OP_ADD'
        local OP_SCOPE='users'
        local RS_ID=$user2
	...
	rlPhaseStartTest "pki_ocsp_ad_usergroups-0011: OCSP Console: Verify Operator user cannot add new user"
	local user2=$OCSP_INST-$RANDOM
        local OP_TYPE='OP_ADD'
        local OP_SCOPE='users'
        local RS_ID=$user2
	...
	rlPhaseStartTest "pki_ocsp_ad_usergroups-0012: OCSP Console: Verify Agent user cannot add new group"
        local group2=$OCSP_INST-group-$RANDOM
        local OP_TYPE='OP_ADD'
        local OP_SCOPE='groups'
        local RS_ID=$group2
        local desc=$group2
	...
	rlPhaseStartTest "pki_ocsp_ad_usergroups-0013: OCSP Console: Verify Audit user cannot add new group"
        local group2=$OCSP_INST-group-$RANDOM
        local OP_TYPE='OP_ADD'
        local OP_SCOPE='groups'
        local RS_ID=$group2
	...
	rlPhaseStartTest "pki_ocsp_ad_usergroups-0014: OCSP Console: Verify Operator user cannot add new group"
        local group2=$OCSP_INST-group-$RANDOM
        local OP_TYPE='OP_ADD'
        local OP_SCOPE='groups'
        local RS_ID=$group2
	...
	rlPhaseStartTest "pki_ocsp_ad_usergroups-0015: OCSP Console: Verify Agent User cannot delete existing group"
        local OP_TYPE='OP_DELETE'
        local OP_SCOPE='groups'
        local RS_ID=$group1
        local test_out=groupdelete.out
        rlLog "curl --capath $CERTDB_DIR \
                --dump-header $admin_out \
                --basic --user "$valid_agent:$valid_agent_pwd" -d \"OP_TYPE=$OP_TYPE&OP_SCOPE=$OP_SCOPE&RS_ID=$RS_ID\" -k \"https://$tmp_ocsp_host:$target_secure_port/ocsp/ug\" > $TmpDir/$test_out" 0 "Delete $group1"
        rlRun "curl --capath $CERTDB_DIR \
                --dump-header $admin_out \
                --basic --user "$valid_agent:$valid_agent_pwd" -d \"OP_TYPE=$OP_TYPE&OP_SCOPE=$OP_SCOPE&RS_ID=$RS_ID\" -k \"https://$tmp_ocsp_host:$target_secure_port/ocsp/ug\" > $TmpDir/$test_out" 0 "Delete $group1"
        rlAssertGrep "HTTP/1.1 200 OK" "$admin_out"
	...
	rlPhaseStartSetup "OCSP Console: Add Group"
        local group1=$OCSP_INST-group-$RANDOM
        local OP_TYPE='OP_ADD'
        local OP_SCOPE='groups'
        local RS_ID=$group1
	...
	rlPhaseStartTest "pki_ocsp_ad_usergroups-0016: OCSP Console: Verify Agent User cannot delete existing group"
        local OP_TYPE='OP_DELETE'
        local OP_SCOPE='groups'
        local RS_ID=$group1
        local test_out=groupdelete.out
	...
	rlPhaseStartTest "pki_ocsp_ad_usergroups-0017: OCSP Console: Verify Audit User cannot delete existing group"
        local OP_TYPE='OP_DELETE'
        local OP_SCOPE='groups'
        local RS_ID=$group1
        local test_out=groupdelete.out
        rlLog "curl --capath $CERTDB_DIR \
                --dump-header $admin_out \
                --basic --user "$valid_audit:$valid_audit_pwd" -d \"OP_TYPE=$OP_TYPE&OP_SCOPE=$OP_SCOPE&RS_ID=$RS_ID\" -k \"https://$tmp_ocsp_host:$target_secure_port/ocsp/ug\" > $TmpDir/$test_out" 0 "Delete $group1"
        rlRun "curl --capath $CERTDB_DIR \
                --dump-header $admin_out \
                --basic --user "$valid_audit:$valid_audit_pwd" -d \"OP_TYPE=$OP_TYPE&OP_SCOPE=$OP_SCOPE&RS_ID=$RS_ID\" -k \"https://$tmp_ocsp_host:$target_secure_port/ocsp/ug\" > $TmpDir/$test_out" 0 "Delete $group1"
        rlAssertGrep "HTTP/1.1 200 OK" "$admin_out"
	...
	rlPhaseStartTest "pki_ocsp_ad_usergroups-0018: OCSP Console: Verify Operator User cannot delete existing group"
        local OP_TYPE='OP_DELETE'
        local OP_SCOPE='groups'
        local RS_ID=$group1
        local test_out=groupdelete.out
        rlLog "curl --capath $CERTDB_DIR \
                --dump-header $admin_out \
                --basic --user "$valid_operator:$valid_operator_pwd" -d \"OP_TYPE=$OP_TYPE&OP_SCOPE=$OP_SCOPE&RS_ID=$RS_ID\" -k \"https://$tmp_ocsp_host:$target_secure_port/ocsp/ug\" > $TmpDir/$test_out" 0 "Delete $group1"
        rlRun "curl --capath $CERTDB_DIR \
                --dump-header $admin_out \
                --basic --user "$valid_operator:$valid_operator_pwd" -d \"OP_TYPE=$OP_TYPE&OP_SCOPE=$OP_SCOPE&RS_ID=$RS_ID\" -k \"https://$tmp_ocsp_host:$target_secure_port/ocsp/ug\" > $TmpDir/$test_out" 0 "Delete $group1"
        rlAssertGrep "HTTP/1.1 200 OK" "$admin_out"
