...
run_ee-ca-retrieval_tests()
{
	...
	rlPhaseStartTest "pki_ca_ee-retrieval-001: CA Cert Retrieval -  Check Request Status servlet(pending request)"
	...
	rlPhaseStartTest "pki_ca_ee-retrieval-002: CA Cert Retrieval -  Check Request Status servlet(completed request)"
	local request_id="1"
	rlLog "curl --basic --dump-header $admin_out -d \"format=id&requestId=$request_id&submit=Submit\" -k \"https://$tmp_ca_host:$target_secure_port/ca/ee/ca/checkRequest\""
	rlRun "curl --basic --dump-header $admin_out -d \"format=id&requestId=$request_id&submit=Submit\" -k \"https://$tmp_ca_host:$target_secure_port/ca/ee/ca/checkRequest\" > $TmpDir/$test_out" 0 "Check certificate request status of $request_id"
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_ca_ee-retrieval-003: CA Cert Retrieval -  Check Request Status servlet(Negative Value)"
        local request_id="-1"
        rlLog "curl --basic --dump-header $admin_out -d \"format=id&requestId=$request_id&submit=Submit\" -k \"https://$tmp_ca_host:$target_secure_port/ca/ee/ca/checkRequest\""
        rlRun "curl --basic --dump-header $admin_out -d \"format=id&requestId=$request_id&submit=Submit\" -k \"https://$tmp_ca_host:$target_secure_port/ca/ee/ca/checkRequest\" > $TmpDir/$test_out" 0 "Check certificate request status of $request_id"
        ...
        rlPhaseStartTest "pki_ca_ee-retrieval-004: CA Cert Retrieval -  Check Request Status servlet(Non numerical characters)"
	...
	rlPhaseStartTest "pki_ca_ee-retrieval-004: CA Cert Retrieval -  List Certificates(Default values list first 20 records)"
	local op=listCerts
	...
	rlPhaseStartTest "pki_ca_ee-retrieval-005: CA Cert Retrieval - List Certificates from range 0x5 to 0x20"
        local op=listCerts
        local queryCertFilter="(|(certStatus=VALID)(certStatus=REVOKED))"
        ...
        local no_of_records=$(cat $TmpDir/$test_out | grep "record.subject=" | wc -l)
        rlAssertEquals "Verify No of records displayed is equal to $maxCount" $no_of_records $maxCount
	rlAssertGrep "record.serialNumber=\"1c\"" "$TmpDir/$test_out"
	...
	rlPhaseStartTest "pki_ca_ee-retrieval-006: CA Cert Retrieval - List only valid certificates"
        local op=listCerts
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_ca_ee-retrieval-007: CA Cert Retrieval - List certs with max count of 10"
        local op=listCerts
        local queryCertFilter="(certStatus=VALID)"
        local serialFrom="0"
        local serialTo="20"
	...
	rlPhaseStartTest "pki_ca_ee-retrieval-008: CA Cert Retrieval - Search certs with max count of 10"
	...
	rlPhaseEnd
        ...
        rlPhaseStartTest "pki_ca_ee-retrieval-009: CA Cert Retrieval - Search certs with subject Name matching Email Address(method=exact)"
        local test_out=srcCerts.txt
        local op=srchCerts
	...
	rlAssertGrep "record.serialNumber=\"$serial_number_without_hex_lower\"" "$TmpDir/$test_out"
        ...
        rlPhaseStartTest "pki_ca_ee-retrieval-0010: CA Cert Retrieval - Search certs with subject Name matching userID(method=exact)"
        local test_out=srcCerts.txt
        local op=srchCerts
        local serialNumberRangeInUse=''
        local serialFrom=''
	...
	rlAssertGrep "record.serialNumber=\"$serial_number_without_hex_lower\"" "$TmpDir/$test_out"
        ...
        rlPhaseStartTest "pki_ca_ee-retrieval-0011: CA Cert Retrieval - Search revoked certs with revoked by valid agent"
        local test_out=srcCerts.txt
        local op=srchCerts
        local serialNumberRangeInUse=''
        local serialFrom=''
	...
	rlPhaseStartTest "pki_ca_ee-retrieval-0012: CA Cert Retrieval - Search revoked certs with revoked by reason Unspecified"
        local op=srchCerts
        local serialNumberRangeInUse=''
        local serialFrom=''
        local serialTo=''
	...
	rlPhaseStartTest "pki_ca_ee-retrieval-0013: CA Cert Retrieval - Search revoked certs with revoked by reason Unspecified & key compromise"
        local op=srchCerts
        local serialNumberRangeInUse=''
        local serialFrom=''
        local serialTo=''
	...
	rlPhaseStartTest "pki_ca_ee-retrieval-0014: CA Cert Retrieval - Search certs issued by Valid agent cert"
        local op=srchCerts
        local serialNumberRangeInUse=''
        local serialFrom=''
        local serialTo=''
	...
	rlPhaseStartTest "pki_ca_ee-retrieval-0015: CA Cert Retrieval - Search certs with a validity period of 15 days"
        local op=srchCerts
        local serialNumberRangeInUse=''
        local serialFrom=''
        local serialTo=''
	...
	rlPhaseStartTest "pki_ca_ee-retrieval-0015: CA Cert Retrieval - Import CA Certificate chain"
	local op='download'
	...
	rlPhaseStartTest "pki_ca_ee-retrieval-0016: CA Cert Retrieval - Download CA certificate in binary format"
	...
	rlPhaseStartTest "pki_ca_ee-retrieval-0017: CA Cert Retrieval - Import latest CRL"
