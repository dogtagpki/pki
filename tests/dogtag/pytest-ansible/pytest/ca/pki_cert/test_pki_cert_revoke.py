...
def test_pki_cert_revoke_with_comments(ansible_module):
    ...
    command = 'pki -d {} -c {} -p {} -P http -n "{}" {} {} --comments ' \
    ...
    for result in revoke_out.values():
        if result['rc'] == 0:
            ...
            pytest.fail("Failed to run pki {} {}".format(cmd, decimal))
...
def test_pki_cert_revoke_with_force(ansible_module):
    ...
    for host, result in revoke_out.items():
        if result['rc'] == 0:
            ...
            pytest.fail("Failed to run pki {} {} --force".format(cmd, cert_id))
...
def test_pki_cert_revoke_which_is_already_revoked(ansible_module):
    ...
    for host, result in revoke_again.items():
        if result['rc'] == 0:
            pytest.fail("Failed to run pki {} {} when certificate "
...
def test_pki_cert_revoke_non_ca_signing_certificate(ansible_module):
    ...
    for host, result in revoke_out.items():
        if result['rc'] == 0:
            pytest.fail("Failed to run pki {} {} --ca --reason unspecified".format(cmd, cert_id))
        else:
            assert "UnauthorizedException: Certificate {} is not a CA signing certificate".format(cert_id) in result['stdout']
...
def test_pki_cert_revoke_non_ca_signing_certificate_with_force(ansible_module):
    ...
    for host, result in revoke_out.items():
        if result['rc'] == 0:
            ...
            assert "UnauthorizedException: Certificate {} is not a CA signing certificate".format(cert_id) in result['stderr']
...
def test_pki_cert_revoke_ca_signing_cert_with_force(ansible_module):
    ...
    for host, result in revoke_out.items():
        if result['rc'] == 0:
            ...
            pytest.fail("Failed to run pki {} {}".format(cmd, '0x1'))
    ...
    for host, result in revoke_out.items():
        if result['rc'] == 0:
            ...
            pytest.fail("Failed to place the certificate off-hold '0x1'.")
...
@pytest.mark.parametrize('user, error', (('CA_AgentR', ['PKIException: Unauthorized']),
...
def test_pki_cert_revoke_with_different_audit_and_agent_cert(ansible_module, user, error):
    ...
    for host, result in revoke_out.items():
        if result['rc'] == 0:
            pytest.fail("Failed to run pki -n {} {} {}".format(user, cmd, cert_id))
        else:
            for e in error:
