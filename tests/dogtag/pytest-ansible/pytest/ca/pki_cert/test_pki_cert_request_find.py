...
def test_pki_ca_cert_request_find_help(ansible_module, subcmd):
    ...
    """
    help_out = ansible_module.command('pki -p {} -P http {} {}'.format(constants.CA_HTTP_PORT,
                                                                       cmd, subcmd))
    ...
    for result in help_out.values():
        ...
        if result['rc'] == 0:
            ...
            if 'PKIException' in result['stderr']:
...
def test_pki_ca_cert_request_find_with_different_status(ansible_module, subcmd):
            ...
            if subcmd != 'pending':
                ...
                for result1 in request_out.values():
                    ...
                    if result1['rc'] == 0:
                        ...
                        pytest.fail("Failed to run pki cert-request-review "
                                     "--action {} {}".format(subcmd, request_id))
    ...
    for result in find_req.values():
        ...
        if result['rc'] == 0:
            ...
            pytest.fail("Failed to run pki {} --status {}".format(cmd, st))
...
def test_pki_ca_cert_request_find_with_status_all(ansible_module):
    ...
    for result in find_req.values():
        ...
        if result['rc'] == 0:
            ...
            pytest.fail("Failed to run pki {} --status {}".format(cmd, 'all'))
...
def test_pki_ca_cert_request_find_with_status_maxResult(ansible_module, max_results, st):
    ...
    for result in max_result.values():
        ...
        if result['rc'] == 0:
            ...
            pytest.fail("Failed to run pki {} --status {} --maxSize {}".format(cmd, st,
                                                                                max_results))
...
def test_pki_ca_cert_request_find_with_type_and_status(ansible_module, r_type, st):
    ...
    for result in type_and_status.values():
        ...
        if result['rc'] == 0:
            ...
            pytest.fail("Failed to run pki ca-cert-request-show with "
                         "--type {} --status {}".format(r_type, st))


@pytest.mark.parametrize('start', [5, -5, 'xe2at', ''])
def test_pki_ca_cert_request_find_with_start_option(ansible_module, start):
    ...
    """
    if start == 5:
        error = 'Number of entries returned'
    elif start == -5:
...
@pytest.mark.parametrize('maxr', [5, -5, 'xe2at', '', '39493840234'])
...
@pytest.mark.parametrize('size', [5, -5, 'xe2at', '', '39493840234'])
def test_pki_ca_cert_request_find_with_size_option(ansible_module, size):
    ...
    if size == 5:
        ...
        error = 'Number of entries returned 0'
    elif size == 'xe2at':
        error = 'NumberFormatException: For input string: "xe2at"'
    elif size == '':
    ...
    for result in req_find.values():
        ...
        if result['rc'] >= 1:
            if 'CERTIFICATE_EXPIRED' in result['stderr']:
                ...
                assert 'FATAL: SSL alert received: CERTIFICATE_UNKNOWN' in result['stderr']
