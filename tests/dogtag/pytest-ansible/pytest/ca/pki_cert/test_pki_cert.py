...
def test_pki_cert(ansible_module, cmd):
    ...
    for host, result in output.items():
        ...
        assert "ca-cert-export                    Export certificate" in result['stdout']
        ...
        assert "ca-cert-status                    Check certificate status" in result['stdout']
        assert "ca-cert-request                   Certificate request management commands" in result['stdout']
        assert "ca-cert-signing-show              Show CA signing certificate" in result['stdout']
        assert "ca-cert-signing-export            Export CA signing certificate" in result['stdout']
        assert "ca-cert-transport-show            Show CA transport certificate" in result['stdout']
        assert "ca-cert-transport-export          Export CA transport certificate" in result['stdout']
