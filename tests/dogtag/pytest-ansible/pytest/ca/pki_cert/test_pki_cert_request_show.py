...
def test_pki_ca_cert_request_show_with_different_no(ansible_module, subcmd):
    ...
    for result1 in request_show.values():
        ...
        if result1['rc'] == 0:
            ...
            pytest.fail("Failed to run pki cert-request-show with {}.".format(subcmd))
    ...
    if cert_id:
        ...
        for result1 in renew_request.values():
            ...
            if result1['rc'] == 0:
                ...
                pytest.fail("Failed to submit certificate renew request.")
    ...
    if submitted_req:
        ...
        for result in approve_renew_req.values():
            ...
            if result['rc'] == 0:
                ...
                pytest.fail("Failed to run pki {} {} --action {}".format(cmd, cert_id, action))
...
def test_pki_ca_cert_request_show_with_invalid_serial_no(ansible_module, serial):
    ...
    for result in request_out.values():
        ...
        if result['rc'] == 0:
            ...
            pytest.fail("Failed to run pki {} {}".format(cmd, serial))
        else:
            assert 'RequestNotFoundException' in result['stderr']
...
def test_pki_ca_cert_request_show_with_different_profiles(ansible_module, profile):
    ...
    for host, result in profile_show_out.items():
        if result['rc'] == 0:
            ...
            pytest.fail("Failed to get xml file for profile {}".format(profile))
    ...
    for result in submit_request.values():
        ...
        if result['rc'] == 0:
            ...
            pytest.fail("Failed to run pki cert-request-submit with xml file.")
        ...
        if result['rc'] == 0:
            ...
            for result1 in request_show.values():
                if result1['rc'] == 0:
                    ...
                    pytest.fail("Failed to run pki cert-request-show with "
                                "{}.".format(result1['cmd']))
