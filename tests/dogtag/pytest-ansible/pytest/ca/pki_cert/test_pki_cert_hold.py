...
def test_pki_cert_hold_using_agent_cert_and_with_comment(ansible_module):
    ...
    for result in hold_out.values():
        if result['rc'] == 0:
            ...
            pytest.fail("Failed to run {} ".format(command))
...
def test_pki_cert_hold_using_agent_cert_with_invalid_nos(ansible_module, no):
    ...
    for result in hold_out.values():
        ...
        if result['rc'] == 0:
            ...
            pytest.fail("Failed to run pki {} {} ".format(cmd, no))
        else:
...
def test_pki_cert_hold_using_valid_agent_cert_with_force(ansible_module):
    ...
    for result in hold_out.values():
        ...
        if result['rc'] == 0:
            ...
            log.info(result['stderr'])
            pytest.fail("Failed to run pki {} {} ".format(cmd, cert_id, '--force'))
...
@pytest.mark.parametrize('no', (808364891351173399048839, 1155025308640651489455451))
...
def test_pki_cert_hold_using_agent_cert_with_junk_text(ansible_module, no, extra_args):
    ...
    for result in hold_out.values():
        ...
        if result['rc'] == 0:
            ...
            log.info(result['stderr'])
            pytest.fail("Failed to run pki {} {} ".format(cmd, hex_no))
        else:
            assert 'CertNotFoundException' in result['stderr']
...
def test_pki_cert_hold_using_agent_cert_with_different_admin_cert(ansible_module, nick):
    ...
    if nick.endswith('V'):
        ...
        # Expired certificates should indicate such. Don't validate the outer wrapping
        # exception which may change.
    ...
    for result in hold_out.values():
        ...
        if result['rc'] == 0:
            ...
            log.info(result['stderr'])
            pytest.fail("Failed to run pki {} {} ".format(cmd, cert_id))
        else:
            assert error in result['stderr']
...
def test_pki_cert_hold_when_cert_already_revoked(ansible_module):
    ...
    for result in hold_out.values():
        ...
        if result['rc'] == 0:
            ...
            assert "BadRequestException: certificate #{} has already been revoked".format(cert_id.split('0x')[1]) in result['stderr']
            log.info("Successfully run pki -n '{}' {} {}".format('CA_AuditV', cmd, cert_id))
...
def test_pki_cert_hold_using_agent_cert_with_i18n_character(ansible_module):
    """
    :id: 0f78ebb6-68a2-4493-9ca9-2df5793c0830
    :Title: Test pki ca-cert-hold using valid Agent Certificate with i18n
    :Description: Pki ca-cert-hold using valid Agent Certificate able to hold the certificate.
    :Requirement: RHCS-REQ Certificate Authority Certificate Enrollment
    :CaseComponent: \-
    :Setup: Use the subsystems setup in ansible to run subsystem commands
    ...
    req_hold = 'pki -d {} -c {} -n "{}" -p {} ca-cert-hold {}'.format(constants.NSSDB, constants.CLIENT_DATABASE_PASSWORD,
                                    constants.CA_ADMIN_NICK, constants.CA_HTTPS_PORT, cert_id)
    log.info("Running: {}".format(req_hold))
    hold_out = ansible_module.expect(
        command=req_hold,
        responses={"Are you sure (Y/N)?": 'y'})
    for result in hold_out.values():
        log.info("Running : {}".format(result['cmd']))
        if result['rc'] == 0:
            assert 'Placed certificate "{}" on-hold'.format(cert_id) in result['stdout']
            assert 'Serial Number: {}'.format(cert_id) in result['stdout']
            assert 'Subject DN: {}'.format(subject) in result['stdout'].encode('utf-8')
            assert 'Issuer DN: ' in result['stdout']
            assert 'Status: REVOKED' in result['stdout']
            assert 'Not Valid Before:' in result['stdout']
            assert 'Not Valid After:' in result['stdout']
            log.info("Successfully placed i18n certificate on-hold")
        else:
            log.error("Failed placed i18n certificate on-hold")
            log.info(result['stderr'])
            pytest.fail("Failed to run pki {} {} ".format(cmd, request_id))
