...
def test_setup(ansible_module):
    db1_setup = CertSetup(nssdb='/tmp/db1_test')
    db1_setup.create_certdb(ansible_module)
    db1_setup.import_ca_cert(ansible_module)
    for sub_sys in subsystem:
        db1_setup.import_admin_p12(ansible_module, sub_sys)

    db2_setup = CertSetup(nssdb='/tmp/db2_test')
    db2_setup.create_certdb(ansible_module)
    db2_setup.import_ca_cert(ansible_module)
    for sub_sys in subsystem:
        db2_setup.import_admin_p12(ansible_module, sub_sys)
        ...
        if result['rc'] == 0:
            ...
            for res in cert_show.values():
                if res['rc'] >= 1:
                    ...
                    pytest.fail("Failed to delete the certificate form the database.")
        else:
...
def test_pki_client_cert_del_wrong_password(ansible_module):
    ...
    for result in cert_find_output.values():
        if result['rc'] >= 1:
            ...
            pytest.fail("Fail: Deleted certificate with wrong password.")
...
def test_pki_client_cert_del_no_cert(ansible_module):
    ...
    for result in cert_del_output.values():
        if result['rc'] >= 1:
            ...
            pytest.fail("Fail: Deleted certificate with wrong nickname.")
    ...
    if serial_no:
        ...
        for res in get_cert.values():
            if res['rc'] == 0:
                ...
                pytest.fail("Failed to import the certificate.")
