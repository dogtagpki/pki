...
def test_pki_client_cert_show_help(ansible_module, args):
    ...
    for result in help_out.values():
        if result['rc'] == 0:
            ...
            pytest.fail("Failed to run pki client-cert-show --help command")
...
def test_pki_client_cert_show_without_nick(ansible_module):
    ...
    for result in show_out.values():
        if result['rc'] >= 1:
            ...
            pytest.fail("Failed: Ran pki client-cert-show command without nick")
...
def test_pki_client_cert_show_invalid_nick(ansible_module):
    ...
    for result in show_output.values():
        if result['rc'] >= 1:
            ...
            pytest.fail("Failed: Ran pki client-cert-show command with invalid nick")
...
def test_pki_client_cert_show_valid_nick(ansible_module):
    ...
    for result in show_out.values():
        if result['rc'] == 0:
            ...
            pytest.fail("Failed to run pki client-cert-show command")
...
def test_pki_client_cert_show_wrong_password(ansible_module):
    ...
    for result in show_output.values():
        if result['rc'] >= 1:
            ...
            pytest.fail("Failed to run {} ".format(show_cmd))
    ...
    for result in cert_show_out.values():
        if result['rc'] == 0:
            ...
            for r in p12_find.values():
                if r['rc'] == 0:
                    assert 'Certificate ID: ' in r['stdout']
                    assert 'Serial Number: ' in r['stdout']
                    assert 'Subject DN: ' in r['stdout']
                else:
                    pytest.fail("Failed to run pki client-cert-show command")
        else:
