...
def test_pki_client_help(ansible_module, args):
    ...
    for result in help_out.values():
        if result['rc'] == 0:
            assert "client-init                       Initialize NSS database" in result['stdout']
            assert "client-cert-find                  Find certificates in NSS database" in result['stdout']
            assert "client-cert-import                Import certificate into NSS database" in result['stdout']
