...
def test_pki_client_cert_mod_help(ansible_module, args):
    ...
    for result in mod_help_out.values():
        if result['rc'] == 0:
            ...
            assert "--trust <trust attributes>   Trust attributes. Default: u,u,u." in result['stdout']
            ...
            pytest.fail("Failed to run pki client-cert-mod --help command.")
...
def test_pki_client_cert_mod(ansible_module):
    ...
    client_out = ansible_module.expect(command=client_cmd,responses={'Enter Password or Pin for "NSS Certificate DB"': "{}".format(constants.CLIENT_DIR_PASSWORD)})
    for result in client_out.values():
        if result['rc'] == 0:
            ...
            pytest.fail("Failed to run pki client-cert-mod command.")
...
def test_pki_client_cert_mod_without_nick(ansible_module):
    ...
    for result in mod_out.values():
        if result['rc'] >= 1:
            ...
            pytest.fail("Failed to run pki client-cert-mod without nick.")
...
def test_pki_client_cert_mod_invalid_nick(ansible_module):
    ...
    for result in mod_out.values():
        if result['rc'] >= 1:
            ...
            pytest.fail("Failed to run pki client-cert-mod with invalid nick.")
...
def test_pki_client_cert_mod_valid_trust(ansible_module):
    ...
    mod_out = ansible_module.expect(command=mod_cmd,responses={'Enter Password or Pin for "NSS Certificate DB"': "{}".format(constants.CLIENT_DIR_PASSWORD)})
    for result in mod_out.values():
        if result['rc'] == 0:
            assert 'Modified certificate "{}"'.format(constants.CA_ADMIN_NICK) in result['stdout']
...
def test_pki_client_cert_mod_invalid_trust(ansible_module):
    ...
    for result in mod_output.values():
        if result['rc'] == 255:
            ...
            pytest.fail("Error: Able to run client-cert-mod with invalid trust flag")
