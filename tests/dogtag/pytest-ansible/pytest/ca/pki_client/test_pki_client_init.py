...
def test_pki_client_init_help(ansible_module, args):
    ...
    if args == '':
        init_out = ansible_module.expect(command=help_cmd,
                                         responses={"Overwrite (y/N)?": 'y'})
        for result in init_out.values():
            print(result)
            if result['rc'] == 0:
                dbs = ansible_module.command('ls /root/.dogtag/nssdb/')
                if result['rc'] == 0:
                    dbs = ansible_module.command('ls /root/.dogtag/nssdb/')
                    for res in dbs.values():
                        assert 'cert9.db' in res['stdout']
                        assert 'key4.db' in res['stdout']
                log.info("Initialized default client directory.")
            else:
                pytest.fail("Failed to initialized the client directory.")

        init_out = ansible_module.expect(command=help_cmd,
                                         responses={"Overwrite (y/N)?": 'N'})
        for result in init_out.values():
            print(result)
            dbs = ansible_module.command('ls /root/.dogtag/nssdb/')
            if result['rc'] == 0:
        ...
        for result in init_out.values():
            if result['rc'] == 0:
                ...
                assert "--force     Force NSS database initialization." in result['stdout']
                assert "--help      Show help message." in result['stdout']
            elif args == 'asdfa':
                assert 'ERROR: Too many arguments specified' in result['stderr']
            else:
                pytest.fail("Failed to run pki client-init --help command.")
...
def test_pki_client_init(ansible_module):
    ...
    for result in client_init_output.values():
        if result['rc'] == 0:
            ...
            pytest.fail("Failed to run pki client-init command.")
    ansible_module.command('rm -rf {}'.format(db2))
...
def test_pki_client_init_no_password(ansible_module):
    ...
    for result in client_init_output.values():
        if result['rc'] >= 1:
            assert "NullPointerException: null" in result['stderr']
           ...
           log.info("Success: Initialized db with default password.")
            ...
            pytest.fail("Failed: Initialized databse without password.")
    ansible_module.command('rm -rf {}'.format(db2))
...
def test_pki_client_init_with_override_the_existing_db_with_force(ansible_module):
    ...
    for result in db2_out.values():
        if result['rc'] == 0:
            ...
            pytest.fail("Failed to run pki client-init --force command.")
    ansible_module.command('rm -rf {}'.format(db2))
...
def test_pki_client_init_with_password_file(ansible_module):
    ...
    for result in client_out.values():
        if result['rc'] == 0:
            ...
            pytest.fail("Failed to initialized client with password file.")

def test_remove_test_setup(ansible_module):
    cmd = "rm -fr /tmp/db1_test /tmp/db2_test"
