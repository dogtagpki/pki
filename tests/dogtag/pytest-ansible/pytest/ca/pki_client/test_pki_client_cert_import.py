...
def test_pki_client_cert_import_help(ansible_module, args):
    ...
    for result in import_help_out.values():
        if result['rc'] == 0:
            ...
            assert "--ca-cert <path>                CA certificate file to import." in result['stdout']
            ...
            pytest.fail("Failed to run pki client-cert-import --help command.")
...
def test_pki_client_cert_import_without_pkcs12(ansible_module):
    ...
    for result in import_out.values():
        if result['rc'] >= 1:
            ...
            pytest.fail("Failed: Able to import cert without command options.")
...
def test_pki_client_cert_import_with_pkcs12_file(ansible_module):
    ...
    for result in cert_find_out.values():
        if result['rc'] == 0:
            ...
            pytest.fail("Failed to run pki client-cert-import command.")


def test_pki_client_cert_import_with_different_pkcs12(ansible_module):
    """
    ...
    client_import = 'pki -d {} -c {} client-cert-import --pkcs12 {} ' \
                    '--pkcs12-password {}'.format(local_db, constants.CLIENT_DIR_PASSWORD,
                                                  '/tmp/sfdew.txt', constants.CLIENT_PKCS12_PASSWORD)
    cert_find_out = ansible_module.command(client_import)
    for result in cert_find_out.values():
        if result['rc'] >= 1:
...
def test_pki_client_cert_import_with_password_file_option(ansible_module):
    ...
    for result in import_out.values():
        if result['rc'] == 0:
            ...
            pytest.fail("Failed to run pki client-cert-import command with password file.")
...
def test_pki_client_cert_import_with_ca_server_option(ansible_module):
    ...
    """
    client_import = 'pki -d {} -c {} -h {} -P {} -p {} client-cert-import RootCA ' \
                    '--ca-server'.format(local_db, constants.CLIENT_DIR_PASSWORD,
                                         constants.MASTER_HOSTNAME,constants.PROTOCOL_UNSECURE,constants.CA_HTTP_PORT)
    ...
    for result in cert_import_out.values():
        if result['rc'] == 0:
            ...
            for res in find_out.values():
                if res['rc'] == 0:
                    assert 'Nickname: CA Signing Certificate' in res['stdout']
                    ...
                    for r in certutil_out.values():
                        if r['rc'] == 0:
                            ...
                            trusts = re.findall("CA.*", r['stdout'])
                            cert_trusts = trusts[0].strip()
                            assert 'CA Signing Certificate' in cert_trusts
                            ...
                            pytest.fail("Failed to check the flags on the CA certificate.")
                else:
                    log.info("Failed to find the certificate in db.")
                    pytest.fail("Failed to find the certificate in db.")
        else:
...
def test_pki_client_cert_import_with_ca_cert_option(ansible_module):
    ...
    """
    get_ca_cert_id = 'pki -P {} -p {} ca-cert-find --name "CA Signing Certificate" --orgUnit "{}" ' \
                     '--org "{}" --matchExactly'.format(constants.PROTOCOL_UNSECURE,constants.CA_HTTP_PORT, instance_name,
    ...
    for res in get_ca_cert_out.values():
        if res['rc'] == 0:
            ...
            export_to_pem = 'pki -P {} -p {} ca-cert-show {} --output {} ' \
                            '--encoded'.format(constants.PROTOCOL_UNSECURE,constants.CA_HTTP_PORT, serial, pem_file)
            ...
            for r in pem_out.values():
                if r['rc'] == 0:
                    ...
                    pytest.fail("Failed to export certificate to the file.")
        else:
            log.error("Failed to get 'CA Signing Certificate'")
            pytest.fail("Failed to get 'CA Signing Certificate'")
...
def test_pki_client_cert_import_with_cert_option(ansible_module):
    ...
    cert_import = 'pki -d {} -c {} client-cert-import "PKI CA Admin" ' \
                  '--cert {}'.format(local_db, constants.CLIENT_DIR_PASSWORD,admin_pem)
...
def test_pki_client_cert_import_with_cert_option_with_diff_files(ansible_module, args):
    ...
    for res in cert_find_output.values():
        if res['rc'] >= 1:
            assert 'ERROR: Incorrect password for internal token' in res['stderr']
...
def test_pki_client_cert_import_valid_trust(ansible_module, trust):
    ...
    for res in cert_find_output.values():
        if res['rc'] == 0:
            ...
            log.info("Failed to import cert to database.")
            pytest.fail("Failed to import cert to database.")
    ansible_module.command('rm -rf {} {}'.format(admin_pem, admin_p12))
...
def test_pki_client_cert_import_invalid_trust(ansible_module):
    ...
    for res in cert_find_output.values():
        if res['rc'] == 0:
            ...
            assert 'ERROR: Command failed. RC: 255' in res['stderr']
...
def test_pki_client_cert_import_valid_serial(ansible_module):
    ...
    cert_import = 'pki -d {} -c {} -P {} -p {} -h localhost client-cert-import "{}" ' \
                  '--serial {}'.format(local_db, constants.CLIENT_DIR_PASSWORD,
                                       constants.PROTOCOL_UNSECURE,constants.CA_HTTP_PORT, user, cert_id)
...
def test_pki_client_cert_import_invalid_serial(ansible_module):
    ...
    for res in cert_import_output.values():
        if res['rc'] >= 1:
            ...
            pytest.fail("Failed: Imported cert with Invalid serial no.")
...
def test_pki_client_cert_import_wrong_pkcspassword(ansible_module):
    ...
    for res in cert_find_output.values():
        if res['rc'] >= 1:
            ...
            pytest.fail("Failed: Imported certificate with wrong pkcs12 password.")
        ...
        if result['rc'] == 0:
            ...
            for res in certutil_out.values():
                if res['rc'] == 0:
                    ...
                    pytest.fail("Failed to run certutil command.")
        else:
            log.info("Failed to import the certificate with the trust 'CT,CT,CT'.")
            pytest.fail("Failed to import the certificate with the trust.")
...
def test_bug_1458429_test_pki_client_cert_import_with_trust_bits(ansible_module):
    ...
    client_import = 'pki -d {} -c {} -P {} -p {} client-cert-import RootCA ' \
                    '--ca-server'.format(local_db, constants.CLIENT_DIR_PASSWORD, constants.PROTOCOL_UNSECURE,
...
def test_pki_client_cert_import_cert_import_from_ca(ansible_module):
    ...
    ansible_module.command(import_ca_admin)
    cert_id = userop.process_certificate_request(ansible_module, subject=subject)
    cert_import = 'pki -d {} -c {} -P {} -p {} -h {} client-cert-import "{}" ' \
                  '--serial {} --trust "CT,C,C"'.format(local_db, constants.CLIENT_DIR_PASSWORD, constants.PROTOCOL_UNSECURE, constants.CA_HTTP_PORT,
        ...
        if res['rc'] == 0:
            ...
            for r in certutil_out.values():
                if r['rc'] == 0:
                    ...
                    pytest.fail("Failed to list the certificate from nssdb.")
        else:
