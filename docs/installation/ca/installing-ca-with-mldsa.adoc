...
[id="installing-ca-with-mldsa"]
= Installing CA with ML-DSA

Follow this process to install a CA subsystem with ML-DSA self-signed CA signing certificate.

Supported ML-DSA strength:

* 44
* 65
* 87

Supported ML-DSA key algorithms:
...
Prepare a deployment configuration, for example `ca-mldsa.cfg`, to deploy CA subsystem. By default the subsystem is deployed into a Tomcat instance called `pki-tomcat`.

A sample deployment configuration, using ML-DSA-44 key type, is available at xref:../../../base/server/examples/installation/ca-mldsa.cfg[/usr/share/pki/server/examples/installation/ca-mldsa.cfg].

To start the installation execute the following command:
[literal]
....
$ pkispawn -f ca-mldsa.cfg -s CA
....
