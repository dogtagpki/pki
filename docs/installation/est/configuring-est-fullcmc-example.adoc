...
[source,bash]
----
curl --cacert ~/.dogtag/pki-ca/ca_signing.crt https://pki.example.com:15443/.well-known/est/
----
...
For ML-DSA device certificate enrollment, copy the ML-DSA profile from the installed examples:

. Copy the ML-DSA profile:
+
[source,bash]
----
cp /usr/share/pki/ca/profiles/ca/estFullcmcMLDSADeviceCert.cfg \
   /var/lib/pki/pki-ca/ca/profiles/ca/estFullcmcMLDSADeviceCert.cfg
----
+
This profile is configured for ML-DSA device certificates with the following key differences from the RSA profile:
+
* Uses `allowedKeys.MLDSA.44`, `allowedKeys.MLDSA.65`, and `allowedKeys.MLDSA.87` for key constraints
* Sets `keyUsageKeyEncipherment=false` (ML-DSA keys are for signing only, not encryption)
* Uses `CMCAuthForEST` for two-tier authentication
* Includes subject name constraint based on RA header client certificate
...
[source,bash]
----
curl --cacert ~/.dogtag/pki-ca/ca_signing.crt \
...
Verify the certificate was issued using the `estFullcmcMLDSADeviceCert` profile.
