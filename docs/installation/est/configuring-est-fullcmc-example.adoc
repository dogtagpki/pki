...
=== Step 10: Configure CA for EST CMC Responses
...
NOTE: This setting is automatically added by the upgrade script for existing CA installations. For new installations, it must be manually configured if EST fullcmc support is needed.

== Part 3: Install EST Subsystem

=== Step 11: Create EST Working Directory
...
=== Step 12: Create EST Configuration
...
=== Step 14: Verify EST Backend Configuration
...
The EST fullcmc endpoint supports two enrollment scenarios, both enforced by the `RAHeaderClientCertSubjectNameConstraint` in the CA profile:

. *Non-agent enrollment*: The client's certificate subject must match the requested certificate subject.
. *Agent enrollment*: A CA agent can request certificates with any subject name (the constraint allows this for agents).

We'll create test users for both scenarios.

=== Step 15: Create Non-Agent Test User
...
=== Step 16: Add Non-Agent User to EST LDAP
...
=== Step 17: Create CA Agent Test User
...
=== Step 18: Add Agent Certificate to CA User and Agent Group
...
=== Step 19: Add Agent User to EST LDAP
...
=== Step 20: Test Non-Agent Enrollment (Matching Subject)
...
=== Step 21: Test Non-Agent Enrollment (Mismatched Subject - Expected Failure)
...
=== Step 22: Test Agent Enrollment (Any Subject - Should Succeed)
