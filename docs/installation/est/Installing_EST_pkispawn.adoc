...
has to be prepared in advance for authentication to work and user
management is out of the EST scope.
...
the database. Installation instruction can be found link:https://www.postgresql.org/download/linux[here].
...
automatically. The connection with the CA uses the credentials
(_username/password_) provided in the configuration file and the
released certificate will be generated with the profile configured
for EST. In such a case the CA signing certificate is
...
After the installation it is recommended to update the EST server
certificates with a new server certificate using a different
profile. Additionally, a certificate for TLS authentication could be
added in the EST nssdb and configured in the file
