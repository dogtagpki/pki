...
Note: The `dc=pki,dc=example,dc=com` subtree is not mandatory, but it's highly recommended in order to avoid conflicts with other applications sharing the same DS instance.
