...
Follow this process using pki cli (`$ man pki-client`) commands to enable SSL connection in DS.
...
[literal,subs="+quotes,verbatim"]
....
# pki -d /etc/dirsrv/slapd-localhost -C /etc/dirsrv/slapd-localhost/pwdfile.txt client-cert-import "CA Signing Cert" --ca-cert ca_signing.crt
....
...
// cfu: this doesn't need to be a role user, right?
As a PKI user (DS admin):
