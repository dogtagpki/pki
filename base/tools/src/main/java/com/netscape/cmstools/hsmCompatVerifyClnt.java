...
/**
 * hsmCompatVerifyClnt - Client-side tool for PKI HSM compatibility verification
 *
...
public class hsmCompatVerifyClnt {
    ...
    public static void main(String[] args) {
        try {
            ...
            // For key archival, use permanent (non-temporary) keys
            // - temporary: false for both RSA and EC
            // - sensitive: -1 (token-dependent) for both
            // - extractable: -1 (token-dependent) for both
            boolean temporary = Boolean.parseBoolean(cmd.getOptionValue("t", "false"));
    ...
    private static void printHelp(Options options) {
        ...
        formatter.printHelp("hsmCompatVerifyClnt",
                ...
                "Use these files with hsmCompatVerifyServ to test HSM operations.\n",
                true);
    }
    ...
    private void run(String clientDB, String clientPasswd, String transportCertFile,
        ...
        if ("RSA".equalsIgnoreCase(algorithm)) {
            ...
            // PKI 10.13 API: RSA uses boolean for sensitive/extractable
            boolean sensitiveBool = (sensitive == 1);
            boolean extractableBool = (extractable == 1);
            ...
            // This matches CRMFPopClient.generateECCKeyPair() at line 669-676
        ...
        // PKI 10.13: Use WrappingParams constructor that auto-detects based on algorithm name
        // This constructor exists in JSS 4.9.x and handles "AES KeyWrap/Wrapped" correctly
        IVParameterSpec ivps = iv != null ? new IVParameterSpec(iv) : null;

        // Use a dummy encryptOID - the constructor will override it based on wrapName
        String encryptOID = org.mozilla.jss.crypto.KeyWrapAlgorithm.AES_CBC_PAD_OID.toString();

        WrappingParams params = new WrappingParams(
            encryptOID,  // Will be auto-detected from wrapName
            keywrapAlg,  // The wrap algorithm string (e.g., "AES KeyWrap/Wrapped")
            algorithm,   // Private key algorithm
            ivps,        // Encryption IV
            ivps         // Wrapping IV
        );

        // CRITICAL FIX: The WrappingParams constructor always sets skWrapAlgorithm=RSA
        // (see WrappingParams.java line 114), but we need RSA_OAEP when --oaep is specified.
        // Without this explicit override, the --oaep flag would not work correctly.
        if (useOAEP) {
        ...
        log("  Run hsmCompatVerifyServ to test HSM archival/recovery with these files", verbose);
    }
