...
public class hsmCompatVerifyClnt {
    ...
    public static void main(String[] args) {
        try {
            ...
            // Support both --user-key-type (new) and --algorithm (old, for backward compatibility)
            String userKeyType = cmd.getOptionValue("user-key-type");
            if (userKeyType == null) {
                userKeyType = cmd.getOptionValue("algorithm", pqcMode ? "ML-KEM" : DEFAULT_ALGORITHM);
            }
            String algorithm = userKeyType; // For backward compatibility with existing code

            // Auto-enable PQC mode if ML-KEM is selected
            if ("ML-KEM".equalsIgnoreCase(userKeyType) && !pqcMode) {
                System.out.println("INFO: Auto-enabling --pqc mode for ML-KEM user key type");
                pqcMode = true;
            }
            ...
            if (clientPasswdFile != null) {
                System.out.println("  --client-passwd-file " + clientPasswdFile);
            }
            ...
            if (userKeyType.equalsIgnoreCase("ML-KEM")) {
                System.out.println("    User key: ML-KEM-" + pqcKemAlgorithm.replaceAll("[^0-9]", ""));
    ...
    private static Options createOptions() {
        ...
        options.addOption(Option.builder()
                ...
                .hasArg()
                .argName("alg")
                .desc("(Deprecated: use --user-key-type) Key algorithm: RSA or EC (default: RSA)")
                .build());
        ...
        options.addOption(Option.builder()
                ...
                .hasArg()
                .argName("algorithm")
                .desc("PQC KEM algorithm for transport/storage: ml-kem-512, ml-kem-768, ml-kem-1024 (default: ml-kem-768)")
                .build());
    ...
    private void runArchivalPQC(String clientDB, String clientPasswd, String transportCertFile,
        ...
        log("Transport/Storage: ML-KEM-" + kemAlgorithm.replaceAll("[^0-9]", ""), verbose);
        ...
        if ("RSA".equalsIgnoreCase(userKeyType)) {
            ...
            log("  Algorithm: ML-KEM-" + kemAlgorithm.replaceAll("[^0-9]", ""), verbose);

            // Map algorithm to strength (normalize input to handle both hyphenated and non-hyphenated formats)
            String normalizedAlg = kemAlgorithm.toLowerCase().replace("-", "");
            int kemStrength;
