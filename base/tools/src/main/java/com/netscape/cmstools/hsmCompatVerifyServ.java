...
public class hsmCompatVerifyServ {
    ...
    public static Options createOptions() {
        ...
        options.addOption(null, "legacyPKCS12", false, "Use legacy PKCS#12 format (PBE_SHA1_DES3_CBC). Default: non-legacy (AES-256-CBC)");
    ...
    public static void printHelp() {
        ...
        System.out.println("      --ca-nickname <nick>      CA nickname on HSM (default: CA Signing Certificate)");
        System.out.println();
        System.out.println("Transport Certificate Options (RSA-2048 only, required for key wrapping):");
        System.out.println("      --transport-subject <dn>  Transport subject DN (default: CN=KRA Transport,O=Dogtag)");
        System.out.println("      --transport-validity <d>  Transport validity (default: 365)");
        System.out.println("      --transport-nickname <n>  Transport nickname (default: KRA Transport Certificate)");
        System.out.println();
        System.out.println("Storage Certificate Options (RSA-2048 only, required for key wrapping):");
        System.out.println("      --storage-subject <dn>    Storage subject DN (default: CN=KRA Storage,O=Dogtag)");
        System.out.println("      --storage-validity <d>    Storage validity (default: 365)");
        System.out.println("      --storage-nickname <n>    Storage nickname (default: KRA Storage Certificate)");
        System.out.println();
        ...
        System.out.println("                                Default: <client-db>/kra-test-wrapped-session.bin");
        System.out.println("      --wrapped-private <file>  Wrapped private key file");
        System.out.println("                                Default: <client-db>/kra-test-wrapped-private.bin");
        System.out.println("      --public-key <file>       Public key file (DER format)");
        System.out.println("                                Default: <client-db>/kra-test-public.der");
        System.out.println("      --iv-file <file>          IV file (for CBC mode algorithms)");
        System.out.println("                                Default: <client-db>/kra-test-iv.bin");
        System.out.println();
        ...
        System.out.println("    --verbose");
        System.out.println();
        System.out.println("Archive only (creates LDIF, no recovery, assuming HSM password in file):");
        System.out.println("  " + TOOL_NAME + " \\");
        System.out.println("    --hsm-token \"TestHSM\" --hsm-token-passwd-file ~/hsm.pwd \\");
        System.out.println("    --subject-dn \"CN=Test User\" \\");
        ...
        System.out.println("    --verbose");
        System.out.println();
        System.out.println("Recovery only (from existing LDIF, assuming passwords in files):");
        System.out.println("  " + TOOL_NAME + " \\");
        System.out.println("    --hsm-token \"TestHSM\" --hsm-token-passwd-file ~/hsm.pwd \\");
    ...
    public void runSetup(
        ...
        // We need to initialize with PKI server DB to have a place to store public certs
     ...
     */
    private void displayCertInfo(X509Certificate cert) throws Exception {
        log("    Subject: " + cert.getSubjectDN());
        log("    Issuer: " + cert.getIssuerDN());
        log("    Serial: 0x" + cert.getSerialNumber().toString(16));

        // Convert JSS X509Certificate to Java X509Certificate to get validity dates
        // In PKI 10.13, JSS X509Certificate doesn't have getNotBefore/getNotAfter
        ...
        java.security.cert.X509Certificate javaCert =
            (java.security.cert.X509Certificate) cf.generateCertificate(bis);

        log("    Valid: " + javaCert.getNotBefore() + " to " + javaCert.getNotAfter());
    }
    ...
    public void runTest(
        ...
        // Set up password store for internal token
        // This is needed so the internal token can be accessed during P12 creation
        NSSDatabase pkiservNSSDB = new NSSDatabase(pkiservDB);

        // Load the password from the provided password file
        PlainPasswordFile passwordStore = null;
        String pkiservPasswd = null;
        if (pkiservPasswdFile != null) {
            try {
                pkiservPasswd = new String(java.nio.file.Files.readAllBytes(
                    java.nio.file.Paths.get(pkiservPasswdFile))).trim();
                passwordStore = new PlainPasswordFile();
                passwordStore.putPassword("internal", pkiservPasswd);
                pkiservNSSDB.setPasswordStore(passwordStore);
                log("  - Password loaded from: " + pkiservPasswdFile);
            } catch (Exception e) {
                log("  - Warning: Could not load password file " + pkiservPasswdFile + ": " + e.getMessage());
                log("  - Will attempt to continue without internal token login");
                passwordStore = null;
            }
        } else {
            log("  - Warning: No password file provided for internal token");
            log("  - Will attempt to continue without internal token login");
        }
        ...
        // Login to internal token (needed for non-legacy P12 creation)
        CryptoToken internalToken = manager.getInternalKeyStorageToken();
        log("  - Internal token name: " + internalToken.getName());
        log("  - Internal token logged in status: " + internalToken.isLoggedIn());

        if (!internalToken.isLoggedIn()) {
            log("  - Internal token not logged in, attempting login");
            // Get password from password store
            if (passwordStore != null) {
                try {
                    String internalPasswd = passwordStore.getPassword("internal", 0);
                    if (internalPasswd != null && !internalPasswd.isEmpty()) {
                        log("  - Password retrieved from password store (length: " + internalPasswd.length() + ")");
                        Password internalPassword = new Password(internalPasswd.toCharArray());
                        try {
                            internalToken.login(internalPassword);
                            log("  - Internal token login successful");
                            log("  - Internal token logged in status after login: " + internalToken.isLoggedIn());
                        } finally {
                            internalPassword.clear();
                        }
                    } else {
                        log("  - Warning: No password found for internal token in password store");
                    }
                } catch (Exception e) {
                    log("  - Warning: Could not login to internal token: " + e.getMessage());
                    e.printStackTrace();
                    log("  - This may cause issues with non-legacy P12 creation");
                }
            } else {
                log("  - Warning: No password store available, cannot login to internal token");
            }
        } else {
            log("  - Internal token already logged in");
        }
        ...
        if (recoverOnly) {
            ...
            for (int recordNum = 0; recordNum < keyRecords.size(); recordNum++) {
                ...
                log("  - PKCS#12 format: " + (legacyPKCS12 ? "Legacy (PBE_SHA1_DES3_CBC)" : "Modern (AES-KWP)"));
            ...
            log("");
            log("To test PKCS#12 import:");
            for (String outFile : outputFiles) {
                log("  pk12util -d " + clientDB + " -i " + outFile);
            }
            ...
            if (archiveOnly) {
                ...
                log("    --output <output.p12> --recovery-passwd <password>");
                return;  // Stop here in archival-only mode
            }
        }  // End of archival mode
        ...
        log("");
        log("To test PKCS#12 import:");
        log("  pk12util -d " + clientDB + " -i " + outputFile);
    }
    ...
    private CertificateExtensions createCACertExtensions(X509Key publicKey) throws Exception {
        ...
        // Generate Extensions (PKI 10.13 API doesn't take publicKey parameter)
        Extensions extensions = generator.createExtensions();

        // Manually add SKID extension since PKI 10.13 API requires PKCS10
        // but we're creating certs directly
        byte[] skidBytes = CryptoUtil.generateKeyIdentifier(publicKey.getKey());
        org.mozilla.jss.netscape.security.x509.SubjectKeyIdentifierExtension skidExt =
            new org.mozilla.jss.netscape.security.x509.SubjectKeyIdentifierExtension(skidBytes);
        extensions.parseExtension(skidExt);
    ...
    private CertificateExtensions createKRACertExtensions(X509Key publicKey, X509Certificate issuerCert) throws Exception {
        ...
        // Generate Extensions (PKI 10.13 API: issuer cert for AKID)
        org.mozilla.jss.crypto.X509Certificate jssIssuerCert = null;
        if (issuerCert != null) {
            // Convert java.security.cert.X509Certificate to JSS X509Certificate
            byte[] issuerBytes = issuerCert.getEncoded();
            jssIssuerCert = CryptoManager.getInstance()
                .importCertPackage(issuerBytes, "temp-issuer");
        }
        Extensions extensions = generator.createExtensions(jssIssuerCert, null);
    ...
    private CertificateExtensions createUserCertExtensions(X509Key publicKey, X509Certificate issuerCert) throws Exception {
        ...
        if (issuerCert != null) {
            ...
            byte[] issuerBytes = issuerCert.getEncoded();
            jssIssuerCert = CryptoManager.getInstance()
                .importCertPackage(issuerBytes, "temp-issuer-user");
        }
        Extensions extensions = generator.createExtensions(jssIssuerCert, null);
    ...
    private void createLDIFFile(
        ...
        // Add blank line at end of record (required by KRATool to detect end of record)
        ldif.append("\n");
