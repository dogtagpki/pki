...
public class hsmCompatVerifyServ {
    ...
    public static void printHelp() {
        ...
        System.out.println();
        System.out.println("Multi-Token Options (optional, for CA/KRA on separate tokens):");
        System.out.println("      --ca-token <name>         Token for CA signing cert (default: --hsm-token value)");
        ...
        System.out.println("      --kra-token <name>        Token for KRA transport/storage certs (default: --hsm-token value)");
    ...
    public static void main(String args[]) throws Exception {
        ...
        // Per-subsystem token options (with fallback to hsm-token)
        // CA token
        String caToken = cmd.getOptionValue("ca-token");
        if (caToken == null) {
            caToken = hsmToken;  // Default to hsm-token
        }
        String caTokenPasswd = cmd.getOptionValue("ca-token-passwd");
        String caTokenPasswdFile = cmd.getOptionValue("ca-token-passwd-file");

        // Read CA token password from file if specified, otherwise use passwd, otherwise default to hsm-token-passwd
        ...
        if (caTokenPasswd == null) {
            caTokenPasswd = hsmTokenPasswd;  // Default to hsm-token-passwd
        }

        // KRA token (for transport and storage)
        String kraToken = cmd.getOptionValue("kra-token");
        if (kraToken == null) {
            kraToken = hsmToken;  // Default to hsm-token
        }
        ...
        // Read KRA token password from file if specified, otherwise use passwd, otherwise default to hsm-token-passwd
        ...
        if (kraTokenPasswd == null) {
            kraTokenPasswd = hsmTokenPasswd;  // Default to hsm-token-passwd
        }
     ...
     * @param caNickname CA certificate nickname on HSM
    ...
    public void runIssueCsr(
        ...
        log("=== KRA HSM Compatibility Verification - Issue CSR Mode ===");
        ...
        log("");
        log("Step 2: Initializing HSM token");
        ...
        org.mozilla.jss.crypto.CryptoStore store = hsmTokenObj.getCryptoStore();
    ...
    public void runTest(
        ...
        if (recoverOnly) {
            ...
            SymmetricKey unwrappedSessionKey = null;
                try {
                    log("  Attempting: Unwrap AES-256 session key using " + rsaWrapAlg + " with RSA transport private key");
                ...
                logError("Failed to unwrap session key on HSM", rsaWrapAlg.toString(),
                        "Unwrap AES-256 symmetric key using RSA private key", e);
                throw e;
            }
