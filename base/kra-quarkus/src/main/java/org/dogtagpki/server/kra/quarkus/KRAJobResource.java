...
/**
 * JAX-RS resource for KRA job management.
 * Replaces KRAJobServlet.
 */
@Path("v2/jobs")
public class KRAJobResource {

    private static final Logger logger = LoggerFactory.getLogger(KRAJobResource.class);
    ...
    public Response getJobs() throws Exception {
        logger.debug("KRAJobResource.getJobs()");
    ...
    public Response getJob(@PathParam("jobId") String jobId) throws Exception {
        logger.debug("KRAJobResource.getJob(): jobId={}", jobId);
        Principal principal = securityContext.getUserPrincipal();
    ...
    public Response startJob(@PathParam("jobId") String jobId) throws Exception {
        logger.debug("KRAJobResource.startJob(): jobId={}", jobId);
        Principal principal = securityContext.getUserPrincipal();
