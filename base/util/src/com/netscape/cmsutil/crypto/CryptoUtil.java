...
//        System.out.println("CryptoUtil: generateECCKeyPair: after KeyPairGenerator initialize with:" + curveName);
    ...
    public static PrivateKey importPKIArchiveOptions(
            ...
            throws InvalidBERException, NoSuchAlgorithmException, Exception {
        ...
        EncryptionAlgorithm encAlg =
            EncryptionAlgorithm.fromOID(algId.getOID());

        /* This is a bit of a hack.  In the past, PKIArchiveOptions
         * export was hardcoded to DES3_CBC_PAD, and import was
         * correspondingly hardcoded to SymmetricKey.Type.DES3 and
         * KeyWrapAlgorithm.DES3_CBC_PAD.
         *
         * Now, we have change the export routine to AES_256_CBC.
         * But we still need to be able to import a PKIArchiveOptions
         * generated by an older version.  So we do the following:
         *
         * 1. Inspect the AlgorithmId.  If it is unrecognised, fail.
         * 2. Check the Alg name (defined in JSS).
         *    - if "DESede", use KeyWrapAlgorithm.DES3_CBC_PAD
         *    - if "AES", use KeyWrapAlgorithm.AES_CBC_PAD
         *    - otherwise, fail.
         *
         * Obviously this heuristic does not handle all possible
         * cryptographically valid inputs, but it does handle all
         * the ciphers previously or currently used by Dogtag for
         * exporting PKIArchiveOptions, plus a few more.
         */
        SymmetricKey.Type symKeyType;
        KeyWrapAlgorithm keyWrapAlg;
        String encAlgTypeString = encAlg.getAlg().toString();
        if (encAlgTypeString.equals("DESede")) {
            symKeyType = SymmetricKey.Type.DES3;
            keyWrapAlg = KeyWrapAlgorithm.DES3_CBC_PAD;
        } else if (encAlgTypeString.equals("AES")) {
            symKeyType = SymmetricKey.Type.AES;
            keyWrapAlg = KeyWrapAlgorithm.AES_CBC_PAD;
        } else {
            throw new NoSuchAlgorithmException(
                "Algorithm is not supported for PKIArchiveOptions import");
        }

        SymmetricKey sk = unwrap(token, symKeyType, 0, SymmetricKey.Usage.DECRYPT, unwrappingKey, encSymKey.getBits(),
        ...
        return unwrap(token, pubkey, false, sk, encPrivKey.getBits(), keyWrapAlg, ivps);
    }
