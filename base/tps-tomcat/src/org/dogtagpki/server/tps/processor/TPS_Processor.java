...
import org.dogtagpki.server.tps.apdu.APDU_Response;
import org.dogtagpki.server.tps.apdu.Get_Data_APDU;
import org.dogtagpki.server.tps.apdu.Get_Status_APDU;
...
import org.dogtagpki.server.tps.msg.TokenPDURequest;
import org.dogtagpki.server.tps.msg.TokenPDUResponse;

import com.netscape.certsrv.apps.CMS;
import com.netscape.certsrv.base.EBaseException;
import com.netscape.certsrv.base.IConfigStore;

public abstract class TPS_Processor {
    ...
    TPS_Status Process(TPSSession session, TPSMessage beginMsg) throws EBaseException {
        return TPS_Status.STATUS_NO_ERROR;
    }

    TPSBuffer extractTokenNSM(TPSBuffer cplc_data) {
    ...
    TPSBuffer extractTokenCUID(TPSBuffer cplc_data) {
    ...
    int SelectCardManager(TPSSession session, String prefix, String tokenType) {
    ...
    int SelectApplet(TPSSession session, byte p1, byte p2, TPSBuffer aid)  {
        ...
        Select_APDU select_apdu = new Select_APDU(p1, p2, aid);

        APDU_Response respApdu = HandleAPDURequest(session, select_apdu);
    ...
    TPSBuffer GetStatus(TPSSession session) {

        CMS.debug("In TPS_Processor.GetStatus.");

        TPSBuffer result = null;

        Get_Status_APDU get_status_apdu = new Get_Status_APDU();

        APDU_Response respApdu = HandleAPDURequest(session, get_status_apdu);
    ...
    APDU_Response HandleAPDURequest(TPSSession session, APDU apdu) {
        ...
        try {
            response_msg = (TokenPDUResponse) session.ReadMsg();
        } catch (Exception e) {
    ...
    TPSBuffer GetCplcData(TPSSession session) {
        CMS.debug("In TPS_Processor.GetData");
        TPSBuffer result = null;

        Get_Data_APDU get_data_apdu = new Get_Data_APDU();

        APDU_Response respApdu = HandleAPDURequest(session, get_data_apdu);
    ...
    TPS_Status Format(TPSSession session, TPSMessage message) throws EBaseException {
    ...
    boolean checkTokenPDUResponse(APDU_Response response) {
