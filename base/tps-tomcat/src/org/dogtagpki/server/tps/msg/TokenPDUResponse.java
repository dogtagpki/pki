...
public class TokenPDUResponse extends TPSMessage {
    public TokenPDUResponse(String message) {
        ...
        if( decoded_pdu_data.length ==  sizeI ) {
            ...
            TPSBuffer responseBuffer = new TPSBuffer(decoded_pdu_data, decoded_pdu_data.length);

            response = new APDU_Response(responseBuffer);
    ...
    private APDU_Response response;

    public APDU_Response GetResponseAPDU() {
        return response;
    }
