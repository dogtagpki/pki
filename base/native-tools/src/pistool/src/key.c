             ...
             PRUint8 *hexKCV )
{
    ...
    //PAS MOD FE1 - Adjust right brackets to hold only 1 byte, purely for display purposes
    PR_fprintf( PR_STDOUT, 
                "[    ]  [  ]\r" );
...
//PAS MOD FE1 - updated to read only 3 bytes (6 asciihex characters) from the KCV buffer
//    PR_fprintf( PR_STDOUT,
...
{
    int           len;
    unsigned char value[AES_128_LENGTH_BYTES];
    unsigned char ivData[AES_CBC_IV_LENGTH];
    PK11SymKey   *key       = NULL;
    ...
    if( ( PL_strcmp( keyType, RESIDENT_KEY ) != 0 )   &&
        ...
        case KEY_ALG_AES:
        {
            // AES key of length 16, 24, or 32 bytes
            ...
            // Import the key to the token non-permanently
            key = PK11_ImportSymKeyWithFlags(
                    /* slot */              slot,
                    /* mechanism type */    CKM_AES_CBC,
    ...
    {
        // AES
        // The data to be encrypted is 16 bytes, each byte having the value 1.
        // The IV is 16 bytes, all zeros.
        PORT_Memset( value, 1, sizeof( value ) );
        PORT_Memset(ivData, 0, sizeof(ivData));

        ivItem.type = siBuffer;
        ivItem.data = ivData;
        ivItem.len = AES_CBC_IV_LENGTH;

        ivParam = PK11_ParamFromIV(CKM_AES_CBC, &ivItem);
        if(ivParam == NULL)
        {
    ...
    {
        // DES2 and DES3
        // The data to be encrypted is 16 bytes, each byte having the value 0.
    ...
    if( ! context ) {
        PR_fprintf( PR_STDERR,
                    "ERROR %i:  Failed to create crypto context!\n\n\n", PR_GetError() );
        status = SECFailure;
        goto done;
    }
    ...
    s = PK11_CipherOp(
        ...
        /* output                */  &value[0],
        /* output length         */  &len,
        /* maximum output length */  sizeof(value),
        /* input                 */  &value[0],
        /* input length          */  sizeof(value) );
    if( s != SECSuccess) {
        PR_fprintf( PR_STDERR,
                    "ERROR %i:  CipherOp Failed!\n\n\n", PR_GetError() );
        status = SECFailure;
        goto done;
    }
                             ...
                             KeyAlgorithm   keyAlg )
{
    ...
    // Parity bits do not apply to AES keys
...
TKS_StoreSymmetricKeyAndNameIt( char              *symmetricKeyName,
                                ...
                                PK11SymKey        *symKey,
                                KeyAlgorithm       keyAlg,
								PRBool prBoolSecure ) //PAS MOD FE4
{
    PK11SymKey *newSymKey             = NULL;
    PRIntn      KCVLen                = KCV_LENGTH;
    PRUint8    *KCV                   = NULL;
    ...
    if(prBoolSecure){
        ...
        rvSymmetricKeyname = PK11_WriteRawAttribute(PK11_TypeSymKey, (void *)newSymKey, CKA_EXTRACTABLE, &seciCKAExtractable);

        if( rvSymmetricKeyname != SECSuccess ){
        	PR_fprintf(PR_STDOUT, "WARNING: Setting the CKA_EXTRACTIBLE attribute for the master key to false failed!\n");

            if( PORT_GetError() == SEC_ERROR_READ_ONLY ){
            	PR_fprintf(PR_STDOUT, "CKA_EXTRACTIBLE is readonly!\n");
            }
...
KeyAlgorithm
TKS_DetectKeyAlgorithmFromMechanism(CK_MECHANISM_TYPE mechanism)
{
    // Is it AES?
    if((mechanism & CKM_AES_KEY_GEN) == CKM_AES_KEY_GEN)
        return KEY_ALG_AES;
    // Is it DES?
    else if((mechanism & CKM_DES2_KEY_GEN) == CKM_DES2_KEY_GEN)
        return KEY_ALG_DES;
    else
        return KEY_ALG_UNKNOWN;
}
