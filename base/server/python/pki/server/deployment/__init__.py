...
class PKIDeployer:
    ...
    def prepare_cert_request_id(self, subsystem, tag, request):
        ...
        if request_id_generator != 'random':
            ...
            # let pki-server ca-cert-create generate the request ID from CSR
            request.systemCert.requestID = None

    def import_cert_request(self, subsystem, tag, request):

        self.prepare_cert_request_id(subsystem, tag, request)
    ...
    def ensure_csr_file(self, subsystem, tag, request):
    ...
    def create_cert(self, subsystem, request):
        ...
        if config.str2bool(self.mdict['pki_ds_setup']):
            tag = request.tag
            kwargs['csr_path'] = self.ensure_csr_file(subsystem, tag, request)
            kwargs['csr_format'] = 'PEM'
            kwargs['import_cert'] = True

            if request.systemCert.requestType:
                kwargs['request_type'] = request.systemCert.requestType

            if request.systemCert.dnsNames:
                kwargs['dns_names'] = request.systemCert.dnsNames

            if request.systemCert.adjustValidity:
                kwargs['adjust_validity'] = True

        cert_data = subsystem.create_cert(**kwargs)
    ...
    def create_admin_cert(self, subsystem, csr):
        ...
        if config.str2bool(self.mdict['pki_ds_setup']):
            self.prepare_cert_request_id(subsystem, 'admin', request)
    ...
    def setup_admin_user(self, subsystem, cert_data):
        ...
        subsystem.add_group_members(groups, uid)
