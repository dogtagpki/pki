...
class ConfigClient:
    ...
    def save_admin_csr(self):
        ...
        with open(self.mdict['pki_admin_csr_path'], "w", encoding='utf-8') as f:
            f.write("-----BEGIN CERTIFICATE REQUEST-----\n")
        with open(os.path.join(
                  self.mdict['pki_client_database_dir'],
                  "admin_pkcs10.bin.asc"), "r", encoding='utf-8') as f:
            admin_certreq = f.read()
        with open(self.mdict['pki_admin_csr_path'], "a", encoding='utf-8') as f:
            f.write(admin_certreq)
            f.write("-----END CERTIFICATE REQUEST-----")
        # Read in and print Admin certificate request
        with open(self.mdict['pki_admin_csr_path'], "r", encoding='utf-8') as f:
            admin_certreq = f.read()
