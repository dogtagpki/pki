...
class AuditCLI(pki.cli.CLI):
    ...
    def __init__(self, parent):
        ...
        self.add_module(EnableSignedAudit(self))

class EnableSignedAudit(pki.cli.CLI):

    def __init__(self, parent):
        super(EnableSignedAudit, self).__init__(
            'signed', 'Enable/Disable signed audit event')

    def print_help(self):
        print('Usage: pki-server %s-audit-signed [OPTIONS]' % self.parent.parent.name)
        print()
    ...
    def execute(self, argv):
        ...
        signed_enable = subsystem.signed_audit_log(enabled)
        if signed_enable:
            if enabled:
