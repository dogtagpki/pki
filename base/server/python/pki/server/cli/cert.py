...
class CertCreateCLI(pki.cli.CLI):
    ...
    def execute(self, argv):
        ...
        try:
            ...
            if output:
                ...
                if not serial and not is_permanent_cert:
            ...
            if cert_tag == 'sslserver':
                ...
                self.create_subsystem_cert(is_permanent_cert=is_permanent_cert, serial=serial,
                ...
                self.create_ocsp_cert(is_permanent_cert=is_permanent_cert, serial=serial,
                                      subsystem=subsystem, c_nssdb=client_nssdb_location,
                                      c_nssdb_pass=client_nssdb_password, c_cert=client_cert,
                                      new_cert_file=new_cert_file, tmpdir=tmpdir)

            elif cert_tag == 'audit_signing':
                self.create_audit_cert(is_permanent_cert=is_permanent_cert, serial=serial,
                                       subsystem=subsystem, c_nssdb=client_nssdb_location,
                                       c_nssdb_pass=client_nssdb_password, c_cert=client_cert,
                                       new_cert_file=new_cert_file, tmpdir=tmpdir)
    ...
    def create_ocsp_cert(self, subsystem, c_nssdb, c_nssdb_pass, tmpdir,
                         c_cert, is_permanent_cert, new_cert_file, serial):
