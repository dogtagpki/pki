...
class CryptographyCryptoProvider(CryptoProvider):
    ...
    def __init__(self, transport_cert_nick, transport_cert=None,
                 transport_cert_fname=None, backend=default_backend()):
        """ Initialize python-cryptography
        """
        super(CryptographyCryptoProvider, self).__init__()
        self.certs = {}

        if transport_cert is None and transport_cert_fname is None:
            raise ValueError(
                "transport_cert or transport_cert_fname must be specified"
            )
        if transport_cert is not None and transport_cert_fname is not None:
            raise ValueError(
                "Both transport_cert and transport_cert_fname " +
                "cannot be specified")

        if transport_cert_fname is not None:
