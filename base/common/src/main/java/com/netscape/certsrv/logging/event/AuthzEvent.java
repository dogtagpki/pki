...
public class AuthzEvent extends SignedAuditEvent {
    ...
    public AuthzEvent(String messageID) {
        super(messageID);
    }

    public static AuthzEvent createSuccessEvent(
            String subjectID,
            String aclResource,
            String operation) {

        AuthzEvent event = new AuthzEvent(AUTHZ_SUCCESS);

        event.setAttribute("SubjectID", subjectID);
        event.setAttribute("Outcome", ILogger.SUCCESS);
        event.setAttribute("aclResource", aclResource);
        event.setAttribute("Op", operation);
    ...
    public static AuthzEvent createFailureEvent(
            String subjectID,
            String aclResource,
            String operation) {

        AuthzEvent event = new AuthzEvent(AUTHZ_FAIL);
