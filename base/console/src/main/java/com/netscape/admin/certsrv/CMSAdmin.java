...
public class CMSAdmin extends AbstractServerObject
    ...
    /**
     * Set the title bar in the following format:
     * <p>
     *
     * <pre>
    ...
    public void startupInstallationWizard(IPage viewInstance) {
        ...
        if (success) {
            InstallWizardInfo wizardInfo = new InstallWizardInfo(mConsoleInfo);
    ...
    private String getAttrVal(LDAPEntry entry, String name) {
        LDAPAttribute findAttr = entry.getAttribute(name, LDAPUtil.getLDAPAttributeLocale());
        if (findAttr != null) {
