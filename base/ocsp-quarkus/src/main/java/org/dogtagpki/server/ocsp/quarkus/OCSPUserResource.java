...
/**
 * JAX-RS resource for OCSP user management.
 * Replaces OCSPUserServlet.
 */
@Path("v2/admin/users")
public class OCSPUserResource {

    private static final Logger logger = LoggerFactory.getLogger(OCSPUserResource.class);
    ...
    public Response findUsers(
        ...
        logger.debug("OCSPUserResource.findUsers()");
    ...
    public Response addUser(String requestData) throws Exception {
        logger.debug("OCSPUserResource.addUser()");
    ...
    public Response getUser(@PathParam("userId") String userId) throws Exception {
        logger.debug("OCSPUserResource.getUser(): userId={}", userId);
    ...
    public Response modifyUser(@PathParam("userId") String userId, String requestData) throws Exception {
        logger.debug("OCSPUserResource.modifyUser(): userId={}", userId);
        UserData userData = JSONSerializer.fromJSON(requestData, UserData.class);
    ...
    public Response removeUser(@PathParam("userId") String userId) throws Exception {
        logger.debug("OCSPUserResource.removeUser(): userId={}", userId);
    ...
    public Response findUserCerts(
            ...
            @QueryParam("start") @DefaultValue("0") int start,
            @QueryParam("size") @DefaultValue("20") int size) throws Exception {
        logger.debug("OCSPUserResource.findUserCerts(): userId={}", userId);
    ...
    public Response addUserCert(@PathParam("userId") String userId, String requestData) throws Exception {
        logger.debug("OCSPUserResource.addUserCert(): userId={}", userId);
        UserCertData certData = JSONSerializer.fromJSON(requestData, UserCertData.class);
        UserCertData cert = createBase().addUserCert(userId, certData, Locale.getDefault());
        if (cert == null) {
            return Response.noContent().build();
        }
        String encodedCertID = URLEncoder.encode(cert.getID(), "UTF-8");
        URI location = uriInfo.getAbsolutePathBuilder().path(encodedCertID).build();
        return Response.created(location).entity(certData.toJSON()).build();
    }
    ...
    public Response getUserCert(@PathParam("userId") String userId, @PathParam("certId") String certId) throws Exception {
        logger.debug("OCSPUserResource.getUserCert(): userId={}, certId={}", userId, certId);
    ...
    public Response removeUserCert(@PathParam("userId") String userId, @PathParam("certId") String certId) throws Exception {
        logger.debug("OCSPUserResource.removeUserCert(): userId={}, certId={}", userId, certId);
    ...
    public Response findUserMemberships(
            ...
            @QueryParam("start") @DefaultValue("0") int start,
            @QueryParam("size") @DefaultValue("20") int size) throws Exception {
        logger.debug("OCSPUserResource.findUserMemberships(): userId={}", userId);
    ...
    public Response addUserMembership(@PathParam("userId") String userId, String groupId) throws Exception {
        logger.debug("OCSPUserResource.addUserMembership(): userId={}, groupId={}", userId, groupId);
    ...
    public Response removeUserMembership(@PathParam("userId") String userId, @PathParam("groupId") String groupId) throws Exception {
        logger.debug("OCSPUserResource.removeUserMembership(): userId={}, groupId={}", userId, groupId);
