...
public class CRLIssuingPoint implements ICRLIssuingPoint, Runnable {
    ...
    void generateDeltaCRL(
        ...
        try {
            ...
            audit(new DeltaCRLGenerationSuccessEvent(getAuditSubjectID()));
        ...
        try {
            ...
            audit(new DeltaCRLPublishingEvent(getAuditSubjectID()));

        } catch (Throwable e) {
            CMS.debug(e);
            log(ILogger.LL_FAILURE,
                    CMS.getLogMessage("CMSCORE_CA_ISSUING_PUBLISH_DELTA", mCRLNumber.toString(), e.toString()));
            audit(new DeltaCRLPublishingEvent(getAuditSubjectID(), e.getMessage()));
        }
    }
